...
#define RMI4_VTG_MIN_UV		2600000
...
void synaptics_rmi4_set_i2c_flag(int value);
...
static ssize_t synaptics_rmi4_f01_gpio_status_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(gpio_status, S_IWUGO,
			synaptics_rmi4_show_error,
			synaptics_rmi4_f01_gpio_status_store),
...
static DEVICE_ATTR(tp_status, 0664,
		synaptics_tp_status_show, synaptics_rmi4_store_error);
...
static DEVICE_ATTR(auto_test, 0664,
		synaptics_tp_auto_test_show, synaptics_rmi4_store_error);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(gpio_is_valid(gpio)){
		...
		dev_err(rmi4_data->pdev->dev.parent,"%s:    gpio-%d  status= %d\n",__func__,gpio,retval);
		//scnprintf(buf, PAGE_SIZE, "%d\n",retval);
	}else{
		dev_err(rmi4_data->pdev->dev.parent,"%s:    gpio-%d  is not valid \n",__func__,gpio);
	}
...
#define CBW_POWER 0
#if CBW_POWER
static int synaptics_rmi4_power_init(struct synaptics_rmi4_data *rmi4_data)
{
	int rc;
	struct synaptics_dsx_board_data *bdata = rmi4_data->hw_if->board_data;

	bdata->vdd = regulator_get(&rmi4_data->pdev->dev, "vdd");
	if (IS_ERR(bdata->vdd)) {
		rc = PTR_ERR(bdata->vdd);
		dev_err(&rmi4_data->pdev->dev, "Regulator get failed vdd rc=%d\n", rc);
	}
	if ((bdata->vdd != NULL) && (regulator_count_voltages(bdata->vdd) > 0) ) {
		rc = regulator_set_voltage(bdata->vdd, RMI4_VTG_MIN_UV, RMI4_VTG_MAX_UV);
		if (rc) {
			dev_err(&rmi4_data->pdev->dev, "Regulator set_vtg failed vdd rc=%d\n", rc);
			goto reg_vdd_put;
		}
	}

	bdata->vcc_i2c = regulator_get(&rmi4_data->pdev->dev, "vcc_i2c");
	if (IS_ERR(bdata->vcc_i2c)) {
		rc = PTR_ERR(bdata->vcc_i2c);
		dev_err(&rmi4_data->pdev->dev, "Regulator get failed vcc_i2c rc=%d\n", rc);
		goto reg_vdd_set_vtg;
	}

	if (regulator_count_voltages(bdata->vcc_i2c) > 0) {
		rc = regulator_set_voltage(bdata->vcc_i2c, RMI4_I2C_VTG_MIN_UV, RMI4_I2C_VTG_MAX_UV);
		if (rc) {
			dev_err(&rmi4_data->pdev->dev, "Regulator set_vtg failed vcc_i2c rc=%d\n", rc);
...
reg_vcc_i2c_put:
	regulator_put(bdata->vcc_i2c);
reg_vdd_set_vtg:
	if ((bdata->vdd != NULL) && (regulator_count_voltages(bdata->vdd) > 0))
		regulator_set_voltage(bdata->vdd, 0, RMI4_VTG_MAX_UV);
reg_vdd_put:
	if ((bdata->vdd != NULL) && (regulator_count_voltages(bdata->vdd) > 0))
		regulator_put(bdata->vdd);
	return rc;	
}
...
static int synaptics_rmi4_power_on(struct synaptics_rmi4_data *rmi4_data, bool on)
{
	...
	if (regulator_count_voltages(bdata->vcc_i2c) > 0) {
		rc = regulator_enable(bdata->vcc_i2c);
		msleep(100);
		if (rc) {
			dev_err(&rmi4_data->pdev->dev, "Regulator vcc_i2c enable failed rc=%d\n", rc);
			regulator_disable(bdata->vdd);
		}
	}
	return rc;
power_off:
	if ((bdata->vdd != NULL) && (regulator_count_voltages(bdata->vdd) > 0)) {
		rc = regulator_disable(bdata->vdd);
		if (rc) {
			dev_err(&rmi4_data->pdev->dev, "Regulator vdd disable failed rc=%d\n", rc);
			return rc;
		}
	}

	if (regulator_count_voltages(bdata->vcc_i2c) > 0) {
		rc = regulator_disable(bdata->vcc_i2c);
		if (rc) {
			dev_err(&rmi4_data->pdev->dev, "Regulator vcc_i2c disable failed rc=%d\n", rc);
			rc = regulator_enable(bdata->vdd);
			if (rc) {
				dev_err(&rmi4_data->pdev->dev,"Regulator vdd enable failed rc=%d\n", rc);
			}
...
pr_err("%s:%d   --cbw--\n",__func__,__LINE__);
		retval = request_threaded_irq(rmi4_data->irq, NULL,
				synaptics_rmi4_irq, bdata->irq_flags | IRQF_ONESHOT | IRQF_TRIGGER_FALLING,
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	TP_query_device = retval;
	printk("synaptics : TP_query_device = %d\n",TP_query_device);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
////////////////////////////////
#if CBW_POWER
	retval = synaptics_rmi4_power_init(rmi4_data);
	if(retval < 0){
		dev_err(&pdev->dev,"%s: Failed to power init \n",__func__);
		//goto err_get_reg;
	}
	retval = synaptics_rmi4_power_on(rmi4_data, true);
	if (retval < 0) {
		dev_err(&pdev->dev, "%s: power on failed\n",__func__);
		//goto pwr_deinit;
	}
#endif
////////////////////////////////
...
err_set_input_dev:
	synaptics_rmi4_set_gpio(rmi4_data, false);
err_ui_hw_init:
err_set_gpio:
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
      ...
      if (retval < 0) {
              dev_err(dev, "failed to power off suspend state\n");
             // goto err_gpio_configure;
		}
#endif	  
	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
