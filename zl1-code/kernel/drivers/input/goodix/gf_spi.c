...
extern int TP_chip_check;
extern int TP_query_device;
...
#endif
	printk("goodix : TP_chip_check = %d, TP_query_device = %d\n", TP_chip_check, TP_query_device);
	if((TP_chip_check < 0) && (TP_query_device < 0)){
		printk("goodix : there is no device found");
		goto error;
	}
