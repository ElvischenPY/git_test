...
#define IT7280_FW_AUTO_UPGRADE   1  // h up date
//#define IT7236_FW_AUTO_UPGRADE   //bin up date
...
#if defined(IT7236_FW_AUTO_UPGRADE)
static void ft_init_update_work(struct work_struct *work)
{
	...
	ts = container_of(delay_work, struct IT7236_ts_data, it7236_update_work);
	dev = &ts->input_dev->dev;

	pr_err("====cbw====    %s : [%d] \n",__func__,__LINE__);
	mutex_lock(&ts->input_dev->mutex);
	IT7280_upgrade_store(dev,NULL,NULL,0);
	pr_err("====cbw====    %s : [%d] \n",__func__,__LINE__);
...
u8 ft_init_update_proc(struct IT7236_ts_data *ts)
{
	dev_dbg(&ts->client->dev, "Ready to run update work.");
	pr_err("====cbw====    %s : [%d] \n",__func__,__LINE__);

	INIT_DELAYED_WORK(&ts->it7236_update_work, ft_init_update_work);
	schedule_delayed_work(&ts->it7236_update_work,
...
pr_err("====cbw==== read firmware id =%#x   \n" ,get_firmware_ver());
	...
	if(ret)
		goto err_get_ver;
#if defined(IT7236_FW_AUTO_UPGRADE)
		err = ft_init_update_proc(ts);
		if (err < 0) {
			dev_err(&client->dev, 
					"IT7236 Create firmware update thread error.\n");
	//			goto exit_power_off;
		}
#endif
	#if IT7280_FW_AUTO_UPGRADE
