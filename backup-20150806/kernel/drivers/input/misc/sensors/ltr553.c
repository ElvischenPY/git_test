...
#define LTR553_LIGHT_INPUT_NAME		"light_sensor"
#define LTR553_PROXIMITY_INPUT_NAME	"proximity_sensor"
...
static int ltr553_parse_dt(struct device *dev, struct ltr553_data *ltr)
{
	...
	dev_err(&ltr->i2c->dev,"====cbw====  ltr553...	%s:[%d]    rc=%d \n",__func__,__LINE__,rc);
	ltr->irq_gpio = rc;
...
static int ltr553_init_device(struct ltr553_data *ltr)
{
	int rc;
	unsigned int tmp;
	dev_err(&ltr->i2c->dev,"====cbw====  ltr553...	%s:[%d] \n",__func__,__LINE__);
...
static int ltr553_process_data(struct ltr553_data *ltr, int als_ps)
{
	...
	dev_err(&ltr->i2c->dev,"====cbw====  ltr553...	%s:[%d] \n",__func__,__LINE__);
		...
		dev_err(&ltr->i2c->dev, "====cbw==== ltr553 %s:[%d]  ps data: 0x%x 0x%x\n",__func__,__LINE__,ps_data[0], ps_data[1]);
...
static void ltr553_report_work(struct work_struct *work)
{
	...
	dev_err(&ltr->i2c->dev,"====cbw====   ltr553...  %s:[%d]   interrupt issued status=%#x  \n",__func__,__LINE__,status);
...
static int ltr553_enable_ps(struct ltr553_data *ltr, int enable)
{
	...
	dev_err(&ltr->i2c->dev,"====cbw====   ltr553...  %d   enable= %d  \n",__LINE__,enable);
		...
		const struct i2c_device_id *id)
{
	...
	if (res) {
	dev_err(&client->dev, "====cbw====  ltr553...  %d\n",__LINE__);
	...
	dev_err(&client->dev, "====cbw====  ltr553...  %d\n",__LINE__);
		...
		dev_err(&client->dev, "====cbw====	ltr553... %d  irq gpio =%d    ltr->irq_flags=%d \n",__LINE__,ltr->irq_gpio,ltr->irq_flags);

		res = devm_request_irq(&client->dev, ltr->irq,
				ltr553_irq_handler,
				/*ltr->irq_flags*/ IRQF_TRIGGER_FALLING| IRQF_ONESHOT,
				"ltr553", ltr);
	...
	dev_err(&client->dev, "ltr553 successfully probed!\n");
