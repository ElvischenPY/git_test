...
#define UV_SENSOR_NAME      "uv_sensor"
...
static struct platform_device *ga1a_pdev = NULL ;
...
static int ga1a_i2c_probe( struct i2c_client *client, const struct i2c_device_id *id )
{
  ...
  pr_debug( "ga1a_i2c_probe \n" ) ;

  data = platform_get_drvdata( ga1a_pdev ) ;
  data->client = client ;
  pdata = client->dev.platform_data ;

  pr_debug( "ga1a i2c attach success!!!\n" ) ;
  ...
  size_t count )
{
  ...
  pr_debug( "uv_enable_store data=%s\n", buf ) ;
...
static int ga1a_power_init(struct ga1a_data *data, bool on)
{
	int rc;
	...
	if(regulator_count_voltages(data->vdd) > 0) {
		rc = regulator_set_voltage(data->vdd, GA1A_MIN_UV, GA1A_MAX_UV);
		if (rc) {
			dev_err(&data->dev,
...
static int ga1a_power_on(struct ga1a_data *data, bool on)
{
	int rc = 0;
...
static int ga1a_probe( struct platform_device *pdev )
{
  ...
  if( !ga1a )
  {
	...
	ga1a_power_init(ga1a, 1);
	ga1a_power_on(ga1a, 1);
...
static int __init ga1a_module_init( void )
{
  ...
  ga1a_pdev = platform_device_register_simple( DEVICE_NAME, 0, NULL, 0 ) ;
  if( IS_ERR( ga1a_pdev ) )
  {
    printk( KERN_ERR "platform_device_register_simple error!! \n" ) ;
    return -1 ;
  }
...
{
  pr_debug( "ga1a_module_exit \n" ) ;

  platform_driver_unregister( &ga1a_driver ) ;
  platform_device_unregister( ga1a_pdev ) ;
}
