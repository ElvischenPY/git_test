...
#define IT7236_FW_AUTO_UPGRADE_BIN   1 //bin up date
...
static int i2cWriteToIt7280(struct i2c_client *client, unsigned char bufferIndex, unsigned char const dataBuffer[], unsigned short dataLength)
{
	...
	if(dataLength < 256) {
		...
		if(ret != 1)
			pr_err("==cbw==%s : i2c_transfer error ret =%d \n", __func__,ret);
		return ret;
	}
	else {
...
static int it7236_upgrade(u8* InputBuffer, int fileSize)
{
	...
	err_temp = i2cWriteToIt7280(gl_ts->client, 0xF4, pucBuffer, 1);
	if(err_temp != 1){
		pr_err("===cbw=== %s : [%d]  xxx  error err_temp=%d \n",__func__,__LINE__,err_temp);
		return err_temp;
	}
	pr_err("===cbw=== %s : [%d]    err_temp=%d \n",__func__,__LINE__,err_temp);
	...
	{
		pucBuffer[0] = 0x05;
		err_temp = i2cWriteToIt7280(gl_ts->client, 0xF7, pucBuffer, 1);	// Write EF PL Mode Cmd
		if(err_temp != 1){
			pr_err("===cbw=== %s : [%d]  xxx  error err_temp=%d \n",__func__,__LINE__,err_temp);
			return err_temp;
		}
...
static void Read_Point(struct IT7236_ts_data *ts)
{
	...
	i2cReadFromIt7280(gl_ts->client, 0xFF, pucBuffer, 1);
	pr_err("====cbw==== 0XFF %s : [%d] RIGHT-0x01 LEFT-0x02 pucBuffer =%d \n",__func__,__LINE__,pucBuffer[0]);

	i2cReadFromIt7280(gl_ts->client, 0xFE, pucBuffer, 1);
	pr_err("====cbw====  0XFE  %s : [%d] RIGHT-0x01 LEFT-0x02 pucBuffer =%d \n",__func__,__LINE__,pucBuffer[0]);
			...
			struct IT7236_ts_data *pdata)
{
	...
	printk("====cbw====    %s : [%d] %d \n",__func__,__LINE__,pdata->wakeup_pin);
	...
	printk("====cbw====    %s : [%d] %d \n",__func__,__LINE__,pdata->power_enable);
...
static int it7280_power_init(struct IT7236_ts_data *data, bool on)
{
	...
	printk("====cbw====    %s : [%d] vdd=%d   \n",__func__,__LINE__,regulator_count_voltages(data->vdd));
		if (IS_ERR(data->vdd)) {
			rc = PTR_ERR(data->vdd);
...
static int IT7280_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(gpio_is_valid(ts->power_enable)){
		printk("====cbw====    %s : [%d]  gpio=%d  \n",__func__,__LINE__,ts->power_enable);
...
////////////////////////////////////////////////////////////////wakeup cbw
	...
	if (ts->irq_gpio) {
		...
		if (ret == 0){
			ts->use_irq = 1;
			printk("====cbw====    %s : [%d]  irq ok  \n",__func__,__LINE__);
		}
		else {
			printk("====cbw====    %s : [%d]  irq err  \n",__func__,__LINE__);
