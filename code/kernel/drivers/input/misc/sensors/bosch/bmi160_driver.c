...
static int sensor_regulator_configure(struct bmi_client_data *data, bool on)
{
	...
	if (!on) {
		...
		if (regulator_count_voltages(data->vio) > 0) {
			...
			if (rc) {
				pr_err("Regulator set failed vio rc=%d\n", rc);
				goto reg_vio_put;
			}
		}
	}
...
static int sensor_regulator_power_on(struct bmi_client_data *data, bool on)
{
	...
	if (!on) {
		...
		if (rc) {
			pr_err("Regulator vio disable failed rc=%d\n", rc);
			rc = regulator_enable(data->vdd);
			pr_err("Regulator vio re-enabled rc=%d\n", rc);
...
int bmi_probe(struct bmi_client_data *client_data, struct device *dev)
{
	...
	dev_info(dev, "bmi160 power on");
	sensor_regulator_configure(client_data, 1);
	sensor_regulator_power_on(client_data, 1);
	...
	dev_info(dev, "sensor_time:%d, %d",
		sensortime_duration_tbl[0].ts_delat,
		sensortime_duration_tbl[0].ts_duration_lsb);
	dev_info(dev, "sensor %s probed successfully", SENSOR_NAME);
