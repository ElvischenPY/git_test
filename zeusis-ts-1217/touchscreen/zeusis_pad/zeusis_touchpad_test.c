...
int test_dbg_cmd_test(struct ts_cmd_node *in_cmd,struct ts_cmd_node *out_cmd)
{
	...
	switch(test_item){
		...
		case TEST_INPUT_ITEM_1:
			...
			break;
		default:
			TP_LOG_ERR("unsupport item :%d \n", test_item);
			break;
	}
	TP_LOG_INFO("test item :%d result :%d\n", test_item, error);
	return error;
}
	...
	for(i=0; i<100; i++)
	{
		...
		if (NO_ERR == pad_put_one_cmd(&cmd)) {
			if(wait_for_completion_timeout(&cq_test_sync_1, 5*HZ)){
				...
				TP_LOG_INFO("CMD PROCESS ELAPSED TIME: %lldms%lldus\n", delta_ms, delta_us);
			}
			else
				TP_LOG_ERR("ts thread process TEST CMD expire 5 sec\n");
		}else
			TP_LOG_ERR("put cmd error\n");
	}
	return 0;
}
...
int cq_item_2_func(void* data)
{
	...
	for (i=0; i<100; i++)
	{
		...
		if (NO_ERR == pad_put_one_cmd(&cmd)) {
			if(wait_for_completion_timeout(&cq_test_sync_1, 5*HZ)){
				TP_LOG_INFO("CMD PROCESS %d FINISH - cpu id:%d\n", i, smp_processor_id());
			}
			else
				TP_LOG_ERR("ts thread process TEST CMD expire 5 sec\n");
		}else
...
int cq_item_3_func(void* data)
{
	...
	for(i=0; i<TP_CMD_QUEUE_SIZE+5; i++){
		cmd.cmd_param.pub_params.algo_param.algo_order = i;
		cq_test_buff[i] = i;
		if(pad_put_one_cmd(&cmd)){
			TP_LOG_INFO("SEND COMPLETE i:%d\n", i);
			if(!i)
			...
			{
				TP_LOG_INFO("CMD PROCESS FINISH\n");
			}
			else
				TP_LOG_ERR("ts thread process TEST CMD expire 10 sec\n");
...
{
	struct dentry *root = g_tp_data.dbg_root;
	struct dentry *dir;
	struct dentry *dent;
	int i;
	int error = -EINVAL;
	...
	if(!root){
		TP_LOG_ERR("g_tp_data dont have dbg_root\n");
		goto out;
	}

	dir = debugfs_create_dir("test_dir", root);
	if (!dir) {
		TP_LOG_ERR("failed to create test_dir directory\n");
		goto out;
	}
	...
	for(i = 0; i<ARRAY_SIZE(g_test_items); i++){
		...
		if (!dent) {
			TP_LOG_ERR("failed to create %s file\n", g_test_items[i].item_name);
...
static int ts_test_thread(void *p)
{
	...
	while(atomic_read(&g_tp_data.state) != TS_WORK && i < WAIT_MAX_COUNT){
		TP_LOG_INFO("touchpad module init not finish\n");
		msleep(1000);
		i++;
	}

	TP_LOG_INFO("touchpad test init\n");

	if(test_create_debugfs()){
		TP_LOG_ERR("touchpad test create debugfs failed\n");
		goto out;
	}

	TP_LOG_INFO("touchpad test start polling\n");
...
static int __init ts_test_init(void)
{
	...
	TP_LOG_INFO("ts_test_init called here\n");
	return 0;
...
MODULE_AUTHOR("robin.wangyuxiang>");
