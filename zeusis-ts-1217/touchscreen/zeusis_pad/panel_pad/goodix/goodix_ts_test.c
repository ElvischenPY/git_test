...
struct goodix_ts_test {
	...
	char test_result[TP_RAWDATA_RESULT_MAX];
	char failed_reason[TP_RAWDATA_RESULT_MAX];
};
...
/**
 * gt1152_passe_config - resolve tx/rx number and key number
 */
static int gt1152_passe_config(struct goodix_ts_config *test_config,
				struct ts_test_params *test_params)
{
	//int i;
	...
	if(Zeusis_TouchKey_Panel){
		test_params->sc_drv_num = test_params->drv_num - 1;
		test_params->sc_sen_num = test_params->sen_num;
		test_params->key_num = test_params->sen_num;
	}
...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	char project_id[20];
	int ret, i, size, reorder, row, column;
	u32 value[8];
...
static int goodix_tptest_prepare(struct goodix_ts_test *ts_test)
{
	...
	} else if(!strcmp(ts_test->product_id, "GT1152")){
		gt1152_passe_config(&ts_test->test_config,
						&ts_test->test_params);
		ts_test->rawdata.addr = 0xB334;
		ts_test->basedata.addr = 0xACEC;
	} else {
		return -ENODEV;
	}
...
/**
 * pad_goodix_tptest_finish - finish test
 */
static inline int pad_goodix_tptest_finish(struct goodix_ts_test *ts_test)
{
	int ret = -ENODEV;
				...
				struct ts_test_params *test_params)
{
	...
	if(Zeusis_TouchKey_Panel){
		int sc_nodes = rawdata->size - test_params->key_num;
		for (i = 0; i < sc_nodes; i++) {
...
end_test:
	...
	if (ts_test->rawdata.size <= TP_RAWDATA_BUFF_MAX) {
...
int pad_goodix_get_rawdata(struct ts_rawdata_info *info,
...
exit_finish:
	pad_goodix_tptest_finish(gts_test);
	kfree(gts_test);
	gts_test = NULL;
	return 0;
}
		...
		for (j = i + 1; j < max_drvs; j++) {
			...
			if (adc_signal > ts_test->test_params.short_threshold) {
				ret = goodix_check_short_resistance(
						...
						r_threshold);
				if (ret < 0) {
					GTP_ERROR("Tx%d-Tx%d short circut", short_pin_num, j);
					err |= ret;
				}
			}
		}
		...
		for (j = 0; j < max_sens; j++) {
			...
			if (adc_signal > ts_test->test_params.short_threshold) {
				ret = goodix_check_short_resistance(
						...
						r_threshold);
				if (ret < 0) {
					GTP_ERROR("Rx%d-Rx%d short circut",
		...
		for (j = 0; j < max_drvs; j++) {
			...
			if (adc_signal > ts_test->test_params.short_threshold) {
				ret = goodix_check_short_resistance(
						...
						r_threshold);
				if (ret < 0) {
					GTP_ERROR("Rx%d-Tx%d short circut",
						short_pin_num - max_drvs, j);
					err |= ret;
				}
			}
