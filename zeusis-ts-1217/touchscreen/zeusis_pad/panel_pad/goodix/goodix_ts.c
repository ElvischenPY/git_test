...
struct goodix_ts_data *pad_goodix_ts;

/**
 * pad_goodix_i2c_write - i2c write.
 * @addr: register address.
 * @buffer: data buffer.
 * @len: the bytes of data to write.
 * Return: 0: success, otherwise: failed
 */
int pad_goodix_i2c_write(u16 addr, u8 * buffer, s32 len)
{
	struct ts_device_data *dev_data = pad_goodix_ts->dev_data;
...
/**
 * pad_goodix_i2c_read - i2c read.
 * @addr: register address.
 * @buffer: data buffer.
 * @len: the bytes of data to write.
 * Return: 0: success, otherwise: failed
 */
int pad_goodix_i2c_read(u16 addr, u8 * buffer, s32 len)
{
	struct ts_device_data *dev_data = pad_goodix_ts->dev_data;
	int ret;
...
/**
 * pad_goodix_i2c_read_dbl_check - read twice and double check
 * @addr: register address
 * @buffer: data buffer
 * @len: bytes to read
 * Return    <0: i2c error, 0: ok, 1:fail
 */
int pad_goodix_i2c_read_dbl_check(u16 addr, u8 * buffer, s32 len)
{
	u8 buf[16] = {0};
	u8 confirm_buf[16] = {0};
	int ret;
	...
	ret = pad_goodix_i2c_read(addr, buf, len);
	if (ret < 0)
		return ret;

	msleep(5);
	memset(confirm_buf, 0, sizeof(confirm_buf));
	ret = pad_goodix_i2c_read(addr, confirm_buf, len);
	if (ret < 0)
		return ret;
...
static int goodix_send_cfg(struct goodix_ts_config *cfg_ptr)
{
	...
	retry = 0;
	while (retry++ < 3) {
		ret = pad_goodix_i2c_write(GTP_REG_CONFIG_DATA, config, cfg_len);
		if (!ret) {
...
static int goodix_send_cmd(u8 cmd, u8 data)
{
	...
	ret = pad_goodix_i2c_write(GTP_REG_CMD + 1, &buffer[1], 2);
	ret |= pad_goodix_i2c_write(GTP_REG_CMD, &buffer[0], 1);
	msleep(50);
...
static int goodix_init_watchdog(void)
{
	...
	return pad_goodix_i2c_write(GTP_REG_CMD, &value[0], 4);
}
...
{
	struct ts_feature_info *info = &g_tp_data.feature_info;
...
static int goodix_cache_roidata(struct goodix_ts_roi *roi)
{
	...
	ret = pad_goodix_i2c_read(ROI_STA_REG, status, ROI_HEAD_LEN);
	if (unlikely(ret < 0))
		return ret;
	...
	ret = pad_goodix_i2c_read(ROI_DATA_REG,
	...
	status[0] = 0x00;
	ret = pad_goodix_i2c_write(ROI_STA_REG, status, 1);
...
static int goodix_request_event_handler(struct goodix_ts_data *ts)
{
	...
	ret = pad_goodix_i2c_read(GTP_REG_RQST, &rqst_data, 1);
	if (ret)
		return ret;
	...
	case GTP_RQST_CONFIG:
		...
		if (ret) {
			...
			pad_goodix_i2c_write(GTP_REG_RQST, &rqst_data, 1);
		}
		break;
	case GTP_RQST_RESET:
		GTP_INFO("Request Reset.");
		pad_goodix_i2c_read(0x5097, &rqst_data, 1);
				...
				struct ts_fingers *info)
{
	...
	ret = pad_goodix_i2c_read(GTP_READ_COOR_ADDR, &touch_data[0], 1 + 8 + 2);
	if (unlikely(ret))
		goto exit;
	...
	/* read the remaining coor data
		...
		*/
	if (touch_num > 1) {
		ret = pad_goodix_i2c_read((GTP_READ_COOR_ADDR + 11),
	...
	if (unlikely(check_sum)) { /* checksum error*/
		ret = pad_goodix_i2c_read(GTP_READ_COOR_ADDR, touch_data,
					3 + 8 * touch_num);
		if (ret)
			goto exit;
				...
				struct ts_fingers *info)
{
	...
	/** package: -head 4B + track points + extra info-
		...
		*/
	ret = pad_goodix_i2c_read(GTP_REG_WAKEUP_GESTURE, doze_buf, 4);
	if (ret < 0)
		return 0;
...
*/
	...
	if (ges_type == 0) {
		...
		doze_buf[0] = 0x00;
		//pad_goodix_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
		//gesture_enter_doze();
...
{
	struct goodix_ts_data *ts = pad_goodix_ts;
	struct ts_fingers *ts_fingers;
	u8 sync_val = 0;
	int ret = 0;

	if (unlikely(!pad_goodix_ts))
		return -ENODEV;
...
/*sync_evt:*/
	if (!ts->rawdiff_mode)
		ret = pad_goodix_i2c_write(GTP_READ_COOR_ADDR, &sync_val, 1);
	else
...
static int goodix_i2c_test(void)
{
	...
	ret = pad_goodix_i2c_read(GTP_REG_HW_INFO, (u8 *)&hw_info,
					sizeof(hw_info));
 ...
 */
int pad_goodix_power_switch(struct goodix_ts_data *ts, int on)
{
...
/**
 * pad_goodix_read_version - Read gt1x version info.
 * @hw_info: address to store version info
 * Return 0-succeed.
 */
int pad_goodix_read_version(struct goodix_hw_info * hw_info)
{
	u8 buf[12] = { 0 };
	...
	while (retry--) {
		ret = pad_goodix_i2c_read(GTP_REG_VERSION, buf, sizeof(buf));
		if (!ret) {
	...
	pad_goodix_ts->sensor_id_valid = true;
	return 0;
}
 ...
 * Return: 0-succeed, -1-faileds
 */
int pad_goodix_parse_cfg_data(struct goodix_ts_data *ts,
...
static int goodix_init_configs(struct goodix_ts_data *ts)
{
	...
	ret = pad_goodix_parse_cfg_data(ts, "normal_config", cfg_data,
				&cfg_len, sensor_id);
	if (ret < 0) {
	...
	ret = pad_goodix_parse_cfg_data(ts, "normal_noise_config",
	...
	ret = pad_goodix_parse_cfg_data(ts, "glove_config", cfg_data,
				&cfg_len, sensor_id);
	if (ret < 0) {
	...
	ret = pad_goodix_parse_cfg_data(ts, "glove__noise_config",
			cfg_data, &cfg_len, sensor_id);
	if (ret < 0) {
	...
	ret = pad_goodix_parse_cfg_data(ts, "holster_config", cfg_data,
				&cfg_len, sensor_id);
	if (ret < 0) {
	...
	ret = pad_goodix_parse_cfg_data(ts, "charger_config",
			cfg_data, &cfg_len, sensor_id);
	if (ret < 0) {
	...
	ret = pad_goodix_parse_cfg_data(ts, "pocket_config",
			cfg_data, &cfg_len, sensor_id);
	if (ret < 0) {
				...
				struct ts_device_data *chip_data)
{
	...
	if (!ret) {
		GTP_INFO("get chip specific charger_supported = %d",
			value);
		g_tp_data.feature_info.charger_info.charger_supported = (u8)value;
	}
#endif
	...
	if (!ret) {
		...
		g_tp_data.feature_info.roi_info.roi_supported = (u8)value;
	} else {
	 ...
	 struct ts_device_data *dev_data, struct platform_device *ts_dev)
{
	...
	pad_goodix_ts = kzalloc(sizeof(struct goodix_ts_data), GFP_KERNEL);
	if (!pad_goodix_ts)
		return -ENOMEM;

	client = dev_data->client;
	pad_goodix_ts->pdev = ts_dev;
	pad_goodix_ts->pdev->dev.of_node = device;
	pad_goodix_ts->dev_data = dev_data;
	pad_goodix_ts->ops.i2c_read = pad_goodix_i2c_read;
	pad_goodix_ts->ops.i2c_write = pad_goodix_i2c_write;
	pad_goodix_ts->ops.chip_reset = pad_goodix_chip_reset;
	pad_goodix_ts->ops.send_cmd = goodix_send_cmd;
	pad_goodix_ts->ops.send_cfg = goodix_send_cfg;
	pad_goodix_ts->ops.i2c_read_dbl_check = pad_goodix_i2c_read_dbl_check;
	pad_goodix_ts->ops.read_version = pad_goodix_read_version;
	pad_goodix_ts->ops.parse_cfg_data = pad_goodix_parse_cfg_data;
	pad_goodix_ts->ops.feature_resume = goodix_feature_resume;
	pad_goodix_ts->tools_support = true;

	pad_goodix_ts->slide_max_x = pad_goodix_ts->dev_data->slide_max_x;
	pad_goodix_ts->slide_max_y = pad_goodix_ts->dev_data->slide_max_y;

	/* do NOT remove these logs */
	GTP_INFO("Driver Version: %s", GTP_DRIVER_VERSION);

	ret = goodix_get_regulators(pad_goodix_ts);
	if (ret < 0)
		//goto err_get_regs;
	...
	ret = goodix_pinctrl_init(pad_goodix_ts);
	if (ret < 0)
		goto err_pinctrl_init;

	ret = goodix_pinctrl_select_normal(pad_goodix_ts);
	if (ret < 0)
		goto err_pinctrl_init;

	/* power on */
	ret =  pad_goodix_power_switch(pad_goodix_ts, SWITCH_ON);
	if (ret < 0)
		goto err_power_on;
	/* detect chip */
	ret = pad_goodix_chip_reset();
	if (ret < 0)
		goto err_power_on;
...
err_power_on:
	goodix_pinctrl_release(pad_goodix_ts);
	 pad_goodix_power_switch(pad_goodix_ts, SWITCH_OFF);
err_pinctrl_init:
	goodix_release_gpio(dev_data);
err_req_gpio:
	goodix_put_regulators(pad_goodix_ts);
//err_get_regs:
	kfree(pad_goodix_ts);
	pad_goodix_ts = NULL;
	return ret;
...
#ifdef ROI
	ret = goodix_ts_roi_init(&pad_goodix_ts->roi);
	if (ret < 0)
		return ret;
#endif
...
static int goodix_slide_config(struct input_dev *slide_dev)
{
...
#ifdef INPUT_TYPE_B_PROTOCOL
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 7, 0))
	input_mt_init_slots(slide_dev, TP_MAX_SLIDE_FINGER, INPUT_MT_DIRECT);
#else
	input_mt_init_slots(slide_dev, TP_MAX_SLIDE_FINGER);
#endif
#endif
	...
	input_set_abs_params(slide_dev, ABS_MT_TRACKING_ID, 0, TP_MAX_SLIDE_FINGER, 0, 0);

	return 0;
}

static int pad_goodix_chip_resume(void)
{
	struct goodix_ts_data *ts = pad_goodix_ts;
	...
	case TS_POWER_OFF_MODE:
		pad_goodix_power_switch(ts, SWITCH_ON);
...
static int goodix_chip_after_resume(void *feature_info)
{
	if (pad_goodix_ts) {
		...
		goodix_feature_resume(pad_goodix_ts);
	}
...
static int goodix_chip_suspend(void)
{
	...
	case TS_POWER_OFF_MODE:
		goodix_pinctrl_select_suspend(ts);
		 pad_goodix_power_switch(ts, SWITCH_OFF);
		break;
	case TS_GESTURE_MODE:
...
{
	if (pad_goodix_ts == NULL || !file_name)
		return -ENODEV;

	snprintf(pad_goodix_ts->firmware_name,
			sizeof(pad_goodix_ts->firmware_name),
            "ts/%sgt1152.img", file_name);
    GTP_INFO("Requesting firmware image %s\n",pad_goodix_ts->firmware_name);
	return pad_gt1x_auto_update_proc(UPDATE_TYPE_HEADER);
}

static int goodix_fw_update_sd(void)
{
	if (pad_goodix_ts == NULL)
		return -ENODEV;

	return pad_gt1x_auto_update_proc(UPDATE_TYPE_FILE);
}
...
static int goodix_chip_get_info(struct ts_chip_info_param *info)
{
	...
	if (!info || !ts)
		return -EINVAL;
	ret = pad_goodix_i2c_read(0x8050, &config_version, 1);
	if (ret < 0 )
		GTP_ERROR("read config version error!");
	GTP_INFO("config version is %d",config_version);
    sprintf(info->mod_vendor, "GT%s_%06X(FW)_%04X(Boot)_%d(config_version)", ts->hw_info.product_id, ts->hw_info.patch_id, ts->hw_info.mask_id >> 8,config_version);
...
int pad_goodix_reset_select_addr(void)
{
	int reset_gpio;
	int irq_gpio;
	...
	reset_gpio = pad_goodix_ts->dev_data->reset_gpio;
	irq_gpio = pad_goodix_ts->dev_data->irq_gpio;
...
#if GTP_INCELL_PANEL
int pad_goodix_write_and_readback(u16 addr, u8 * buffer, s32 len)
{
    int ret;
    u8 d[len];

    ret = pad_goodix_i2c_write(addr, buffer, len);
    if (ret < 0)
        return -1;

    ret = pad_goodix_i2c_read(addr, d, len);
...
int pad_goodix_incell_reset(void)
{
#define RST_RETRY       5
    ...
    do {
    	/* select i2c address */
	pad_goodix_reset_select_addr();

    	/* test i2c */
        ret = pad_goodix_i2c_read(0x4220, d, 1);
    ...
    do {
        d[0] = 0x0C;
        ret = pad_goodix_write_and_readback(0x4180, d, 1);
    ...
    do {
        d[0] = 0x00;
        ret = pad_goodix_write_and_readback(0x4305, d, 1);
        if (ret < 0)
            continue;

        d[0] = 0x2B;
        d[1] = 0x24;
        ret = pad_goodix_write_and_readback(0x42c4, d, 2);
        if (ret < 0)
            continue;

        d[0] = 0xE1;
        d[1] = 0xD3;
        ret = pad_goodix_write_and_readback(0x42e4, d, 2);
        if (ret < 0)
            continue;
...
int pad_goodix_chip_reset(void)
{
...
static int goodix_glove_switch(struct ts_glove_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->glove_switch) {
			...
			ret = goodix_feature_switch(pad_goodix_ts,
...
static int goodix_charger_switch(struct ts_charger_info *info)
{
	...
	case TS_ACTION_READ:
		if (info->charger_switch) {
			ret = goodix_send_cfg(&pad_goodix_ts->charger_config);
			GTP_INFO("Charger cfg switch on");
		} else {
			ret = goodix_send_cfg(&pad_goodix_ts->normal_config);
...
static int goodix_check_hw_status(void)
{
	...
	for (i = 0; i < 3; i++) {
		ret = pad_goodix_i2c_read(GTP_REG_CMD, esd_buf, 4);
		if (unlikely(ret < 0))
			break;
	...
	if (likely(!ret && i < 3)) {
		...
		ret = pad_goodix_i2c_write(GTP_REG_CMD, &feed_wd[0],
							sizeof(feed_wd));
	} else {
...
static int goodix_regs_operate(struct ts_regs_info *info)
{
	...
	case TS_ACTION_WRITE:
		...
		retval = pad_goodix_i2c_write(regs_addr, value, info->num);
		...
		break;
	case TS_ACTION_READ:
		retval = pad_goodix_i2c_read(regs_addr, value, info->num);
		if (retval < 0) {
...
static int goodix_roi_switch(struct ts_roi_info *info)
{
	...
	case TS_ACTION_WRITE:
		if (info->roi_switch == 1) {
			pad_goodix_ts->roi.enabled = true;
		} else if (info->roi_switch == 0) {
			pad_goodix_ts->roi.enabled = false;
		} else {
...
static u8* goodix_roi_rawdata(void)
{
	...
	mutex_lock(&pad_goodix_ts->roi.mutex);
	if (pad_goodix_ts->roi.enabled && pad_goodix_ts->roi.data_ready)
		rawdata_ptr = (u8 *)pad_goodix_ts->roi.rawdata;
	mutex_unlock(&pad_goodix_ts->roi.mutex);
		...
		struct ts_test_type_info *info)
{
	...
	case TS_ACTION_READ:
		memcpy(info->tp_test_type,
			pad_goodix_ts->dev_data->tp_test_type,
			TS_CAP_TEST_TYPE_LEN);
...
struct ts_device_ops pad_ts_goodix_ops = {
...
//	.chip_irq_top_half = goodix_irq_top_half,
	...
	.chip_reset = pad_goodix_chip_reset,
...
//    .chip_before_suspend = goodix_before_suspend,
	...
	.chip_resume = pad_goodix_chip_resume,
    .chip_after_resume = goodix_chip_after_resume,
    .chip_get_rawdata = pad_goodix_get_rawdata,
