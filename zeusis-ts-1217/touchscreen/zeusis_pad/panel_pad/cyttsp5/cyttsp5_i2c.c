...
ssize_t cyttsp5_dsm_record_basic_err_info(struct device *dev)
{
    ...
    if (!vci_power_info) {
        TP_LOG_ERR("%s: vci_power_info kzalloc fail\n", __func__);
        goto error_alloc_data;
    }

    vddio_power_info = kzalloc(DSMINFO_LEN_MAX, GFP_KERNEL);
    if (!vddio_power_info) {
        TP_LOG_ERR("%s: vddio_power_info kzalloc fail\n", __func__);
        goto error_alloc_data;
    }
...
static int cyttsp5_i2c_read_default_nosize(struct device *dev, u8 *buf, u32 max)
{
    ...
    if (!buf) {
        TP_LOG_ERR("%s: input parameter error.\n", __func__);
        return -EINVAL;
    }
    ...
    if (rc < 0 || rc != msg_count) {
        TP_LOG_ERR("%s: I2C transfer error, rc = %d, msg_count = %d\n",
    ...
    if (size > max) {
        TP_LOG_ERR("%s: ERROR, size = %d, max = %d\n", __func__,size, max);
        return -EINVAL;
    }
    ...
    if (rc < 0) {
        TP_LOG_ERR("%s: I2C read error, rc = %d\n", __func__,rc);
        return rc;
    } else if (rc != size) {
        TP_LOG_ERR("%s: I2C read error, rc = %d, size = %d\n", __func__, rc, size);
        return -EIO;
    }
...
#endif
        TP_LOG_ERR("%s: I2C transfer error, rc = %d\n", __func__,rc);
        return (rc < 0) ? rc : -EIO;
    }
    ...
    const struct i2c_device_id *i2c_id)
{
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        TP_LOG_ERR("%s: I2C functionality not Supported.\n", __func__);
        return -EIO;
    }
...
/* if support device tree, get pdata from device tree */
    ...
    if (match) {
        ...
        if (rc < 0) {
            TP_LOG_ERR("%s: device tree create and get pdata fail, rc = %d.\n",
                        __func__,rc);
            return rc;
        }
    } else {
        TP_LOG_ERR("%s: No device mathced.\n", __func__);
        return -ENODEV;
    }
#endif
    ...
    if (rc && match) {
        ...
        TP_LOG_ERR("%s: cyttsp5 probe fail.\n", __func__);
        return rc;
    }
#endif
    ...
    TP_LOG_INFO("%s: cyttsp5 probe success.\n", __func__);
...
static int __init cyttsp5_i2c_init(void)
{
    ...
    if (rc) {
        TP_LOG_ERR("%s: Cypress v5 I2C Driver add fail, rc = %d.\n",__func__,rc);
    } else {
        TP_LOG_INFO("%s: Cypress TTSP v5 I2C Driver add successful.\n",__func__,rc);
    }
    return rc;
}
