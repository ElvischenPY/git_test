        ...
        u8 param_id, u32 param_value)
{
    ...
    list_for_each_entry(param, &cd->param_list, node) {
        if (param->id == param_id) {
            ...
            TP_LOG_DEBUG("%s: Update parameter id:%d value:%d\n",
    ...
    TP_LOG_DEBUG("%s: Add parameter id:%d value:%d\n",
        __func__, param_id, param_value);
...
wait:
    ...
    if (with_timeout) {
        ...
        if (IS_TMO(t)) {
            TP_LOG_ERR("%s: tmo waiting exclusive access, t = %d\n",
                __func__,t);
            return -ETIME;
        }
    } else {
        ...
        struct cyttsp5_hid_output *hid_output)
{
    ...
    if (command_code != hid_output->command_code) {
        TP_LOG_ERR("%s: HID output response, wrong command_code:%X\n",
        ...
        struct cyttsp5_hid_output *hid_output)
{
    ...
    if (rc){
        TP_LOG_ERR("%s: cyttsp5_hid_send_output_user_  error\n",
            __func__);
        goto error;
    }
        ...
        struct cyttsp5_hid_output *hid_output)
{
    ...
    if (rc) {
        TP_LOG_ERR("%s: Fail cyttsp5_adap_transfer, rc = %d\n",
                    __func__,rc);
    }
...
resend:
    ...
    if (rc) {
        if(retry) {
            ...
            TP_LOG_ERR("%s:send_output_user_and_wait_error\n",__func__);
            return rc;
        }
    }
        ...
        struct cyttsp5_hid_desc *desc)
{
    ...
    TP_LOG_DEBUG("%s: HID len:%X HID ver:%X\n", __func__,
        ...
        u8 *report_desc, size_t len)
{
    ...
    TP_LOG_DEBUG("%s: Report descriptor length: %zu\n",
        __func__, len);
...
static int setup_report_descriptor(struct cyttsp5_core_data *cd)
{
    ...
    for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
        ...
        if (field) {
            ...
            TP_LOG_DEBUG("%s: ofs:%zu size:%zu min:%zu max:%zu bofs:%zu report:%d",
...
static int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
    ...
    for (t = 0; t < cd->num_hid_reports; t++) {
        ...
        for (j = 0; j < report->num_fields; j++) {
            ...
            TP_LOG_DEBUG("  Collections Phys:%08X App:%08X Log:%08X\n",
                field->collection_usage_pages
        ...
        enum cyttsp5_atten_type type, int mode)
{
    ...
    TP_LOG_DEBUG("%s: check list type=%d mode=%d\n",
        __func__, type, mode);
    spin_lock(&cd->spinlock);
    list_for_each_entry_safe(atten, atten_n,
        ...
        if (!mode || atten->mode & mode) {
            spin_unlock(&cd->spinlock);
            TP_LOG_DEBUG("%s: attention for '%s'", __func__,
                dev_name(atten->dev));
...
static void cyttsp5_start_wd_timer(struct cyttsp5_core_data *cd)
{
    if (!CY_WATCHDOG_TIMEOUT) {
        TP_LOG_INFO("%s: watchdog timeout:%d\n", __func__, CY_WATCHDOG_TIMEOUT);
        return;
    }
...
static void cyttsp5_watchdog_timer(unsigned long handle)
{
    ...
    TP_LOG_DEBUG("%s: Watchdog timer triggered\n", __func__);
...
static int cyttsp5_put_device_into_easy_wakeup_(struct cyttsp5_core_data *cd)
{
    ...
    if (rc || status == 1) {
        TP_LOG_ERR("%s: Get in easy wakeup mode fail, rc = %d, gesture = %d, status = %d", 
    ...
    TP_LOG_INFO("%s: Get in easy wakeup mode success, gesture = %d, status = %d", 
    ...
    if(cd->gesture_id == CYTTSP5_GESTURE_DOUBLE_CLICK && cd->double_tap_enabled) {
        for(i = 0; i < 2; i++) {
            if (cd->gest_pos[i].x < cd->dtz_x0 || cd->gest_pos[i].x > cd->dtz_x1 ||
                ...
                TP_LOG_INFO("%s: %d:Double click out of area:x0 = %d, y0 = %d ",
                                __func__, __LINE__, cd->dtz_x0, cd->dtz_y0);
                TP_LOG_INFO("x1 = %d, y1 = %d\n", cd->dtz_x1, cd->dtz_y1);
                TP_LOG_INFO("%s: %d:Click at:x0 = %d, y0 = %d\n",
...
static int parse_touch_input(struct cyttsp5_core_data *cd, int size)
{
    ...
    TP_LOG_DEBUG("%s: Received touch report\n", __func__);
    if (!si->ready) {
        TP_LOG_ERR(
...
{
    TP_LOG_DEBUG("%s: Received cmd interrupt\n", __func__);
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
    ...
    TP_LOG_DEBUG("%s: Read input successfully\n", __func__);
    return rc;
}
...
static int cyttsp5_restore_parameters_(struct cyttsp5_core_data *cd)
{
    ...
    list_for_each_entry(param, &cd->param_list, node) {
        spin_unlock(&cd->spinlock);
        TP_LOG_DEBUG("%s: Parameter id:%d value:%d\n",
...
reset:
    ...
    if (reset || retry != CY_CORE_STARTUP_RETRY_COUNT) {
        /* reset hardware */
        TP_LOG_INFO("%s: start to rest hw & wait.\n", __func__);
    ...
    TP_LOG_INFO("%s: start to get sysinfo.\n", __func__);
    rc = cyttsp5_hid_output_get_sysinfo_(cd);
    if (rc) {
...
static int cyttsp5_startup(struct cyttsp5_core_data *cd, bool reset)
{
    ...
    if (cd->glove_mode_enabled) {
        ...
        if (rc < 0) {
            TP_LOG_ERR("%s: set glove mode failed, rc = %d.\n", __func__, rc);
        } else {
...
static int cyttsp5_core_suspend(struct device *dev)
{
    ...
    if (cd->fw_upgrade_flag == FW_UPGRADE_RUNNING) {
        TP_LOG_INFO("%s: Fw upgrade is running, exit suspend.\n", __func__);
        mutex_unlock(&cd->system_lock);
        return 0;
    }
    mutex_unlock(&cd->system_lock);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    mutex_unlock(&cd->system_lock);
    TP_LOG_INFO("%s: easy_wakeup_gesture:%s\n", __func__, buf);
    return ret;
}
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (!(cd->cpdata->flags & CY_CORE_FLAG_WAKE_ON_GESTURE)) {
        TP_LOG_ERR("%s: check flag fail, flag = %d\n",
                    __func__,cd->cpdata->flags);
        return -EINVAL;
    }

    if (!cd->features.easywake) {
        TP_LOG_ERR("%s: not suppert easy wakeup, flag = %d\n",
    ...
    if (ret < 0) {
        TP_LOG_ERR("%s: Parse input value fail, value: %s\n",
                    __func__, buf);
        return ret;
    }

    TP_LOG_INFO("%s: Input value: %lu\n", __func__,value);
    if (value > 0xFFFF) {
        TP_LOG_ERR("%s: Error value: %lu\n", __func__,value);
        return -EINVAL;
    }
    ...
    if (cd->sysinfo.ready && IS_PIP_VER_GE(&cd->sysinfo, 1, 2)) {
        ...
        TP_LOG_ERR("%s: easy wakeup Error value: %d\n", __func__,
                    cd->sysinfo.ready);
        TP_LOG_ERR("%s: easy wakeup Error value: %d\n", __func__,
    ...
    if (ret) {
        TP_LOG_ERR("%s: end with error NO.: %d\n", __func__,ret);
        return ret;
    }
...
static ssize_t cyttsp5_loglevel_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE,"g_TP_LOG_level = %d.\n",g_TP_LOG_level);
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    TP_LOG_INFO("cyttsp5_chip_info_show called\n");
    if(TS_UNINIT == atomic_read(&g_cyttsp5_state) ){
        TP_LOG_INFO("ts initial faild\n");
        return len;
    }

    if (!cd->sysinfo.ready) {
        TP_LOG_INFO("cyttsp5_sysinfo is not ready\n");
...
ssize_t cyttsp5_chip_info_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    TP_LOG_INFO("cyttsp5_chip_info_store called\n");
    if(TS_UNINIT == atomic_read(&g_cyttsp5_state) ){
        TP_LOG_INFO("ts initial faild\n");
        ret = -EINVAL;
        goto out;
    ...
    TP_LOG_INFO("sscanf value is %u\n", value);
    get_info_flag = value;
out:
    TP_LOG_INFO("cyttsp5_chip_info_store done\n");
    return ret;
}
static int cyttsp5_set_touch_mode(struct device *dev,u32 value)
{
...
static unsigned int cyttsp5_get_touch_mode(struct device *dev)
{
    ...
    if (rc < 0) {
        TP_LOG_ERR("%s: cyttsp5_hid_output_get_param failed r=%d\n",__func__, rc);
        return rc;
    }
...
{
    struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
    TP_LOG_INFO("%s: glove_mode_enabled=%d\n", __func__, cd->glove_mode_enabled);
                ...
                struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    TP_LOG_DEBUG("%s: enable = %d,x0 = %d,y0 = %d,x1 = %d,y1 = %d\n",
...
static int tp_fb_notifier_callback(struct notifier_block *self,
        unsigned long event, void *data)
{
    ...
    if(atomic_read(&mmi_test_status)){
        TP_LOG_INFO("%s: MMI test is running, exit now.\n", __func__);
        return 0;
    }
    ...
    if (event == FB_EVENT_BLANK && evdata) {
        ...
        if (*blank == FB_BLANK_UNBLANK) {
            ...
            if(cd->cpdata->has_holster == 1){
                TP_LOG_INFO("%s: This Phone has holster,then resume.\n", __func__);
                cyttsp5_core_resume(cd->dev);
                cd->is_suspend = 0;
            ...
            if(cd->cpdata->has_holster == 1){
                TP_LOG_INFO("%s: This Phone has holster,then suspend.\n", __func__);
                cd->is_suspend = 1;
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
    ...
    rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq, irq_flags,
        dev_name(dev), cd);
    if (rc < 0) {
        TP_LOG_ERR("%s: Error, could not request irq, rc = %d\n",
                    __func__,rc);
    }
...
static int cyttsp5_parse_config(struct device_node *node,struct cyttsp5_power_control *power_ctrl)
{
    ...
    TP_LOG_INFO("irq_config = %d,vci_power_type = %d,vddio_power_type = %d\n",
...
static int cyttsp5_pinctrl_select_lowpower(struct cyttsp5_core_data *cd)
{
    ...
    if (retval < 0) {
        TP_LOG_ERR("%s: set pinctrl lowpower error\n", __func__);
    }
...
static int cyttsp5_regulator_get(struct device *dev, struct cyttsp5_power_control *power_ctrl)
{
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        ...
        if (IS_ERR(power_ctrl->vci)) {
            TP_LOG_ERR("Failed to get cyttsp5 vdd.\n");
            return  -EINVAL;
         }
    }
    ...
    if(power_ctrl->vddio_power_type == VDDIO_LDO_CTRL_MODE){
        ...
        if (IS_ERR(power_ctrl->vddio)) {
            TP_LOG_ERR("Failed to get cyttsp5 vbus.\n");
            if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
...
static int cyttsp5_regulator_put(struct cyttsp5_power_control *power_ctrl)
{
    ...
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        if (!IS_ERR(power_ctrl->vci)) {
            regulator_put(power_ctrl->vci);
            TP_LOG_INFO("%s: release vci regulator success.\n",__func__);
        }
    }
    ...
    if(power_ctrl->vddio_power_type == VDDIO_LDO_CTRL_MODE){
        if (!IS_ERR(power_ctrl->vddio)) {
            ...
            TP_LOG_INFO("%s: release vddio regulator success.\n",__func__);
        }
    }
...
static void cyttsp5_regulator_set(struct cyttsp5_power_control *power_ctrl)
{
    ...
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        ...
        if(rc < 0){
            TP_LOG_ERR("failed to set cyttsp5 vci to 2080V, rc = %d\n", rc);
        }
    }
    ...
    if(power_ctrl->vddio_power_type == VDDIO_LDO_CTRL_MODE){
        ...
        if(rc < 0){
            TP_LOG_ERR("failed to set cyttsp5 vddio\n");
        }
    }
...
static int cyttsp5_regulator_enable(struct cyttsp5_power_control *power_ctrl)
{
    ...
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        ...
        if (rc < 0) {
            TP_LOG_ERR("failed to enable cyttsp5 vci(2.8V), rc = %d\n", rc);
            goto exit;
        }
    }
    ...
    if(power_ctrl->vddio_power_type == VDDIO_LDO_CTRL_MODE){
        ...
        if (rc < 0) {
            TP_LOG_ERR("failed to enable cyttsp5 vddio, rc = %d\n", rc);
            goto exit;
         }
    }
...
static int cyttsp5_regulator_disable(struct cyttsp5_power_control *power_ctrl)
{
    if(power_ctrl->vci_power_type == VCI_LDO_CTRL_MODE){
        if (!IS_ERR(power_ctrl->vci)) {
            ...
            TP_LOG_INFO("%s: disable vci regulator success.\n",__func__);
            goto exit;
        }
    }
    ...
    if(power_ctrl->vddio_power_type == VDDIO_LDO_CTRL_MODE){
        if (!IS_ERR(power_ctrl->vddio)) {
            ...
            TP_LOG_INFO("%s: disable vddio regulator success.\n",__func__);
            goto exit;
        }
    }
...
{
    if(on != 0 && on != 1){
        TP_LOG_ERR("%s: input value is error, on = %d.\n",    __func__, on);
        return -EINVAL;
    }
        ...
        u16 irq, size_t xfer_buf_size)
{
    ...
    TP_LOG_INFO("%s: cyttsp5_core_probe start.\n", __func__);
    ...
    if (pdata->core_pdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
        if (!pdata->core_pdata->power) {
            TP_LOG_ERR("%s: Missing platform data function\n",
                    __func__);
            rc = -ENODEV;
            goto error_no_pdata;
        }
    ...
    if(rc < 0){
        TP_LOG_ERR( "%s: get mmi test limit error rc=%d\n", __func__, rc);
    }
    ...
    if (cd->cpdata->detect) {
        TP_LOG_INFO("%s: Detect HW\n", __func__);
        rc = cd->cpdata->detect(cd->cpdata, cd->dev,
                cyttsp5_platform_detect_read);
        if (rc) {
            TP_LOG_INFO("%s: No HW detected, rc = %d\n", __func__,rc);
            rc = -ENODEV;
            goto error_detect;
        }
    }
    ...
    TP_LOG_INFO("%s: cyttsp5_core_probe setup irq.\n", __func__);
    ...
    TP_LOG_INFO("%s: cyttsp5_mt_probe start.\n", __func__);
    ...
    TP_LOG_INFO("%s: cyttsp5_btn_probe start.\n", __func__);
    ...
    TP_LOG_INFO("%s: cyttsp5_proximity_probe start.\n", __func__);
    ...
    if (rc) {
        TP_LOG_ERR( "%s: Fail create link rc=%d\n", __func__, rc);
        goto error_attr_create;
    }
    ...
    TP_LOG_INFO("%s: cyttsp5_core_probe success.\n", __func__);
