...
static int parser_set_state(struct parser_data *data, char *field)
{
    ...
    if( new_state==ERROR ){
        TP_LOG_ERR("%s, updata state error.  field is %s\n", __func__, field);
        data->state = ERROR;
        return iRet;
    }
...
static void tp_field_read_callback(void *s, size_t len, void *d)
{
    ...
    if (data->state == ERROR){
        TP_LOG_ERR("%s: state is ERROR !\n",__func__ );
        return;
    }

    TP_LOG_DEBUG("%s,  current field is %s,  current state is %d.\n", __func__, field, data->state);
    ...
    switch (data->state) {
        case START:
            ...
            if (!ret){
                ...
                TP_LOG_INFO("%s: START field: %s\n", __func__, field);
            }
        break;

    default:
        if( (data->state == END)  || (data->state == ERROR) ){
            TP_LOG_INFO("%s,  END OR ERROR state is %d.\n", __func__, data->state);
            return;
        }
        ...
        if(  (data->state == OPENS_GETING)  || (data->state == IDAC_GETING)
            ...
            //ok
            TP_LOG_DEBUG("%s,  state is %d.\n", __func__, data->state);
        }
        else{//error
            TP_LOG_ERR("%s,  error state is %d.\n", __func__, data->state);
            return;
        }

        ret = sscanf(field, "%d", &idata);
        if (ret == 1) {
            TP_LOG_DEBUG("%s: state is %d, index is %d, data is %d\n", __func__, data->state, data->iCurNum, idata);
...
int tp_configuration_get(char *filename, struct cyttsp5_sensor_info_limit *sensor_info)
{
    ...
    TP_LOG_DEBUG("%s,  parser_init complete.\n", __func__);
    ...
    while(1){
        ...
        if(bytes_read <= 0) {
            TP_LOG_ERR("%s read file: %s  ERROR or end! \n", __func__, filename);
            break;
        }
        ...
        if(data.state ==  END) {
            TP_LOG_INFO("%s,  all need data has get success !\n", __func__);
            break;
        }
    }

    TP_LOG_DEBUG("%s csv_parse end !\n", __func__);
...
{
    TP_LOG_INFO("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \
        %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, \
        %d, %d, %d, %d \n", \
...
void mmi_info_limit_print(struct cyttsp5_sensor_info_limit *mmi_info)
{
    ...
    TP_LOG_INFO("%s,  opens.min data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    pData = mmi_info->opens.ebye_rx;
    TP_LOG_INFO("%s,  opens.ebye_rx data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    TP_LOG_INFO("%s,  opens.ebye_rx_allowed_max data is %d\n", __func__, mmi_info->opens.ebye_rx_allowed_max);
    TP_LOG_INFO("%s,  opens.ebye_rx_allowed_max_num data is %d\n", __func__, mmi_info->opens.ebye_rx_allowed_max_num);
    pData = mmi_info->opens.ebye_tx;
    TP_LOG_INFO("%s,  opens.ebye_tx data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    TP_LOG_INFO("%s,  opens.ebye_tx_allowed_max data is %d\n", __func__, mmi_info->opens.ebye_tx_allowed_max);
    TP_LOG_INFO("%s,  opens.ebye_tx_allowed_max_num data is %d\n", __func__, mmi_info->opens.ebye_tx_allowed_max_num);

    //printf idac;
    pData = mmi_info->idac.max;
    TP_LOG_INFO("%s,  idac data max is \n", __func__);
    print_oneline_txnumdata(0, pData);
    pData = mmi_info->idac.min;
    TP_LOG_INFO("%s,  idac data min is \n", __func__);
    print_oneline_txnumdata(0, pData);
    pData = mmi_info->idac.delta;
    TP_LOG_INFO("%s,  idac data delta is \n", __func__);
    print_oneline_txnumdata(0, pData);

    //printf local pwc;
    pData = mmi_info->localPWC.max;
    TP_LOG_INFO("%s,  localPWC.max data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    pData = mmi_info->localPWC.min;
    TP_LOG_INFO("%s,  localPWC.min data  is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    ...
    TP_LOG_INFO("%s,  mutual_noise.max data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    ...
    TP_LOG_INFO("%s,  mutual_rawdata.max data is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    pData = mmi_info->mutual_rawdata.min;
    TP_LOG_INFO("%s,  mutual_rawdata.min data  is \n", __func__);
    for(i=0; i<iRxMulTxNum;i+=iRxNum) {
        print_oneline_rxnumdata(i, pData);
    }
    ...
    TP_LOG_INFO("%s,  self_noise.max data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);

    //printf self rawdata;
    pData = mmi_info->self_rawdata.max;
    TP_LOG_INFO("%s,  self_rawdata.max data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);
    pData = mmi_info->self_rawdata.min;
    TP_LOG_INFO("%s,  self_rawdata.min data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);

    //printf self localpwc;
    TP_LOG_INFO("%s,  selflocalPWC.self_gidac_max_rx data is %d \n", __func__, mmi_info->selflocalPWC.self_gidac_max_rx);
    TP_LOG_INFO("%s,  selflocalPWC.self_gidac_max_tx data is %d \n", __func__, mmi_info->selflocalPWC.self_gidac_max_tx);
    TP_LOG_INFO("%s,  selflocalPWC.self_gidac_min_rx data is %d \n", __func__, mmi_info->selflocalPWC.self_gidac_min_rx);
    TP_LOG_INFO("%s,  selflocalPWC.self_gidac_min_tx data is %d \n", __func__, mmi_info->selflocalPWC.self_gidac_min_tx);
    pData = mmi_info->selflocalPWC.max;
    TP_LOG_INFO("%s,  selflocalPWC.max data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);
    pData = mmi_info->selflocalPWC.min;
    TP_LOG_INFO("%s,  selflocalPWC.min data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);
    pData = mmi_info->selflocalPWC.delta;
    TP_LOG_INFO("%s,  selflocalPWC.delta data(rx->tx) is \n", __func__);
    print_oneline_rxnumdata(0, pData);
    print_oneline_txnumdata(0, pData+iRxNum);
...
static int cyttsp5_get_sensor_info(struct device *dev)
{
    ...
    TP_LOG_INFO("%s,tx_num = %d, rx_num = %d,hw_mmi_index = %d.\n", __func__,
                mmi_info_limit.tx_num,mmi_info_limit.rx_num,hw_mmi_index);

    if(mmi_info_limit.tx_num <= 0 || mmi_info_limit.rx_num <= 0){
        TP_LOG_ERR("%s,get tx number & rx number by systeninfo error.\n", __func__);
        retval = -EINVAL;
        goto out;
    }

    TP_LOG_INFO("%s,product_name is %s, chip_name is %s, panel_id is %d.\n", __func__, product_name, chip_name, panel_id);
    ...
    TP_LOG_INFO("%s, config_file_name is %s.\n", __func__, config_file_name);
...
static int cyttsp5_change_scan_status(struct device *dev,int value)
{
    ...
    if(value != 0 && value != 1 && value != 2){
        TP_LOG_ERR( "%s Input value is error,value = %d.\n",__func__,value);
        return -1;
    }

    ret = cyttsp5_send_command(dev,Command_function[value]);
    if (ret < 0) {
        TP_LOG_ERR( "%s Failed to change scan status.\n",__func__);
        return -1;
    }
...
static void cyttsp5_build_test_command(struct device *dev,char *cmd,char *data_id,int data_len)
{
    int i,j;
    int readlength = 0;
    char *pad_cmd_head = "0x04 0x00 0x0A 0x00 0x2F 0x00 ";
    ...
    */
    ...
    TP_LOG_DEBUG("Enter %s,cmd = %s,data_id = %s,data_len = %d.\n",
    ...
    for(i = 0,j = 0;i < data_len;i += CY_READ_DATA_LEN,j++){
        ...
        strncpy(tmp_cmd_str,pad_cmd_head,sizeof(tmp_cmd_str));
                ...
                int data_len,int data_byte,int other)
{
    ...
    if (ret < 0 ) {
        TP_LOG_ERR("%s Failed to suspend scan.\n",__func__);
        return  -1;
    }
    ...
    if (ret < 0 ) {
        TP_LOG_ERR( "%s Failed to scan panel once.\n",__func__);
        goto exit;
    }
...
static int cyttsp5_get_rawdata(struct device *dev)
{
    ...
    for(i = 0;i < length;i++){
        ...
        || dad->data_buf[i] < m_rawdata.min[j]){
            TP_LOG_ERR( "%s check is failed. dad->data_buf[%d] is %d, rawdata min[%d]:max[%d]\n",__func__, i, dad->data_buf[i], m_rawdata.min[j], m_rawdata.max[j] );
            check_flag = -1;
            break;
        }
...
static int cyttsp5_get_self_rawdata(struct device *dev)
{
    ...
    for(i = 0;i < length;i++){
        if(dad->data_buf[i] > s_rawdata.max[j] || dad->data_buf[i] < s_rawdata.min[j]){
            TP_LOG_ERR( "%s check is failed. dad->data_buf[%d] is %d. s_rawdata[%d] min %d- max %d \n",__func__, i, dad->data_buf[i],  j, s_rawdata.min[j], s_rawdata.max[j]);
            check_flag = -1;
            break;
        }
        //jump over null data
...
static int cyttsp5_get_mutual_noise(struct device *dev)
{
    ...
    for (k = 0; k < 5; k++) {
        ret = cyttsp5_get_SensorData(dev,"0x00","0x2B",length,CYTTSP5_DATA_TWO_BYTE,0);
        if (ret < 0) {
            TP_LOG_ERR( "%s get rawdata failed.\n",__func__);
            goto exit;
        }
        if (k ==0 ) {
    ...
    for (i = 0; i < length; i++) {
        if(mut_noise.max[j] < dad->data_buf[i]) {
            TP_LOG_ERR( "%s mutual noise check  failed. raw[%d] is %d-%d, mut_noise.max[%d] is %d \n",__func__, i, min_raw[i], max_raw[i], j, mut_noise.max[j]);
            ret = -1;
            goto exit;
        }
        //jump over null data
...
static int cyttsp5_get_self_noise(struct device *dev)
{
    ...
    for (k = 0; k < 5; k++) {
        ret = cyttsp5_get_SensorData(dev,"0x03","0x2B",length,CYTTSP5_DATA_TWO_BYTE,0);
        if (ret < 0) {
            TP_LOG_ERR( "%s get self rawdata failed.\n",__func__);
            goto exit;
        }
        if (k == 0) {
            memcpy(max_raw,dad->data_buf,sizeof(int) * length);
    ...
    for (i = 0; i < length; i++) {
        if(s_noise.max[j] < dad->data_buf[i]) {
            TP_LOG_ERR( "%s self noise check  failed.  raw[%d] is %d-%d, s_noise.max[%d] is %d \n",__func__, i, min_raw[i], max_raw[i], j, s_noise.max[j]);
            ret = -1;
            goto exit;
        }
        //jump over null data
...
static int cyttsp5_get_LocalPWC(struct device *dev)
{
    ...
    for(i = 0;i < length;i++){
       if(dad->data_buf[iHeadNum+i] > localPWC.max[j] || dad->data_buf[iHeadNum+i] < localPWC.min[j]){
            TP_LOG_ERR( "%s check is failed. dad->data_buf[%d] is %d, localPWC[%d] max[%d] min[%d]\n",
...
static int cyttsp5_get_GlobalGIDAC(struct device *dev)
{
    ...
    for(i = 0;i < tx_num;i++){
        if(dad->data_buf[i] > idac.max[i] || dad->data_buf[i] < idac.min[i]){
            TP_LOG_ERR( "%s check is failed, dad->data_buf[%d] is %d, idac max[%d] min[%d].\n",
    ...
    for(i = 0;i < tx_num-1;i++){
        if(abs(dad->data_buf[i] - dad->data_buf[i+1]) > idac.delta[i]){
            TP_LOG_ERR( "%s uniformity check is failed,  delta data is [%d]%d-%d.\n",__func__, i, dad->data_buf[i], dad->data_buf[i+1]);
            delta_flag = -1;
            break;
        }
    }
   ...
   if( (dad->data_buf[0] > sPWC.self_gidac_max_rx ) || (dad->data_buf[0] < sPWC.self_gidac_min_rx) ) {
    ...
    for(i = 0;i < rx_add_tx;i++){
        if( (dad->data_buf[iHeadNum+i] > sPWC.max[j]) || (dad->data_buf[iHeadNum+i] < sPWC.min[j]) ) {
            if(i<rx_num){
                TP_LOG_ERR( "%s  Self-cap Local PWC rx %d check is failed. value is %d. sPWC[%d] min %d-max %d.\n",__func__, i, dad->data_buf[iHeadNum+i], j, sPWC.min[j], sPWC.max[j] );
                rx_flag = -1;
            }
            else{
                TP_LOG_ERR( "%s  Self-cap Local PWC tx %d check is failed. value is %d. sPWC[%d] min %d-max %d.\n",__func__, i, dad->data_buf[iHeadNum+i],  j, sPWC.min[j], sPWC.max[j] );
                tx_flag = -1;
            }
            goto exit;
        }
    ...
    for(i = 0;i < rx_add_tx-1;i++){
        ...
        if(abs(dad->data_buf[iHeadNum+i] - dad->data_buf[iHeadNum+i+1]) > sPWC.delta[j]) {
            if(i<rx_num){
                TP_LOG_ERR( "%s  Self-cap Local PWC delta rx %d check is failed. value is %d-%d. sPWC[%d] delta %d.\n",__func__, i, dad->data_buf[iHeadNum+i], dad->data_buf[iHeadNum+i+1], j, sPWC.delta[j] );
                rx_flag = -1;
            }
            else{
                TP_LOG_ERR( "%s  Self-cap Local PWC delta tx %d check is failed. value is %d-%d. sPWC[%d] delta %d.\n",__func__, i, dad->data_buf[iHeadNum+i],  dad->data_buf[iHeadNum+i+1], j, sPWC.delta[j] );
                tx_flag = -1;
            }
            goto exit;
        }
...
static int cyttsp5_get_OPENS(struct device *dev)
{
    ...
    for(i = 0;i< length ;i++) {
        if( dad->data_buf[i] < sen_opens.min[j]){
            TP_LOG_ERR("%s Opens MIN LIMITS error. %d data is %d, sen_opens.min[%d] is %d\n",__func__, i, dad->data_buf[i], j, sen_opens.min[j] );
            goto exit;
        }
        //jump over null data
        if( (i+1)%mmi_info_limit.rx_num == 0){
    ...
    for(i = 0;i< length-1 ;i++) {  //差值需要考虑
        ...
        if(abs(opens_diff[i]) > sen_opens.ebye_rx[j]){
            TP_LOG_ERR( "%s rx uniformity E-By-E check is failed, opens_diff[%d] is %d, sen_opens.ebye_rx[%d] is %d\n",__func__, i, opens_diff[i], j, sen_opens.ebye_rx[j]);
            goto exit;
        }
        ...
        if( abs(opens_diff[i]) > rx_allowed_max ) {
            ...
            if( iCount > rx_allowed_max_num) {
                TP_LOG_ERR( "%s rx allowed max data num check failed, data is %d, allowed num is %d\n",__func__, rx_allowed_max, rx_allowed_max_num);
                goto exit;
            }
        }
    ...
    for(i = 0; i< rx_num * (tx_num -1); i++) {
        ...
        if(abs(opens_diff[i]) > sen_opens.ebye_tx[j]){
            TP_LOG_ERR( "%s tx uniformity E-By-E check is failed, opens_diff[%d] is %d, sen_opens.ebye_tx[%d] is %d.\n",__func__, i, opens_diff[i], j, sen_opens.ebye_tx[j]);
            goto exit;
        }
        ...
        if( abs(opens_diff[i]) > tx_allowed_max ) {
            ...
            if( iCount > tx_allowed_max_num) {
                TP_LOG_ERR( "%s tx allowed max data num check failed, data is %d, allowed num is %d\n",__func__, tx_allowed_max, tx_allowed_max_num);
                goto exit;
            }
        }
    ...
    if (data_buf[0] == 0x07
        ...
        && (data_buf[4] == 0xA6 || data_buf[4] == 0x26)) {
            if (data_buf[5] != 0x0 || data_buf[6] != 0x0) {
                TP_LOG_ERR("%s Short test failed.\n",__func__);
                check_flag = 0;
        }
    } else {
        TP_LOG_ERR("%s Short test failed.\n",__func__);
...
static int cyttsp5_proc_data_print(struct device *dev,int *data_buf,struct seq_file *m)
{
    ...
    if(data_buf == NULL){
        TP_LOG_INFO("%s data_buf is NULL.\n",__func__);
        return 0;
    }
...
int cyttsp5_check_items(struct device *dev,enum cyttsp5_sensor_check item, bool reset,int *data_buf)
{
    ...
    if (item < CYTTSP_I2C_PING || item >= CYTTSP_CHECK_MAX) {
        TP_LOG_ERR("%s: invalid input item\n", __func__);
        return -EINVAL;
    }
...
static int cyttsp5_rawdata_proc_show(struct seq_file *m, void *v)
{
    ...
    if(error < 0){
        TP_LOG_ERR("%s Failed to get sensor info.\n",__func__);
        goto out;
    }
...
static int cyttsp5_data_proc_show(struct seq_file *m, void *v)
{
    ...
    TP_LOG_INFO("cyttsp5_data_proc_show, buffer size = %ld\n", m->size);
...
out:
    atomic_set(&mmi_test_status, 0);
    TP_LOG_INFO("cyttsp5_data_proc_show done.\n");
    return error;
}
