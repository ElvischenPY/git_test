    ...
    struct cyttsp5_touch *touch, u8 *xy_mode)
{
    ...
    TP_LOG_DEBUG("%s: time=%X tch_num=%d lo=%d noise=%d counter=%d\n",
        __func__,
    ...
    struct cyttsp5_touch *touch)
{
    ...
    TP_LOG_DEBUG("%s: flip=%s inv-x=%s inv-y=%s x=%04X(%d) y=%04X(%d)\n",
    ...
    unsigned int locus_num)
{
    ...
    if (IS_GESTURE_ENABLE(gesture, cd->easy_wakeup_gesture)) {
        *reprot_gesture_key_value =    cd->wakeup_keys->keys[gesture];
        ...
        TP_LOG_DEBUG("%s: Gesture detected,cd->gesture_count[%d]=%d!\n",__func__, gesture,cd->gesture_count[gesture]);
    ...
    unsigned int *reprot_gesture_point_num)
{
    ...
    case LINEAR_SLIDE_TOP_TO_BOTTOM2:
        ...
        break;
    default:
        TP_LOG_ERR("%s:unknow event_value = 0x%x\n", __func__, event_value);
        return -1;
    }
    return 0;
}
    ...
    unsigned int *reprot_gesture_point_num)
{
    ...
    case CIRCLE_SLIDE_CLOCKWISE:
        ...
        break;
    default:
        TP_LOG_DEBUG("%s:unknow event_value = 0x%x\n", __func__, event_value);
        return -1;
    }
    return 0;
}
    ...
    switch(event) {
        ...
        case LINEAR_SLIDE_DETECTED:
            if (!(APP_ENABLE_LINEAR & cd->easy_wakeup_gesture)) {
                TP_LOG_INFO("%s:LINEAR_SLIDE not enabled!\n",__func__);
                return 0;
            }

            TP_LOG_INFO("%s:LINEAR_SLIDE detected!\n",__func__);
        ...
        case SPECIFIC_LETTER_DETECTED:
            if (!(APP_ENABLE_LETTERS & cd->easy_wakeup_gesture)) {
                TP_LOG_INFO("%s:SPECIFIC_LETTER not enabled!\n", __func__);
                return 0;
            }
            TP_LOG_INFO("%s:SPECIFIC_LETTER detected!\n", __func__);
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md)
{
    ...
    TP_LOG_DEBUG("%s: extract data num_cur_tch=%d\n", __func__,
        num_cur_tch);
    if (num_cur_tch)
...
int cyttsp5_mt_probe(struct device *dev)
{
    ...
    if (md->si) {
        rc = cyttsp5_setup_input_device(dev);
        if (rc) {
            TP_LOG_ERR("%s,cyttsp5_setup_input_device fail, rc = %d\n",
                        __func__,rc);
