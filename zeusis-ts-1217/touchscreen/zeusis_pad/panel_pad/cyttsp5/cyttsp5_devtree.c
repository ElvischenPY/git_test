...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
    ...
    if (rc) {
        TP_LOG_ERR("%s,get_inp_dev_name fail, rc = %d\n",__func__,rc);
    ...
    if (!rc) {
        pdata->flags = value;
        TP_LOG_INFO("%s,pdata->flags is %d\n", __func__,value);
    } else {
        TP_LOG_ERR("%s,[cy,flags] read fail, rc = %d\n",__func__,rc);
    }

    rc = of_property_read_u32(dev_node, "cy,vkeys_x", &value);
    if (!rc) {
        pdata->vkeys_x = value;
        TP_LOG_INFO("%s,pdata->vkeys_x is %d\n", __func__,value);
    } else {
        TP_LOG_ERR("%s,[cy,vkeys_x] read fail, rc = %d\n",__func__,rc);
    }

    rc = of_property_read_u32(dev_node, "cy,vkeys_y", &value);
    if (!rc) {
        pdata->vkeys_y = value;
        TP_LOG_INFO("%s,pdata->vkeys_y is %d\n", __func__,value);
    } else {
        TP_LOG_ERR("%s,[cy,vkeys_y] read fail, rc = %d\n",__func__,rc);
    }

    /* Required fields */
    pdata->frmwrk = create_and_get_touch_framework(dev_node);
    if (pdata->frmwrk == NULL) {
        TP_LOG_ERR("%s,pdata->frmwrk is NULL\n", __func__);
        rc = -EINVAL;
        goto fail_free_pdata;
    } else if (IS_ERR(pdata->frmwrk)) {
        rc = PTR_ERR(pdata->frmwrk);
        TP_LOG_ERR("%s,pdata->frmwrk is error, rc = %d\n", __func__,rc);
        goto fail_free_pdata;
    }
#ifdef ENABLE_VIRTUAL_KEYS
...
static void *create_and_get_btn_pdata(struct device_node *dev_node)
{
    ...
    rc = get_inp_dev_name(dev_node, &pdata->inp_dev_name);
    if (rc) {
        TP_LOG_ERR("%s,get_inp_dev_name fail, \n", __func__);
        goto fail_free_pdata;
    }
...
static void *create_and_get_proximity_pdata(struct device_node *dev_node)
{
    ...
    pdata->frmwrk = create_and_get_touch_framework(dev_node);
    if (pdata->frmwrk == NULL) {
        TP_LOG_ERR("%s,create_and_get_touch_framework fail.\n", __func__);
        rc = -EINVAL;
        goto fail_free_pdata;
    } else if (IS_ERR(pdata->frmwrk)) {
        TP_LOG_ERR("%s,create_and_get_touch_framework fail.\n", __func__);
        ...
        struct device_node *core_node, const char *name)
{
    ...
    if (NULL == core_node || NULL == name) {
        TP_LOG_ERR("%s,core_node or name is missing\n", __func__);
        return NULL;
    }

    data = create_and_get_u16_array(core_node, name, &size);
    if (IS_ERR_OR_NULL(data)) {
        TP_LOG_ERR("%s,create and get %s fail.\n", __func__,name);
        return (void *)data;
    }
    ...
    if (!rc) {
        TP_LOG_DEBUG("%s,[%s] is %d.\n", __func__, name, tag_value);
        setting->tag = tag_value;
    } else {
        TP_LOG_ERR("%s,[%s] read fail, rc = %d.\n", __func__, name, rc);
    }
        ...
        struct device_node *core_node)
{
    ...
    pdata = kzalloc(sizeof(*pdata), GFP_KERNEL);
    if (!pdata) {
        TP_LOG_ERR("%s,kzalloc core_pdata fail.\n", __func__);
        rc = -ENOMEM;
        goto fail;
    }
    ...
    if (!rc) {
        ...
        TP_LOG_DEBUG("%s,pdata->easy_wakeup_gesture = %d.\n",
    ...
    if (!rc) {
        ...
        TP_LOG_DEBUG("%s:easy_wakeup_supported_gestures = %d.\n", __func__, value);
    } else {
    ...
    if (!rc) {
        ...
        TP_LOG_DEBUG("%s,pdata->upgrade_ttconfig = %d.\n",
                    __func__,pdata->upgrade_ttconfig);
    } else {
        pdata->upgrade_ttconfig = 0;
        TP_LOG_ERR("%s,[cy,upgrade_ttconfig] read fail, rc = %d.\n",__func__, rc);
        goto fail_free;
    }
    ...
    if (!rc) {
        ...
        TP_LOG_DEBUG("%s,pdata->has_holster = %d.\n",
    ...
    if (!rc) {
        ...
        TP_LOG_DEBUG("%s,pdata->no_cali_flag = %d.\n",
    ...
    TP_LOG_INFO("%s: " "product_name:%s chip_name:%s irq_gpio:%d rst_gpio:%d"
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
    ...
    if (!adap_dev->of_node) {
        TP_LOG_ERR("%s,adap_dev->of_node is null.\n", __func__);
        return -ENODEV;
    }

    pdata = kzalloc(sizeof(*pdata), GFP_KERNEL);
    if (!pdata) {
        TP_LOG_ERR("%s,kzalloc pdata memory fail.\n", __func__);
        return -ENOMEM;
    }
    ...
    for_each_child_of_node(adap_dev->of_node, core_node) {
        ...
        if (!rc) {
            TP_LOG_INFO("%s,core_node name:%s\n", __func__,name);
        } else {
            TP_LOG_ERR("%s,core_node name get fail, rc = %d.\n",
                        __func__,rc);
        }

        pdata->core_pdata = create_and_get_core_pdata(core_node);
        if (IS_ERR(pdata->core_pdata)) {
            TP_LOG_INFO("%s,create_and_get_core_pdata fail.\n", __func__);
        ...
        for_each_child_of_node(core_node, dev_node) {
            ...
            if (rc) {
                TP_LOG_ERR("%s,get_device_type fail, rc = %d.\n",__func__,rc);
                break;
            }
            ...
            *pdata_ptr[type].pdata
            ...
            if (IS_ERR(*pdata_ptr[type].pdata)) {
                ...
                TP_LOG_ERR("%s,create_and_get_device_pdata fail, rc = %d\n", __func__,rc);
            }
    ...
    TP_LOG_INFO("%s: %d child node(s) found\n", __func__, count);
