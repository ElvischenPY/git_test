...
int cyttsp5_get_holster_mode(struct device *dev)
{
	...
	if (rc || !cyttsp5_command_status(dev)) {
		TP_LOG_ERR("%s: Send read TP holster mode command failed.\n", __func__);
		return -1;
	}
	...
	if (!cyttsp5_command_status(dev) || num_read < 0) {
		TP_LOG_ERR("%s: Get TP holster mode command response failed.\n", __func__);
		return -1;
	}

    for (i = 0; i < num_read; i++) {
        TP_LOG_DEBUG("%s,response_buf[%d]=0x%02X.\n", __func__,i,response_buf[i]);
		index += scnprintf(response_str + index, COVER_RESPONSE_MAX_LEN,
			"%02X ", response_buf[i]);
	}

    TP_LOG_DEBUG("%s,response_str=%s.\n", __func__,response_str);
    ...
    TP_LOG_ERR("%s: get touch holster mode failed.\n", __func__);
	return -1;
}
...
int cyttsp5_set_holster_mode(struct device *dev,int holster_switch)
{
	...
	TP_LOG_DEBUG("Enter %s,holster_switch = %d.\n", __func__,holster_switch);
	...
	if (holster_switch == flag) {
		TP_LOG_INFO("%s,Touch is already in %s mode.\n",__func__,(flag == 1) ? "holster ON" : "holster OFF");
		return 0;
	}

    TP_LOG_DEBUG("%s,Begin to switch holster mode.\n", __func__);
	...
	num_read = cyttsp5_command_response(dev,response_buf);
	if (!cyttsp5_command_status(dev) || num_read < 0) {
		TP_LOG_ERR("%s: Get command response failed\n", __func__);
        goto exit;
	}
    ...
    for(i = SET_HOLSTER_SUCCESSFUL_ONE;i <= SET_HOLSTER_SUCCESSFUL_TWO;i++){
        if (strncmp(holster_response_state[i], response_str, length) == 0){
            TP_LOG_INFO("%s,Set TP holster mode successful!\n",__func__);
            return 0;
        }
	}
    ...
    rc = -1;
exit:
    TP_LOG_ERR("%s,Set TP holster mode failed.\n",__func__);
	return rc;
}
...
void cyttsp5_holster_mode_work_function(struct work_struct *work)
{
	...
	if (rc < 0) {
		TP_LOG_INFO("%s:set tp holster mode %s failed!\n",
...
int cyttsp5_in_available_area(struct device *dev, int x_axis, int y_axis)
{
	...
	TP_LOG_DEBUG("%s:holster value is %d, (%d,%d), (%d,%d)\n",
