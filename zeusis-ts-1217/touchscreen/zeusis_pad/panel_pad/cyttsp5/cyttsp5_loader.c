        ...
        u32 fw_ver_new, u32 fw_revctrl_new)
{
    ...
    TP_LOG_INFO("%s: Builtin fw version:0x%04X  IC fw version:0x%04X\n", __func__,
            fw_ver_img, fw_ver_new);

    if (fw_ver_new > fw_ver_img) {
        TP_LOG_INFO("%s: Builtin fw version is High!\n", __func__);
        return 1;
    }

    if (fw_ver_new < fw_ver_img) {
        TP_LOG_INFO("%s: Builtin fw version is Low!\n", __func__);
        return -1;
    }

    fw_revctrl_img = ld->si->cydata.revctrl;

    TP_LOG_INFO("%s: Builtin fw revctrl:0x%04X IC fw revctrl:0x%04X\n", __func__,
    ...
    TP_LOG_INFO("%s: version info same\n", __func__);
...
static int cyttsp5_load_app_(struct device *dev, const u8 *fw, int fw_size)
{
    ...
    TP_LOG_INFO("%s: Enter to load app\n", __func__);
    ...
    while (p < last_row) {
        ...
        TP_LOG_DEBUG("%s: array_id=%02X row_num=%04X(%d) row_size=%04X(%d)\n",
        ...
        if (rc) {
            ...
            TP_LOG_DEBUG("%s: Parse Row (a=%d r=%d ret=%d\n",
                        __func__, row_image->array_id,
                        row_image->row_num, rc);
        }
    ...
    TP_LOG_INFO("%s:load app successful.\n", __func__);
        ...
        int fw_size)
{
    ...
    TP_LOG_INFO("%s: cyttsp5 upgrade firmware\n", __func__);
        ...
        struct device *dev)
{
    ...
    while ((fw = *fws++)) {
        ...
        TP_LOG_DEBUG("%s: Found mismatching fw:%p with Panel ID: 0x%02X\n",
            __func__, fw, fw->panel_id);
    }
        ...
        bool forced)
{
    ...
    TP_LOG_INFO("%s: upgrade firmware from platform start.\n",
                __func__);
    if (!ld->loader_pdata) {
...
static void _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
    ...
    if (!fw) {
        ...
        #endif
        TP_LOG_ERR("%s: Missing parameter\n", __func__);
        ...
        const struct firmware *fw)
{
    ...
    if (upgrade > 0) {
        TP_LOG_INFO("%s: fw will upgrade!\n", __func__);
        return 1;
    }

    TP_LOG_INFO("%s: fw will not upgrade!\n", __func__);
    return 0;
}
...
static int upgrade_firmware_from_class(struct device *dev)
{
    ...
    TP_LOG_INFO( "%s: Enabling firmware class loader\n", __func__);
    ...
    if (retval < 0) {
        TP_LOG_ERR("%s: Fail request firmware class file load, ret = %d\n",
                    __func__,retval);
        return retval;
    }
...
static char *generate_firmware_filename(struct device *dev)
{
    ...
    if (!filename) {
        TP_LOG_ERR("%s: kzalloc fail\n", __func__);
        return NULL;
    }

    TP_LOG_INFO("%s: start to generate_firmware_filename.\n",__func__);

    panel_id = cyttsp5_get_panel_id(dev);
    TP_LOG_INFO("%s: panel_id = %d \n", __func__, panel_id);

    product_name = core_pdata->product_name;
    TP_LOG_INFO("%s: product_name = %s\n", __func__, product_name);
    ...
    if (!ld->si) {
        ...
        TP_LOG_INFO("%s: sysinfo is null, use default firmware: %s\n", __func__, filename);
    } else {
    ...
    if ((ld->si) && (findfirmware == false)) {
        snprintf(filename, FILENAME_LEN_MAX, "ts/%s_%s_default.bin", product_name, chip_name);
        TP_LOG_INFO("%s: no matching firmware, use default firmware: %s\n", __func__, filename);
    }

    if(strlen(filename) < strlen("ts/x_x_x.bin")){
        TP_LOG_INFO("%s: Filename = %s is inval.\n", __func__, filename);
        return NULL;
    }
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
    ...
    TP_LOG_INFO("%s: upgrade firmware from buildin start.\n", __func__);
    ...
    if (!filename) {
        ...
        #endif
        TP_LOG_ERR("%s: generate firmware filename fail\n", __func__);
        return -ENOMEM;
    }
        ...
        const u8 *ttconfig_data, int ttconfig_size)
{
    ...
    /* Check for config CRC */
        ...
        if (rc) {
            TP_LOG_INFO( "%s: crc check fail, will not upgrade ttconfig\n", __func__);
            return 0;
        }
        ...
        struct cyttsp5_touch_config *ttconfig)
{
    ...
    if (cyttsp5_check_firmware_version(dev, fw_ver_config, fw_revctrl_config)) {
        TP_LOG_ERR( "%s: FW and ttconfig versions mismatch, will not upgrade\n", __func__);
        return 0;
    }
        ...
        struct device *dev)
{
    ...
    if (!ttconfigs) {
        TP_LOG_ERR("%s: not ttconfigs data found\n", __func__);
        return NULL;
    }
    ...
    while ((ttconfig = *ttconfigs++)) {
        ...
        TP_LOG_INFO("%s: ttconfig->panel_id = %d,panel_id = %d.\n", __func__,ttconfig->panel_id, panel_id);
        TP_LOG_INFO("%s: ttconfig->product_name = %s,product_name = %s.\n", __func__,ttconfig->product_name, product_name);
        ...
        TP_LOG_DEBUG("%s: Found mismatching ttconfig:%p with Panel ID: 0x%02X\n",
            __func__, ttconfig, ttconfig->panel_id);
    }
...
static int upgrade_ttconfig_from_platform(struct device *dev)
{
    ...
    TP_LOG_INFO("%s: upgrade ttconfig from platform start.\n",__func__);
    if (!ld->loader_pdata) {
    ...
    if (upgrade) {
        TP_LOG_INFO("%s: upgrade ttconfig\n", __func__);
        rc = cyttsp5_upgrade_ttconfig(dev, param_regs->data, param_regs->size);
        TP_LOG_INFO("%s: upgrade ttconfig, rc = %d\n", __func__,rc);
        return rc;
    }
        ...
        char *buf, loff_t offset, size_t count)
{
    ...
    TP_LOG_DEBUG("%s: offset:%lld count:%zu\n", __func__, offset, count);
        ...
        struct work_struct *fw_and_config_upgrade)
{
    ...
    if (!upgrade_firmware_from_platform(dev, false)) {
        TP_LOG_ERR("%s: PLATFORM_FW_UPGRADE fail\n", __func__);
    } else {
        TP_LOG_INFO("%s: update firmware from platform\n", __func__);
    }
#endif
    ...
    if (!retval) {
        TP_LOG_INFO("%s: firmware have upgraded successful.\n", __func__);
    } else {
#if defined (CONFIG_ZEUSIS_DSM)
...
#endif
        TP_LOG_INFO("%s: firmware haven't upgraded, retval = %d\n", __func__,retval);
    }
#endif

    if(core_pdata->upgrade_ttconfig == 0){
        TP_LOG_INFO("%s: Not need to upgrade ttconfig,return here.\n", __func__);
        goto exit;
    }
    ...
    if (!retval) {
        TP_LOG_INFO("%s: ttconfig have upgreded\n", __func__);
    } else {
        TP_LOG_INFO("%s: ttconfig haven't upgrade, retval = %d\n", __func__,retval);
    }
...
exit:
    ...
    mutex_unlock(&cd->system_lock);
    if (core_pdata->support_sec_upgrd_firmware) {
        TP_LOG_INFO("is_firmware_broke:[%d]\n",ld->is_firmware_broke);
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    TP_LOG_ERR("%s: Enter manual_upgrade\n", __func__);
    if (ld->is_manual_upgrade_enabled) {
        TP_LOG_ERR("%s: System is doing fw upgrade, please try later\n",
                    __func__);
        return -EBUSY;
    }
    ...
    if (rc < 0) {
        TP_LOG_ERR("%s: upgrade firmware from class fail\n", __func__);
        ld->is_manual_upgrade_enabled = 0;
    }
...
static int cyttsp5_fw_update_from_sdcard(struct device *dev,u8 *file_path,bool update_flag)
{
    ...
    TP_LOG_INFO("%s: header_size = %d,fw_size = %d.\n",__func__,header_size,fw_size);

    if (header_size >= (fw_size + 1)) {
        TP_LOG_ERR("%s: Firmware format is invalid,header_size = %d,fw_size = %d.\n",
            __func__,header_size,fw_size);
        goto exit;
    }
	...
	if (update_flag == 1){
        TP_LOG_INFO("%s force update fw form sdcard.\n",__func__);
                ...
                struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    TP_LOG_INFO("%s: start to update fw from sdcard.\n", __func__);

    if(buf == NULL){
        TP_LOG_ERR("%s: input buf is NULL.\n", __func__);
        return -EINVAL;
    }

  	rc = kstrtoul(buf, 10, &update_flag);
	if (rc < 0){
        TP_LOG_INFO("%s: kstrtoul error.\n",__func__);
		return rc;
	}

	if (update_flag != 0x01 && update_flag != 0x00){
        TP_LOG_INFO("%s: input value is error.\n",__func__);
		return -EINVAL;
	}
    ...
    TP_LOG_INFO("%s: file_path = %s.\n",__func__,CYTTSP5_FW_FROM_SD_NAME);

    rc = cyttsp5_fw_update_from_sdcard(dev,CYTTSP5_FW_FROM_SD_NAME,update_flag);
    if (rc < 0){
        TP_LOG_ERR("%s: fw update from sdcard failed.\n",__func__);
        goto exit;
    }

    TP_LOG_INFO("%s: fw update from sdcard is successful.\n",__func__);
...
static int cyttsp5_loader_probe(struct device *dev)
{
    ...
    if (rc) {
        TP_LOG_ERR("%s: Error, could not create fw_update_sd\n",
                __func__);
...
static int __init cyttsp5_loader_init(void)
{
    ...
    if (!cmd) {
        TP_LOG_ERR("%s: cyttsp5_get_commands fail\n", __func__);
        return -EINVAL;
    }
