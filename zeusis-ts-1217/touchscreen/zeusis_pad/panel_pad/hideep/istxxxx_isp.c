...
static int pgm_r_reg(struct ts_bus_info *ext_io, u32 addr, u32 *val)
{
	...
	ret = zeusis_ts_i2c_read(bulk, 5, (u8*)&(packet[2]), 4);
#else
...
static int pgm_r_mem(struct ts_bus_info *ext_io, u32 addr, struct pgm_packet *packet, u32 len)
{
	...
	packet->header.w[1] = htonl(addr);
#ifdef HIDEEP_REPEAT_START
	ret = zeusis_ts_i2c_read((u8*)&packet->header.b[3], 5, (u8*)packet->payload, len);
#else
...
static int hideep_enter_pgm(struct ts_bus_info *ext_io)
{
	...
	TP_LOG_INFO("hideep_enter_pgm enter\n");
	...
	if( status != htonl(pattern)){
		TP_LOG_ERR("enter_pgm : error(%08x):\n", status);
		return -1;
	}
...
void hideep_read_6spp(struct ts_bus_info *ext_io)
{
	...
	TP_LOG_INFO("%s, from 0 to 3 is [0x%08x],[0x%08x],[0x%08x],[0x%08x]\n",__FUNCTION__ ,temp[0], temp[1], temp[2], temp[3]);
}
static int hideep_program_nvm(struct ts_bus_info *ext_io, const u8 *ucode, size_t len, int offset)
{
	...
	if(retry < 0){
		TP_LOG_ERR("enter_pgm : failed\n");
		return -1;
	}
	TP_LOG_INFO("%s, pgm is enter\n",__FUNCTION__);
	hideep_read_6spp(ext_io);
	hideep_pgm_protect(ext_io);
	pages      = (len + NVM_PAGE_SIZE - 1)/ NVM_PAGE_SIZE;
	addr       = offset;
	TP_LOG_DEBUG("addr = 0x%x, len = %d(0x%x)",addr, len);
	len_r      = len;
	len_w      = len_r;
	for(i=0; i<pages; i++)	{
		...
		if(ret < 0){
			TP_LOG_ERR("hideep_program_nvm : error(%08x):\n", addr);
		}
		addr   += NVM_PAGE_SIZE;
		len_r  -= NVM_PAGE_SIZE;
		len_w   = len_r;
...
static int hideep_verify_nvm(struct ts_bus_info *ext_io, const u8 *ucode, size_t len, int offset)
{
	...
	for(i=0; i<pages; i++){
		...
		for(j=0;j<(len_v/4);j++)
			TP_LOG_DEBUG("address = 0x%08x, Read data = 0x%08x, Source data = 0x%08x",addr+j*4,pbuf[j],psourcebuf[j]);
		ret  = memcmp(&(ucode[addr]), packet_r.payload, len_v);
		if(ret != 0){
			u8 *read = (u8*) packet_r.payload;
			for(j=0; j<NVM_PAGE_SIZE; j++)
			TP_LOG_ERR("%02x : %02x\n", ucode[addr+j], read[j]);
			TP_LOG_ERR("verify : error(addr : %d)\n", addr);
			ret = -1;
		}
		addr   += NVM_PAGE_SIZE;
		len_r  -= NVM_PAGE_SIZE;
...
int hideep_fuse_ucode( struct ts_bus_info *ext_io, const u8 *code, size_t len, int offset)
{
	...
	TP_LOG_INFO("start, len = %d,offset = 0x%x", len, offset);
	...
	while(retry--){
		...
		if(ret == 0)
			break;
		TP_LOG_ERR(" download uc failed(%d)\n", retry);
	}
	if(retry != 0){
		TP_LOG_INFO("download uc success\n"          );
	}
#endif
...
int hideep_load_ucode(struct ist510e *ts_drv, const char *fn, u16 ver)
{
	...
	struct ts_bus_info *bus_io   = ts_drv->zeusis_ts_data->bops;
	u16 version;

	ret = request_firmware(&fw_entry, fn, dev);
	if(ret != 0){
		TP_LOG_INFO("request_firmware : fail(%d)\n", ret);
		goto err;
	}
	memcpy((unsigned char*)&version,  fw_entry->data+HIDEEP_VR_VERSION_ADDR_BASE,2);
	TP_LOG_INFO("current fw = %d.%2d, binary fw = %d.%2d",ver>>8,ver&0xff, version>>8,version&0xff);
	if(version <= ver){
		TP_LOG_INFO("no need to update.\n");
