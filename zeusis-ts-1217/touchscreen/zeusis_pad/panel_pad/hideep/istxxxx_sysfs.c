...
ssize_t mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return count;
mode_store_err:
	TP_LOG_ERR("incorrect count = %d\n", count);
	if(count)
		TP_LOG_ERR("buf[0] = 0x%x\n", buf[0]);
	return -1;
}
...
ssize_t vr_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TP_LOG_INFO("Value(0x%04x)= 0x%02x\n", dev_info->vr_addr, val);
	return dev_info->vr_size;
vr_store_err:
	TP_LOG_INFO("vr store error ret = 0x%x\n", ret);
	return -1;
}
 ...
 ssize_t touch_event_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	TP_LOG_INFO("touch_event_show enter \n");
...
int hideep_sysfs_init(struct ist510e *ts)
{
	...
	ret = sysfs_create_group(&g_tp_data.ts_dev->dev.kobj, &ist510e_ts_attr_group);
	if (ret != 0){
		TP_LOG_ERR("unable to create sysfs group.\n");
		goto err;
	}
	TP_LOG_INFO("hideep_sysfs_init finished\n");
...
int hideep_sysfs_uninit(struct ist510e *ts)
{
	...
	if(ts->event_time)
		kfree(ts->event_time);
#endif
	sysfs_remove_group(&g_tp_data.ts_dev->dev.kobj, &ist510e_ts_attr_group);
	return 0;
}
