...
static int32_t hideep_get_vreg(struct ist510e *ts,u32 addr, u32 len)
{
	...
	TP_LOG_INFO("hideep_get_vreg start\n");
	ret = hideep_i2c_read(ts, addr, len, debug_dev->vr_buff  );
	if(ret < 0)
		goto i2c_err;
	TP_LOG_INFO("hideep_get_vreg(0x%02x:%d)\n", addr, len);

	return ret;
i2c_err:
	TP_LOG_ERR("hideep_get_vreg error\n");
	return ret;
}
...
static int32_t hideep_set_vreg(struct ist510e *ts, u32 addr, u32 len)
{
	...
	TP_LOG_INFO("hideep_set_vreg start\n");
	do{
	...
	TP_LOG_INFO("(0x%02x:%d)\n",  addr, len);

	return ret;
i2c_err:
	TP_LOG_ERR("hideep_set_vreg error\n");
	return ret;
}
...
static size_t hideep_download_uc(struct ist510e *ts, const char __user *uc, int offset, size_t count)
{
	...
	TP_LOG_INFO("start offset = 0x%0x, count = %d\n",offset, count);
	len += count;
	len -= 47*1024;
	if( len > 2048)
		goto hideep_download_uc_err;

	TP_LOG_INFO("hideep_download_uc processing\n");
	...
	if (ret < 0){
		TP_LOG_ERR("ADDR_UC : copy_to_user\n");
		kfree(ucode);
		return 0;
	}
	disable_irq(ts->irq);
	ret = hideep_fuse_ucode(ts->zeusis_ts_data->bops, ucode, count, offset);
	enable_irq(ts->irq);
	if (ret < 0){
		TP_LOG_ERR("Fail fuse ucode\n");
		kfree(ucode);
		return 0;
	}
	kfree(ucode);
	TP_LOG_INFO("ist510e_download_uc(%d)\n", count);

	return count;
hideep_download_uc_err:
	TP_LOG_ERR("data(size = %d) to be downloaded is oversize \n",(offset)+ count);
	return -1;
}
...
static int hideep_iface_open(struct inode *inode, struct file *file)
{
	...
	TP_LOG_INFO("hideep_iface_open start\n");
	dev_info = container_of(inode->i_cdev, struct ist510e_debug_dev, cdev);
	if (dev_info == NULL){
		TP_LOG_ERR("No such char device node\n");
		return -ENODEV;
	}
	file->private_data = dev_info;
	TP_LOG_INFO("hideep_iface_open end\n");
...
{
	TP_LOG_INFO("hideep_iface_release start\n");
...
static unsigned int hideep_iface_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	TP_LOG_INFO("hideep_iface_poll start\n");
	if(!file->private_data){
		TP_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	...
	TP_LOG_INFO("hideep_iface_poll end\n");
	return mask;
}
...
static ssize_t hideep_iface_read(struct file *file, char __user *buf, size_t count, loff_t *offset)
{
	...
	TP_LOG_INFO("hideep_iface_read start count = %d, (*offset) = %d\n",count,*offset);
	if(!file->private_data){
		TP_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	...
	if( (*offset) <ADDR_VR_END ){
		...
		TP_LOG_INFO("reading image\n");
		drv_info->im_size = count;
		rd_buffer = drv_info->im_buff;
		rd_len = count;
	}else{
		TP_LOG_ERR("ist510e_read : undefined address\n");
		return -EFAULT;
	}
	if(rd_buffer == NULL){
		TP_LOG_ERR("hideep_iface_read error\n");
	...
	if (ret < 0){
		TP_LOG_ERR("error : copy_to_user\n");
		return -EFAULT;
	}
	TP_LOG_INFO("hideep_iface_read end\n");
	return rd_len;
...
static ssize_t hideep_iface_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	...
	TP_LOG_INFO("hideep_iface_write count = %d, (*offset) = %d\n",count,*offset);
	if(!file->private_data){
		TP_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	if((*offset) <ADDR_VR_END ){
		...
		if(( VR_SINGLE_TOUCH ==(*offset))&&(1==count)){
			...
			switch(drv_info->vr_buff[0]){
				...
				case 2:
					...
					break;
				default:
					TP_LOG_ERR("error apple weight incorrect parameter\n");
					break;
			}
			return 1;
		}else
		...
		TP_LOG_INFO("download fw vr, offset = 0x%x, count = %d\n", (*offset),count);
		wr_len = hideep_download_uc(drv_info->ts, buf, (*offset)&0xffff, count);
	}else{
		TP_LOG_ERR("ist510e_write : undefined address\n");
		return 0;
	}
	TP_LOG_INFO("hideep_iface_write end\n");
...
static loff_t hideep_iface_llseek(struct file *file, loff_t off, int whence)
{
	...
	TP_LOG_INFO("start, whence = %d, offset = 0x%08x\n", whence, off);
	if(!file->private_data){
		TP_LOG_ERR("file->private_data is NULL\n");
		return -EFAULT;
	}
	switch(whence){
		...
		case 1  :
			TP_LOG_INFO("set mode, off = 0x%08x\n",off);
			if(off == 0x1000){
				drv_info->im_r_en = 1;
		...
		case 2  :
			TP_LOG_INFO("set im/vr size, off = 0x%08x\n",off);
			drv_info->im_size = off&0xffff;
			drv_info->vr_size = (off>>16)&0xffff;
			TP_LOG_INFO("ISTCORE_IFACE_CFG : %d\n", drv_info->im_size);
			TP_LOG_INFO("ISTCORE_IFACE_CFG : %d\n", drv_info->vr_size);
			newpos=file->f_pos;
			break;
		default:
	...
	if (newpos < 0)
		return -EINVAL;
	file->f_pos = newpos;
	TP_LOG_INFO("hideep_iface_llseek end\n");
...
static long hideep_iface_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	TP_LOG_INFO("hideep_iface_ioctl start\n");

	if(!file->private_data){
		TP_LOG_ERR("file->private_data is NULL\n");
		goto hideep_iface_ioctl_null;
	}
	TP_LOG_INFO("cmd = 0x%8x,IST510E_CFG = 0x%8x,IST510E_RDIM = 0x%8x\n",\
	...
	switch(cmd){
		case IST510E_CFG:
			...
			TP_LOG_INFO("ISTCORE_IFACE_CFG : im_size = %d,vr_size = %d\n", \
				drv_info->im_size,drv_info->vr_size);
			break;
		case IST510E_RDIM:
			TP_LOG_INFO("IST510E_RDIM\n");
	...
	TP_LOG_INFO("hideep_iface_ioctl end\n");
...
hideep_iface_ioctl_notty:
	TP_LOG_ERR("notty not match magic\n");
	return -ENOTTY;
hideep_iface_ioctl_notty2:
	TP_LOG_ERR("notty over max\n");
	return -ENOTTY;
...
static void hideep_iface_unregister(struct ist510e *ts)
{
	...
	TP_LOG_INFO("hideep_iface_unregister start\n");
...
static int hideep_iface_register(struct ist510e *ts, u32 minor)
{
	...
	TP_LOG_INFO("hideep_iface_register start\n");
	...
	TP_LOG_INFO("hideep_iface_register end\n");

	return 0;
err:
	TP_LOG_ERR("ist510e_debug_register failed\n");
	return err;
}
...
int hideep_iface_init(struct ist510e *ts)
{
	...
	TP_LOG_INFO("istxxxx_iface_init done\n");
