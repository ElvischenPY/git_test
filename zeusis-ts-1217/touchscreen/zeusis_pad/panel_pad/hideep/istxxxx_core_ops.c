...
{
	struct ts_device_data *dat = g_istxxxx_data->zeusis_ts_data;
	struct platform_device *platform_dev = g_istxxxx_data->zeusis_ts_dev;
...
static void hideep_regulator_enable(void)
{
	...
	TP_LOG_INFO("hideep_regulator_enable is called\n");
	if (1 == dat->vci_regulator_type) {
		if (!IS_ERR(g_istxxxx_data->tp_vci)) {
...
static void hideep_power_on(void)
{
	...
	TP_LOG_INFO("hideep_power_on called\n");
...
static int hideep_gpio_request(void)
{
	...
	int retval = NO_ERR;
	TP_LOG_INFO("hideep_gpio_request\n");
...
int zeusis_ts_i2c_read(u8 *reg_addr, u16 reg_len, u8 *buf, u16 len)
{
	int count = 0;
	int ret;
	struct i2c_msg xfer[2];

	TP_LOG_DEBUG("ts_i2c_read enter\n");
	/*register addr*/
	...
	count = 0;
	do {
		ret = i2c_transfer(g_tp_data.client->adapter, &xfer[0], 1);
		if (ret == 1){
			goto reading_data;
		}
...
reading_data:
	count = 0;
	do {
		ret = i2c_transfer(g_tp_data.client->adapter, &xfer[1], 1);
		if (ret == 1) {
			return NO_ERR;
		}
do_retry:
...
int hideep_i2c_read(struct ist510e *ts, u16 addr, u16 len, u8 *buf)
{
	...
	TP_LOG_DEBUG("hideep_i2c_read:addr = 0x%04x,len = %d\n",addr,len);
	mutex_lock(&ts->i2c_mutex);
#ifdef HIDEEP_REPEAT_START
	ret = zeusis_ts_i2c_read((u8*)&addr, 2, buf, len);
#else
	ret = ts->zeusis_ts_data->bops->bus_read((u8*)&addr, 2, buf, len);
#endif
	if(ret < 0)
	    goto i2c_err;
...
i2c_err:
	mutex_unlock(&ts->i2c_mutex);
	TP_LOG_ERR("i2c read error, ret = %d\n",ret);
	return -1;
}
...
int hideep_i2c_write(struct ist510e *ts, u16 addr, u16 len, u8 *buf)
{
	...
	TP_LOG_DEBUG("hideep_i2c_write:addr = 0x%04x,len = %d\n",addr,len);
	if(g_tp_log_cfg>=1){
		for(i=0;i<len;i++){
			if(i%16==0){
				if(i!=0)
	...
	ret = ts->zeusis_ts_data->bops->bus_write(ts->seg_buff, 2+len);
	if(ret < 0){
		TP_LOG_ERR("i2c write address error\n");
		goto i2c_err;
	}
	mutex_unlock(&ts->i2c_mutex);
	return  0;
...
i2c_err:
	mutex_unlock(&ts->i2c_mutex);
	TP_LOG_ERR("i2c write error, ret = %d\n",ret);
	return -1;
}
...
static s32 hideep_get_event(struct ist510e *ts)
{
	...
	TP_LOG_DEBUG("hideep_get_event enter\n");
	...
	return touch_count;
i2c_err:
	TP_LOG_ERR("hideep_get_event i2c err\n");
	return -1;
}
...
static s32 hideep_get_image(struct ist510e *ts)
{
	...
	TP_LOG_DEBUG("hideep_get_image enter\n");
	ret = hideep_i2c_read(ts, VR_ADDR_IMAGE, debug_dev->im_size, debug_dev->im_buff);
	if(ret < 0)
		goto i2c_err;
	TP_LOG_DEBUG( "load image from sensor(%d)\n", debug_dev->im_size);
...
static int hideep_get_version(struct ist510e *ts)
{
	...
	if(ret < 0)
		goto i2c_err;
	TP_LOG_INFO("vr version : %04x\n", dmz_info->pannel.version);

	return 0;
i2c_err:
	TP_LOG_ERR("hideep_get_version error\n");
	return ret;
}
...
static int hideep_regulator_vci_enable(struct ist510e *ts_drv)
{
	...
	if(ret < 0) {
		TP_LOG_ERR("tp_vci voltage error\n");
		return -EINVAL;
	}
	ret = regulator_enable(ts_drv->tp_vci);
	if (ret < 0) {
		TP_LOG_ERR("tp_vci enable  error\n");
		return -EINVAL;
	}
...
void hideep_reset_ic(void)
{
	...
	struct ts_device_data *dat  = ts_drv->zeusis_ts_data;
	TP_LOG_INFO("hideep_reset_ic enter \n");
	gpio_direction_output(dat->reset_gpio, 1);
	mdelay(2);
	gpio_direction_output(dat->reset_gpio, 0);
	mdelay(5);
	gpio_direction_output(dat->reset_gpio, 1);
	TP_LOG_INFO("hideep_reset_ic finish \n");
	return ;
}

static void hideep_dump_config(struct ts_device_data *data)
{
	TP_LOG_INFO("irq_gpio             : %d\n",data->irq_gpio);
	TP_LOG_INFO("irq_config           : %d\n",data->irq_config);
	TP_LOG_INFO("reset_gpio           : %d\n",data->reset_gpio);
	TP_LOG_INFO("algo_id              : %d\n",data->algo_id);
	TP_LOG_INFO("tp_func_flag        : %d\n",data->tp_func_flag);
	TP_LOG_INFO("vci_gpio_ctrl        : %d\n",data->vci_gpio_ctrl);
	TP_LOG_INFO("vddio_gpio_ctrl      : %d\n",data->vddio_gpio_ctrl);
	TP_LOG_INFO("vci_regulator_type   : %d\n",data->vci_regulator_type);
	TP_LOG_INFO("vddio_regulator_type : %d\n",data->vddio_regulator_type);
	TP_LOG_INFO("x_max                : %d\n",data->x_max);
	TP_LOG_INFO("y_max                : %d\n",data->y_max);
	TP_LOG_INFO("x_max_mt             : %d\n",data->x_max_mt);
	TP_LOG_INFO("y_max_mt             : %d\n",data->y_max_mt);
	TP_LOG_INFO("ic_type              : %d\n",data->ic_type);
	return;
}
...
static int hideep_parse_config(struct device_node *device, struct ts_device_data *data)
{
	...
	TP_LOG_INFO("hideep_parse_config enter\n");
	...
	if (1 == data->unite_cap_test_interface) {
		...
		for(index = 0; index < array_len; index++){
			...
			if (ret) {
				TP_LOG_ERR("read index = %d,raw_data_limit = %s,ret = %d error,\n", index, raw_data_dts, ret);
				ret = 0;
				//goto err;
			}
	...
	TP_LOG_INFO(" hideep_parse_config OK.\n");

	return NO_ERR;
err:
	TP_LOG_ERR("hideep_parse_config failed\n");
	return -EINVAL;;
}
...
static int hideep_input_config(struct input_dev *input_dev)
{
	...
	TP_LOG_INFO("hideep_input_config enter\n");
	ts_drv->input_dev = input_dev;
	hideep_capability(ts_drv->input_dev, ts_drv->zeusis_ts_data);
...
static int hideep_chip_detect(struct device_node *device, struct ts_device_data *ts_data, struct platform_device *ts_dev)
{
	...
	TP_LOG_INFO("hideep_chip_detect enter\n");

	g_istxxxx_data = kzalloc(sizeof(struct ist510e), GFP_KERNEL);
	if(g_istxxxx_data == NULL) {
		TP_LOG_ERR("g_istxxxx_data alloc error\n");
		goto err;
	}

	g_istxxxx_data->zeusis_ts_data = ts_data;
	g_istxxxx_data->zeusis_ts_dev = ts_dev ;
	g_istxxxx_data->zeusis_ts_dev->dev.of_node = device;                //for dsti reference
	...
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.sleep_mode = TS_POWER_OFF_MODE;
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.easy_wakeup_gesture = false;
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.easy_wakeup_flag = false;
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.palm_cover_flag = false;
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.palm_cover_control = false;
	g_istxxxx_data->zeusis_ts_data->easy_wakeup_info.off_motion_on = false;
	...
	if (ret < 0) {
		TP_LOG_ERR("regulator error\n");
		goto err_rgt;
	}
	ret = hideep_gpio_request();
	if (ret < 0) {
		TP_LOG_ERR("gpio get  error\n");
		goto err_gpio;
	}
	...
	if(ret < 0) {
		TP_LOG_ERR("no ic is detected\n");
		goto err_aux;
	}
#endif
	TP_LOG_INFO("detect ok.\n");
...
{
	struct ist510e *ts_drv =  g_istxxxx_data;
	s32 ic_type  =  ts_drv->zeusis_ts_data->ic_type;
	int ret = 0;

	TP_LOG_INFO("hideep_init enter\n");
	switch(ic_type)
	{
		case IST_GREEN:
			break;
		case IST_CRIMSON:
			...
			if(ret < 0) {
				TP_LOG_ERR("debug interface device error\n");
				goto err_deivce;
			}
/*
...
static int hideep_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	out_cmd->cmd_param.pub_params.algo_param.algo_order = ts->zeusis_ts_data->algo_id;
	...
	gesture_info = &g_istxxxx_data->zeusis_ts_data->easy_wakeup_info;
	info->cur_finger_number = ts->input_touch_nr;
	TP_LOG_DEBUG("apple_weight_init = %d, input_event_nr = %d",ts->apple_weight_init, ts->input_event_nr);
			...
			case HIDEEP_GES_CHAR:
				...
				switch(gesture[1]){
					...
					case HIDEEP_CHAR_W:
						...
						break;
					default:
						TP_LOG_ERR("incorrect character.\n");
						break;
				}
				TP_LOG_DEBUG("character wake up.\n");
				break;
			default:
				TP_LOG_ERR("incorrect ID.\n");
				break;
			}
		}
		TP_LOG_DEBUG("info->gesture_wakeup_value = %d\n",info->gesture_wakeup_value);
	...
	for(i = 0; i < info->cur_finger_number; i++){
		...
		TP_LOG_DEBUG("%s id = %d, flag = 0x%02x\n",__FUNCTION__,id, finger[i].flag);
	...
	TP_LOG_DEBUG("temp_status = 0x%x, pre_status = 0x%x, eor = 0x%x, result = 0x%x", temp_status, pre_status, temp_status^pre_status,(temp_status^pre_status)&temp_status);
	if((temp_status!=pre_status) && ((temp_status^pre_status)&temp_status))
	{
		TP_LOG_DEBUG("%s ROI readying\n",__FUNCTION__);
	...
	if(ts->input_key_nr){
		...
		case 5:
			...
			break;
		default:
			TP_LOG_ERR("the get_button_number is invald\n");
			break;
        }
	}
...
static int hideep_before_suspend(void)
{
	...
	if(ts->apple_weight_init){
		TP_LOG_DEBUG("still in apple weight mode\n");
...
static int hideep_suspend(void)
{
	...
	switch (gesture_info->sleep_mode) {
		...
		case TS_GESTURE_MODE:
			TP_LOG_INFO("enter gesture mode.\n");
			...
			break;
		default:
			TP_LOG_ERR("no suspend.\n");
			return -EINVAL;
	}
	TP_LOG_INFO("suspend end.\n");
ret:
	return NO_ERR;
}
...
static int hideep_resume(void)
{
	...
	switch (gesture_info->sleep_mode) {
		...
		case TS_GESTURE_MODE:
			...
			break;
		default:
			TP_LOG_ERR("no resume.\n");
			return -EINVAL;
	}
	TP_LOG_INFO("resume end.\n");
...
static int hideep_fw_update_sd(void)
{
	...
	if(ret < 0){
		TP_LOG_ERR("fw update failed\n");
		goto err;
	}
	TP_LOG_INFO("fw update success\n");
...
static int hideep_fw_update_boot(char *file_name)
{
	...
	struct ist510e *ts_drv = g_istxxxx_data;
	struct device *dev = &ts_drv->zeusis_ts_dev->dev;
	...
	TP_LOG_INFO("hideep_fw_update_boot enter, firmware name is %s\n", firmware_name);
	ret = request_firmware(&fw_entry, firmware_name, dev);
	if(ret != 0){
		TP_LOG_ERR("request_firmware(%s) failed\n", firmware_name);
		goto err;
	}
	release_firmware(fw_entry);
	ret = hideep_i2c_read(ts_drv, VR_VERSION_ADDR, 2, (u8*)&version);
	if(ret < 0){
		TP_LOG_ERR("read i2c fail.\n");
		goto err;
	}
	TP_LOG_INFO("vr version : %04x\n", version);
...
static int hideep_get_info(struct ts_chip_info_param *info)
{
	...
	gesture_info = &g_istxxxx_data->zeusis_ts_data->easy_wakeup_info;
	TP_LOG_INFO("hideep_get_info enter, ts->dev_state = %d\n", ts->dev_state);
	if((ISTCORE_PWR_NORMAL != ts->dev_state)&&
		(TS_GESTURE_MODE != gesture_info->sleep_mode)){
		TP_LOG_ERR("hideep_get_rawdata is not in normal mode \n");
		return -1;
	}
	if((TS_GESTURE_MODE == gesture_info->sleep_mode)&&
...
int hideep_get_image_page(struct ts_rawdata_info *info,u16 command,u16 offset)
{
	...
	TP_LOG_INFO("hideep_get_image_page enter\n");
	dev->i_rdy = 0;
	dev->vr_addr = 0x0;
	dev->vr_size = 0x1;
	vr_buff = command;
	...
	for(retry = 0;retry <5;retry ++){
		...
		do{
			...
			mdelay(10);
			time_out++;
			TP_LOG_DEBUG("time count %d, dev->im_buff[0] = 0x%02x\n",time_out,dev->im_buff[0]);
			if(time_out>10000)
	...
	if (dev->im_buff)
		kfree(dev->im_buff);
	TP_LOG_INFO("hideep_get_image_page end\n");
	return 0;
hideep_get_image_page_timeout_err:
	if (dev->im_buff)
		kfree(dev->im_buff);
hideep_get_image_page_command_err:
	TP_LOG_ERR("hideep_get_image_page error\n");
	return -1;
}
...
int hideep_get_aulura_image_page(struct ts_rawdata_info *info,u16 command,u16 offset)
{
	...
	TP_LOG_INFO("hideep_get_aulura_image_page enter cmd = 0x%04x\n",command);
	...
	if (dev->im_buff)
		kfree(dev->im_buff);
	TP_LOG_INFO("hideep_get_aulura_image_page end.\n");
...
hideep_get_aulura_image_page_command_err:
	TP_LOG_ERR("hideep_get_aulura_image_page error\n");
	return -1;
}
...
static void hideep_rawdata_image_data_test(struct ts_rawdata_info *info)
{
	...
	TP_LOG_INFO("hideep_rawdata_image_data_test\n");
	...
	for(i = 0; i < tx*(rx-1); i++) {
		if((temp_buffer[i] < rawdata_max[i]) && (temp_buffer[i] > rawdata_min[i]))
			...
			TP_LOG_INFO("over limit, buffer[%d]:%d\n", i, temp_buffer[i]);
	}

	TP_LOG_INFO("result_count = %d\n", result_count);
...
static void hideep_rawdata_image_z_test(struct ts_rawdata_info *info)
{
	...
	TP_LOG_INFO("hideep_rawdata_image_z_test\n");
	...
	for(i = 0; i < tx; i++) {
		if((temp_z_buffer[i] < rawdata_z_max[i]) && (temp_z_buffer[i] > rawdata_z_min[i]))
			...
			TP_LOG_INFO("over limit, z_buffer[%d]:%d\n", i, temp_z_buffer[i]);
	}
...
static void hideep_delta_image_data_test(struct ts_rawdata_info *info)
{
	...
	TP_LOG_INFO("hideep_delta_image_data_test\n");
...
static void hideep_delta_image_z_test(struct ts_rawdata_info *info)
{
	...
	TP_LOG_INFO("hideep_delta_image_z_test\n");
...
static int hideep_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	gesture_info = &g_istxxxx_data->zeusis_ts_data->easy_wakeup_info;
	TP_LOG_INFO("hideep_get_rawdata enter, ts->dev_state = %d\n", ts->dev_state);
	if((ISTCORE_PWR_NORMAL != ts->dev_state)&&
		(TS_GESTURE_MODE != gesture_info->sleep_mode)){
		TP_LOG_ERR("hideep_get_rawdata is not in normal mode \n");
		return -1;
	...
	dev->im_r_en = 1;
	if(dev->im_buff == NULL) {
		TP_LOG_INFO("hideep_get_rawdata im_buff is NULL\n");
...
int hideep_get_image_by_cmd(u16 command)
{
	...
	TP_LOG_DEBUG("hideep_get_image_by_cmd enter\n");
...
hideep_get_image_timeout_err:
	TP_LOG_ERR("get image time out\n");
	return -1;
hideep_get_image_command_err:
	TP_LOG_ERR("send command error\n");
	return -1;
}
...
static int hideep_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	struct ist510e *ts   = g_istxxxx_data;
	struct ist510e_debug_dev *dev = &ts->debug_dev;
	struct ts_device_data *data= ts->zeusis_ts_data;
	DWZ_INFO_T  *dmz_info = &ts->dwz_info;
	u8 *str_buf;
	u8 *str_mms_buf;
	int min,max,sum,i,j;
	...
	*/
	...
	TP_LOG_DEBUG("hideep_get_rawdata t = %d, r = %d\n",info->buff[1],info->buff[0]);
	mutex_lock(&ts->dev_mutex);
	dev->im_r_en = 1;

	TP_LOG_INFO("x,y rawdata enter\n");
	//reading x,y rawdata....
	ret = hideep_get_image_by_cmd(OPM_DEMOD);
	if(ret != 0) {
		TP_LOG_ERR("read image fail. ret = 0x08x\n",ret);
	...
	//checking z rawdata.
	TP_LOG_INFO("checking z rawdata enter\n");
...
static int hideep_holster_switch(struct ts_holster_info *info)
{
	...
	TP_LOG_INFO("switch = %d, Holster window_info is (x0=%d,y0=%d)(x1=%d,y1=%d)\n",\
		info->holster_switch,x0,y0,x1,y1);
	switch(info->op_action){
		case TS_ACTION_WRITE:
			if(!info->holster_switch){
				TP_LOG_INFO("disable holster mode.\n");
				cmd_buf[0] = 0x00;
				ret = hideep_i2c_write(ts, VR_HOLSTER, 1, cmd_buf);
				if(ret < 0){
					TP_LOG_ERR("i2c write error.\n");
					break;
				}
			}else{
...
static void apple_weight_timeout_func(struct work_struct *work)
{
	u8 cmd;
	struct ist510e *ts   = g_istxxxx_data;
	TP_LOG_DEBUG("apple_weight_timeout_func enter\n");
	do
	{
		if(!g_istxxxx_data->apple_weight_init){
			TP_LOG_DEBUG("apple_weight_timeout_func thread finish\n");
			break;
		}
		mdelay(100);
		TP_LOG_DEBUG("apple_weight_timeout_func time out = %d", g_istxxxx_data->apple_weight_timeout);
	...
	TP_weigh_mode = 0;
	TP_LOG_DEBUG("apple_weight_timeout_func end\n");
	return 0;
}
static int hideep_single_touch_switch(struct ts_single_touch_info *info)
{
	...
	if(TS_ACTION_WRITE == info->op_action){
		TP_LOG_DEBUG("hideep_single_touch_switch single_touch_switch = %d\n", info->single_touch_switch);
		switch(info->single_touch_switch){
		case 0:
			TP_LOG_DEBUG("apple weight STOP\n");
		...
		case 1:
			if(!g_istxxxx_data->apple_weight_init){
				TP_LOG_DEBUG("apple weight INIT\n");
				...
				TP_LOG_DEBUG("apple weight INIT2\n");
				g_istxxxx_data->apple_weight_timeout = 0;
			}
			hideep_item_switch(info->op_action, VR_SINGLE_TOUCH, 1, &info->single_touch_switch);
			break;
		case 2:
			TP_LOG_DEBUG("apple weight RESET\n");
			g_istxxxx_data->apple_weight_timeout=0;
			break;
		default:
			TP_LOG_ERR("error 0x0818 msg\n");
			break;
		}
	}else if(TS_ACTION_READ == info->op_action){
		TP_LOG_DEBUG("hideep_single_touch_switch read");
		if(g_istxxxx_data->apple_weight_flag){
			TP_LOG_DEBUG("apple weight read %d\n", g_istxxxx_data->apple_weight_buff);
			g_istxxxx_data->apple_weight_flag = false;
...
static int hideep_glove_switch(struct ts_glove_info *info)
{
	...
	TP_LOG_INFO("enter, action = %d, switch = %d\n",info->op_action , info->glove_switch);
...
static int hideep_palm_switch(struct ts_palm_info *info)
{
	...
	TP_LOG_INFO("hideep_palm_switch enter\n");
...
static int hideep_roi_switch(struct ts_roi_info *info)
{
	...
	if(!g_istxxxx_data){
		TP_LOG_ERR("%s: g_istxxxx_data is NULL\n",__FUNCTION__);
		ret = -ENOMEM;
		return ret;
	}
	switch (info->op_action) {
	case TS_ACTION_WRITE:
		TP_LOG_INFO("%s: write swtich = %d\n",__FUNCTION__,info->roi_switch );
