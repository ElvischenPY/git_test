...
{
	int rc = 0;
	TP_LOG_INFO("cypress_reg_algo called\n");

	rc = pad_register_algo_func(chip_data, &cypress_algo_f1);		//put algo_f1 into list contained in chip_data, named algo_t1
	if(rc)
		goto out;
out:
	return rc;
...
static inline void debug_print_i2c_transfer(bool action, u8 addr, u8 *buf, int size)
{
	...
	if(action)
		TP_LOG_DEBUG("[cypress debug] i2c action write addr:0x%x, size:%d\n", addr, size);
	else
		TP_LOG_DEBUG("[cypress debug] i2c action read addr:0x%x, size:%d\n", addr, size);

	for(;index < size; index++)
		TP_LOG_DEBUG("[cypress debug] buf value :0x%x\n", *(buf+index));
}
...
static int cypress_i2c_write(u8 addr, u8 *buf, u16 length)
{
	...
	if(rc < 0)
		TP_LOG_ERR("failed to write, rc=%d\n", rc);
...
static int cypress_handshake(u8 mode)
{
	...
	TP_LOG_DEBUG("mode[0]: 0x%x\n", mode);

	if (mode & CY_HST_MODE_CHANGE) {
		TP_LOG_ERR("Host mode change bit set, NO handshake. mode = 0x%x\n", mode);
		return NO_ERR;
	}
	rc = cypress_i2c_write(reg_addr, &cmd, sizeof(cmd));
	if (rc < 0)
		TP_LOG_ERR("bus write fail on handshake, rc = %d\n", rc);
...
static int check_hst_mode(u8 hst_mode)
{
	...
	TP_LOG_DEBUG("make sure wheather in mode: 0x%x\n", hst_mode);

	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), mode, sizeof(mode));
	if (rc < 0) {
		TP_LOG_ERR("Failed to read mode from 0x00, rc = %d\n", rc);
		return rc;
	}
	TP_LOG_DEBUG("mode[0-2]: 0x%02X 0x%02X 0x%02X, current mode: 0x%02X\n", mode[0], mode[1], mode[2], cur_mode);

	cur_mode = mode[0] & CY_HST_MODE;
	switch (cur_mode) {
		case CY_HST_OPERATE:								//operation mode  000
			TP_LOG_DEBUG("cur_mode:0x%x, operational mode\n", cur_mode);
			break;
		case CY_HST_CAT:									//configuration and test mode	010
			TP_LOG_DEBUG("cur_mode:0x%x, CaT mode\n", cur_mode);
			break;
		case TP_LOG_DEBUG:
			TP_LOG_DEBUG("cur_mode:0x%x, sysinfo mode\n", cur_mode);					//sysinfo mode  001
			break;
		default:
			TP_LOG_DEBUG("Unkown mode:0x%x, mode[0] = 0x%02X\n", cur_mode, mode[0]);
			break;
	}
	...
	if (cur_mode == hst_mode) {
		TP_LOG_DEBUG("OK, its correct mode\n");
	} else {
		TP_LOG_DEBUG("Sorry, its uncorrect mode, cur_mode = %d\n", cur_mode);
		return -EINVAL;
	}

	TP_LOG_DEBUG("cmd_ofs = %d, mode[cmd_ofs] = 0x%x\n", cmd_ofs, mode[cmd_ofs]);
	if (mode[cmd_ofs] & CY_CMD_COMPLETE) {
		TP_LOG_DEBUG("Received command complete\n");
		core_data.complete_cmd = true;
	} else {
		TP_LOG_DEBUG("command is not complete\n");
		core_data.complete_cmd = false;
	}

	rc = cypress_handshake(mode[0]);
	if (rc < 0)
		TP_LOG_DEBUG("Fail handshake mode[0] = 0x%X, rc = %d\n", mode[0], rc);
...
static int cypress_power_on_reset(void)
{
	...
	if(rc < 0){
		TP_LOG_ERR("failed to set cypress vdd to 2.8V, rc = %d\n", rc);
		return  -EINVAL;
	}

	rc = regulator_enable(core_data.vdd);
	if (rc < 0) {
		TP_LOG_ERR("failed to enable cypress vdd(2.8V), rc = %d\n", rc);
		return -EINVAL;
	}

	rc = regulator_enable(core_data.vbus);
	if (rc < 0) {
		TP_LOG_ERR("vbus enable error:%d \n", rc);
		return -EINVAL;
	}
	msleep(5);
	return rc;
...
static int cypress_soft_reset(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("FAILED to execute SOFT reset\n");
		return rc;
	}
	TP_LOG_INFO("execute SOFT reset\n");
	return 0;
}
...
static int cypress_checkout_bl(void)
{
	...
	**so, read 3 timer per 200ms, to make sure it is in bootloader mode, in which mode[0] is 0x01 */
	for (i = 0; i < I2C_RW_TRIES; i++) {
		rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), mode, sizeof(mode));
		if (rc < 0) {
			TP_LOG_ERR("Failed to read register 0x00, i = %d, rc = %d\n", i, rc);
		} else {
			TP_LOG_DEBUG("mode[0-2]:0x%X 0x%X 0x%X\n", mode[0], mode[1], mode[2]);
			if (IS_BOOTLOADER(mode[0])) {
				TP_LOG_INFO("In bootloader mode\n");
				return NO_ERR;
			} else {
				TP_LOG_ERR("NOT in bootloader mode!mode[0] = %d, i = %d\n", mode[0], i);
				cypress_soft_reset();
				rc = -EIO;
			}
		}
...
static int cypress_suspend()
{
	...
	if (core_data.process_state == CY_STATE_FW_UPDATE) {
		TP_LOG_INFO("not suspend during updating the fw\n");
		goto out;
	}

	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), &mode, sizeof(mode));
	if (rc) {
		TP_LOG_ERR("Fail read mode r=%d\n", rc);
		goto out;
	}
	mode |= CY_HST_SLEEP;
	rc = cypress_i2c_write(reg_addr, &mode, sizeof(mode));
	if (rc) {
		TP_LOG_ERR("Fail write mode r=%d\n", rc);
		goto out;
	}
	TP_LOG_INFO("write DEEP SLEEP succeeded\n");
...
static int cypress_get_glove_switch(u8 *glove_switch)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("exec cmd error\n");
		goto out;
	}

	TP_LOG_INFO("return_buf=0x%x,0x%x,0x%x\n", read_return_buf[0], read_return_buf[1], read_return_buf[2]);
	if (read_return_buf[0] != CY_OP_PARA_SCAN_TYPE) {
		TP_LOG_ERR("return data error: read_return_buf[0] = %d\n", read_return_buf[0]);
		rc= -EINVAL;
		goto out;
	}
...
static int cypress_set_glove_switch(u8 glove_switch)
{
	...
	switch (glove_switch) {
		...
		case CY_SIGNAL_DISPARITY_STYLUS:
			...
			break;
		default:
			TP_LOG_ERR("Invalid signal status=%d\n", 	(int)glove_switch);
			rc = -EINVAL;
			goto out;
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("exec cmd error. %d\n", rc);
		goto out;
	}
	TP_LOG_INFO("return_buf=0x%x,0x%x\n",write_return_buf[0],write_return_buf[1]);
...
static int cypress_glove_switch(struct ts_glove_info *info)
{
	...
	switch (info->op_action){
		case TS_ACTION_WRITE:
			...
			if(rc < 0){
				TP_LOG_ERR("set glove switch(%d), failed : %d", info->glove_switch, rc);
			}
			break;
		case TS_ACTION_READ:
			rc = cypress_get_glove_switch(&info->glove_switch);
			if(rc < 0){
				TP_LOG_ERR("get glove switch(%d), failed : %d", info->glove_switch, rc);
			}
			break;
		default:
...
static int cypress_set_holster_switch(u8 holster_switch)
{
	...
	switch (holster_switch) {
		case CY_HOLSTER_OFF_LOW:
			TP_LOG_DEBUG("to set holster low\n");
			holster_type = CY_OP_PARA_HOLSTER_COM_OFF;
			break;
		case CY_HOLSTER_ON_HIGH:
			TP_LOG_DEBUG("to set holster high\n");
	...
	TP_LOG_DEBUG("return_buf=0x%x,0x%x\n",write_return_buf[0],write_return_buf[1]);
...
static int cypress_holster_switch(struct ts_holster_info *info)
{
	...
	switch (info->op_action) {
		case TS_ACTION_WRITE:
			...
			if (rc < 0) {
				TP_LOG_ERR("set holster switch(%d), failed : %d", info->holster_switch, rc);
			}
			break;
		default:
			TP_LOG_ERR("invalid  holster switch status: %d", info->holster_switch);
			rc = -EINVAL;
			break;
	}
...
static int cypress_hand_detect(struct ts_hand_info *info)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("send execute handness_detection init command fail!\n");
		goto err;
	}
	...
	rc = cypress_exec_cmd(CY_HST_OPERATE,
			...
			return_buf, sizeof(return_buf));
	if (rc < 0) {
		TP_LOG_ERR("send execute handness_detection command fail!\n");
		goto err;
	}
...
static int cypress_resume()
{
	...
	/*not suspend during updating the fw*/
	if (core_data.process_state == CY_STATE_FW_UPDATE) {
		TP_LOG_INFO("not resume during updating the fw\n");
		goto out;
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("Fail set output irq gpio=%d\n", core_data.cypress_chip_data->irq_gpio);
		goto out;
	} else {
		udelay(2000);
		rc = gpio_direction_input(core_data.cypress_chip_data->irq_gpio);
		if (rc < 0) {
			TP_LOG_ERR("Fail set input irq gpio=%d\n", core_data.cypress_chip_data->irq_gpio);
			goto out;
		}
		TP_LOG_INFO("HW power up succeeds, gpio=%d\n", core_data.cypress_chip_data->irq_gpio);
	}
	...
	for (i=0; i < I2C_RW_TRIES-1; i++) {
		...
		if (rc < 0) {
			TP_LOG_ERR("goto CY_HST_OPERATE failed[%d], check again\n", i);
		} else {
			break;
		}
	}
...
static int cypress_sysinfo_offsets(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("failed to read HST_MODE register, rc = %d\n", rc);
		return rc;
	}
	...
	TP_LOG_DEBUG("cydata_ofs:0x%4x, test_ofs:0x%4x, pcfg_ofs:0x%x, opcfg_ofs:0x%x, ddata_ofs:0x%x, mdata_ofs:0x%x\n",core_data.sysinfo.sysinfo_merged_data.cydata_ofs,
...
static int cypress_sysinfo_test(void)
{
	...
	if (test == NULL) {
		TP_LOG_ERR("Failed to alloc test memory, test_size = %d\n", test_size);
		rc = -ENOMEM;
		goto out;
	}
...
static int cypress_sysinfo_cydata(void)
{
	...
	TP_LOG_DEBUG("test_ofs = 0x%x, cydata_ofs = 0x%x\n", core_data.sysinfo.sysinfo_merged_data.test_ofs,
				core_data.sysinfo.sysinfo_merged_data.cydata_ofs);
	...
	if (buf == NULL) {
		TP_LOG_ERR("Failed to alloc cydata buffer(null), cydata_size = %d\n", cydata_size);
		rc =  -ENOMEM;
		goto out;
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("Failed to read cydata, rc = %d\n", rc);
		goto free_buf;
	}
...
static int cypress_sysinfo_opcfg(void)
{
	...
	TP_LOG_DEBUG("ddata_ofs = 0x%x, opcfg_ofs = 0x%x\n",
	...
	if (opcfg == NULL) {
		TP_LOG_ERR("Failed to alloc opcfg memory, opcfg_size = %d\n", opcfg_size);
		rc = -ENOMEM;
		goto out;
	}
...
{
	int rc = 0;
	TP_LOG_DEBUG("xy_mode_size = %d, xy_data_size = %d\n", core_data.sysinfo.sysinfo_merged_data.mode_size,
			core_data.sysinfo.sysinfo_merged_data.data_size);
	core_data.sysinfo.xy_mode = kzalloc(core_data.sysinfo.sysinfo_merged_data.mode_size, GFP_KERNEL);
       if (core_data.sysinfo.xy_mode == NULL){
	   	TP_LOG_ERR("Failed to alloc memory for xy_mode, size is %d\n", core_data.sysinfo.sysinfo_merged_data.mode_size);
		return -1;
       }
       ...
       if (core_data.sysinfo.xy_data == NULL){
	   	TP_LOG_ERR("Failed to alloc memory for xy_data, size is %d\n", core_data.sysinfo.sysinfo_merged_data.data_size);
		rc = -1;
	   	goto err;
       }
	TP_LOG_DEBUG("xy_mode=%p xy_data=%p\n", core_data.sysinfo.xy_mode, core_data.sysinfo.xy_data);
...
static int cypress_switch_to_operate(void)
{
	...
	for (i = 0; i <  I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			TP_LOG_ERR("failed to set mode to Operational rc = %d\n", rc);
			continue;
		}
		msleep(2);
		rc = check_hst_mode(CY_HST_OPERATE);
		if (rc < 0) {
			TP_LOG_ERR("check operate failed, try again, rc = %d, i = %d\n", rc, i);
		} else {
			TP_LOG_INFO("operate mode\n");
			return NO_ERR;
		}
	}
...
static int cypress_get_sysinfo_regs(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Failed to get sysinfo offsets, rc = %d\n", rc);
		return rc;
	}
	rc = cypress_sysinfo_cydata();			//some version information
	if (rc < 0){
		TP_LOG_ERR("Failed to get sysinfo cydata, rc = %d\n", rc);
		return rc;
	}
	rc = cypress_sysinfo_test();
	if(rc < 0){
		TP_LOG_ERR("Failed to get sysinfo test, rc = %d\n", rc);
		return rc;
	}
	rc = cypress_sysinfo_pcfg();
	if (rc < 0){
		TP_LOG_ERR("Failed to get sysinfo pcfg, rc = %d\n", rc);
		return rc;
	}
	/*operational mode offset*/
	rc = cypress_sysinfo_opcfg();
	if (rc < 0) {
		TP_LOG_ERR("Failed to get sysinfo opcfg, rc = %d\n", rc);
		return rc;
	}
       rc = cypress_sysinfo_memory_alloc();
       if (rc < 0) {
               TP_LOG_ERR("Failed to alloc memory, rc = %d\n", rc);
               return rc;
       }
	/* provide flow control handshake */
	rc = cypress_handshake(core_data.sysinfo.sysinfo_data.hst_mode);
	if (rc < 0)
		TP_LOG_ERR("handshake fail on sysinfo reg, rc = %d\n", rc);
...
static int cypress_read_config_block(u8 ebid, u16 row, u8 *data, u16 length)
{
	...
	if (rc) {
		TP_LOG_ERR("Error executing command r=%d\n", rc);
		goto free_buffer;
	}
	...
	if (return_buf[0] != CY_CMD_STATUS_SUCCESS
		...
		TP_LOG_ERR("Fail executing command\n");
		rc = -1;
		goto free_buffer;
	}
...
static int cypress_get_ttconfig_version(u16 *version)
{
	...
	if (rc) {
		TP_LOG_ERR("Error on read config block\n");
		goto exit;
	}
...
static int cypress_get_config_block_crc(u8 ebid, u16 *crc)
{
	...
	/* Validate response */
	if (return_buf[0] != CY_CMD_STATUS_SUCCESS) {
		TP_LOG_ERR("Fail executing command, return_buf[0] = %d\n", return_buf[0]);
		return -1;;
	}
...
static int cypress_switch_to_cat(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			TP_LOG_ERR("Failed to set mode to cat rc = %d\n", rc);
			continue;
		}
		msleep(50);
		rc = check_hst_mode(CY_HST_CAT);
		if (rc < 0) {
			TP_LOG_ERR("wrong in cat mode, try again, rc = %d, i = %d\n", rc, i);
		} else {
			TP_LOG_INFO("OK, successfully in cat mode ...\n");
			return NO_ERR;
		}
	}
...
static int cypress_get_ttconfig_info(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("failed to get ttconfig version rc=%d\n", rc);
		return rc;
	}

	rc = cypress_get_config_length(CY_TCH_PARM_EBID, &length, &max_length);
	if (rc < 0) {
		TP_LOG_ERR("failed to get ttconfig length rc=%d\n",rc);
		return rc;
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("failed to get ttconfig crc = %d, rc = %d\n", crc, rc);
		return rc;
	}
	...
	TP_LOG_DEBUG("TT Config Version:%04X Length:%d Max Length:%d CRC:%04X\n",
...
static int cypress_exec_cmd(u8 mode, u8 *cmd_buf, size_t cmd_size, u8 *return_buf, size_t return_buf_size)
{
	...
	TP_LOG_DEBUG("mode = 0x%x\n", mode);
	for (i = 0; i < cmd_size; i++) {
		TP_LOG_DEBUG("cmd_buf[%d] = %d\n", i, cmd_buf[i]);
	}
	...
	switch (mode) {
		...
		case CY_HST_OPERATE:
			...
			break;
		default:
			TP_LOG_ERR("Unsupported mode 0x%x for exec cmd\n", mode);
			return -EACCES;
	}
	...
	for(i = 0; i < I2C_RW_TRIES + 2; i++){
		...
		if (rc < 0) {
			TP_LOG_ERR("Failed read complete bit rc = %d\n",rc);
			return rc;
		}
		if ((command & CY_CMD_COMPLETE_MASK) == 0) {
			TP_LOG_ERR("some command is not completed yet, i= %d\n", i);
			msleep(1000);
		} else {
			TP_LOG_INFO("command is completed, i =%d\n", i);
			break;
		}
	}
	if (i == I2C_RW_TRIES + 2) {
		TP_LOG_ERR("check cmd failed\n");
		return -EINVAL;
	}
	/*
	 * Write new command
	...
	if (rc < 0) {
		TP_LOG_ERR("Error on write command rc = %d\n", rc);
		return rc;
	}
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		...
		if (rc < 0) {
			...
			if (!core_data.complete_cmd) {
				TP_LOG_ERR("command is not completed, complete_cmd = %d, i =%d\n", core_data.complete_cmd, i);
				msleep(2000);
			}else{
				TP_LOG_INFO("everything is ok, i = %d\n", i);
				break;
			}
		}
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("Error on read rc = %d\n", rc);
		return rc;
	}

	for (i = 0; i < return_buf_size; i++) {
		TP_LOG_DEBUG("return_buf[%d] = %d\n", i, return_buf[i]);
	}
	return NO_ERR;
}
...
static int set_hst_mode(u8 new_mode)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Failed to read mode, rc = %d\n", rc);
		return rc;
	}
	...
	if (rc < 0)
		TP_LOG_ERR("Failed to write mode change, rc = %d, mode(value) = 0x%x\n", rc, mode);
...
static int cypress_exit_bl_into_sysinfo(void)
{
	...
	if(rc < 0){
		TP_LOG_ERR("Failed to write ldr_exit to 0x00, rc = %d\n", rc);
		return rc;
	}
	msleep(500);
	...
	if (rc < 0) {
		TP_LOG_ERR("read output packet failed\n");
		return rc;
	}
	if(0 == memcmp(output_packet, ldr_err_app, sizeof(ldr_err_app))) {
		TP_LOG_ERR("output packet equal to ldr_err_app, force update fw\n");
	...
	for (i = 0; i < I2C_RW_TRIES*10; i++) {
		...
		if (rc < 0) {
			TP_LOG_DEBUG("not in sysinfo mode, rc = %d, i = %d. wait 200ms read again\n", rc, i);
			msleep(200);
		} else {
			TP_LOG_INFO("In sysinfo mode\n");
			break;
		}
	}
	if (rc<0)
		TP_LOG_ERR("not in sysinfo mode, rc =%d\n", rc);
	return rc;
}
...
static int cypress_get_status(u8 *buf, int size, unsigned long timeout_ms)
{
	...
	if (timeout_ms != 0) {
		...
		TP_LOG_DEBUG("ries=%d rc =%d status=%02X\n", tries, rc, buf[1]);
	}
	return rc;
}
...
static int cypress_ldr_parse_row(u8 *row_buf, struct cypress_hex_image *row_image)
{
	...
	if (row_image->row_size > ARRAY_SIZE(row_image->row_data)) {
		TP_LOG_ERR("row data buffer overflow\n");
		rc = -1;
		goto out;
	}
...
static int cypress_get_crc_from_params(u16 *crc)
{
	...
	if (!param_regs || !param_regs->data || !param_regs->size) {
		TP_LOG_INFO("missing param regs for crc check.\n");
		return -1;
	}
...
static int cypress_check_ttconfig_crc(void)
{
	...
	TP_LOG_INFO("img_crc:0x%04X new_crc:0x%04X\n", core_data.sysinfo.ttconfig.crc, cfg_crc_new);

	if (cfg_crc_new != core_data.sysinfo.ttconfig.crc) {
		TP_LOG_INFO("Config CRC different, will upgrade\n");
		return 1;
	}

	TP_LOG_INFO("Config CRC equal, will NOT upgrade\n");
	return 0;
}
/*
 * return code:
...
static bool cypress_check_version(const struct firmware *fw)
{
	...
	if (fw == NULL) {
		TP_LOG_ERR("NULL firmware received, update FW\n");
		return true;
	}
	if (fw->data == NULL || fw->size == 0) {
		TP_LOG_ERR("No firmware received, update FW\n");
		return true;
	}

	TP_LOG_INFO("Found firmware\n");
...
static int cypress_initialize_device(void)
{
	...
	for (i = 0; i < try_cnt; i++) {
		...
		rc = cypress_goto_bl();
		if (rc < 0) {
			TP_LOG_ERR("Failed to reset and check bl, rc = %d\n", rc);
			return rc;
		}
		...
		if (!rc) {
			TP_LOG_INFO("cypress device goto SI mode successfully\n");
			break;
		}
	}
	if (i == try_cnt) {
		TP_LOG_ERR("sorry, failed into sysinfo mode! rc = %d\n", rc);
		return rc;
	}
	/* read sysinfo data */
	rc = cypress_get_sysinfo_regs();
	if (rc < 0){
		TP_LOG_ERR("failed to get sysinfo regs rc = %d\n", rc);
		return rc;
	}

	rc = cypress_get_ttconfig_info();
	if(rc < 0){
		TP_LOG_ERR("get ttconfig_info failed rc = %d\n", rc);
		return rc;
	}

	rc = cypress_set_opmode();
	if (rc < 0){
		TP_LOG_ERR("failed to set opmode rc = %d\n", rc);
		return rc;
	}
...
static int cypress_dl_source_init(int fw_size)
{
	...
	if (fw_size % core_data.ld.image_rec_size != 0) {
		TP_LOG_ERR("Firmware image is misaligned\n");
		rc = -EINVAL;
		return rc;
	}
	...
	if (core_data.ld.row_buf == NULL) {
		TP_LOG_ERR("Failed to alloc row buffers(%p)\n", core_data.ld.row_buf);
		rc = -ENOMEM;
		return rc;
	}
...
static int cypress_ldr_enter_result(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Fail get status to Enter Loader command r =%d\n", rc);
		return rc;
	}
	...
	if (status == ERROR_SUCCESS) {
		...
		TP_LOG_ERR("Failed to ldr enter\n");
		rc = -EIO;
	}

	TP_LOG_DEBUG("status=%d, bl_ver=%08X rev_id=%02X silicon_id=%08X\n", status,
...
static int cypress_ldr_enter_command(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("write block failed %d\n", rc);
		return rc;
	}
	TP_LOG_DEBUG("done\n");
	return NO_ERR;
...
static int cypress_ldr_init_command(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Fail writing command=%02X\n", ldr_init_cmd[CY_CMD_BYTE]);
		return rc;
	}
	TP_LOG_DEBUG("done\n");
	return NO_ERR;
			     ...
			     size_t status_size, unsigned long timeout_ms)
{
	...
	if (timeout_ms > 0) {
		...
		if (rc < 0 || status_buf[0] != CY_START_OF_PACKET) {
			TP_LOG_ERR("Error getting status, rc = %d, status_buf[0]=%02X\n", rc, status_buf[0]);
			if (!(rc < 0))
				rc = -EIO;
			return rc;
		} else {
			if (status_buf[CY_STATUS_BYTE] != ERROR_SUCCESS) {
				TP_LOG_ERR("Status=0x%02X error\n", status_buf[CY_STATUS_BYTE]);
				rc = -EIO;
...
static int cypress_load_app(const u8 *fw, int fw_size)
{
	...
	TP_LOG_DEBUG("p=%p, fw=%p, fw_size=%d\n", p, fw, fw_size);
LOOP:
	switch (state) {
	...
	if ((state == FW_UP_SUC) || ((state == FW_UP_FAIL))) {
		TP_LOG_INFO("fw upload exit, state = %d\n", state);
		...
		if (rc < 0) {
			TP_LOG_ERR("Failed to initialize device\n");
		} else {
...
static int cypress_firmware_calibrate(void)
{
	...
	TP_LOG_INFO("Requesting mode change to CAT\n");
	rc = cypress_switch_to_cat();
	if (rc < 0) {
		TP_LOG_ERR("Sorry, Error on request set mode rc = %d\n", rc);
		goto out;
	}

	TP_LOG_INFO("Calibrating for Mutual Capacitance Screen\n");
	...
	if (rc < 0) {
		TP_LOG_ERR("Unable to execute calibrate command.\n");
		goto out;
	}
	if (return_buf[0] != 0) {
		TP_LOG_ERR("sorry, calibrate command unsuccessful\n");
		rc = -EINVAL;
		goto out;
	}

	TP_LOG_INFO("Calibrating for Mutual Capacitance Button\n");
	cmd_buf[1] = 0x01; /* Mutual Capacitance Button */
	rc = cypress_exec_cmd(CY_HST_CAT, cmd_buf, 2, return_buf, 1);
	if (rc < 0) {
		TP_LOG_ERR("Unable to execute Mutual Capacitance Button calibrate command.\n");
		goto out;
	}
	if (return_buf[0] != 0) {
		TP_LOG_ERR("Mutual Capacitance Button calibrate command unsuccessful\n");
		rc = -EINVAL;
		goto out;
	}

	TP_LOG_INFO("Calibrating for Self Capacitance Screen\n");
	cmd_buf[1] = 0x02; /* Self Capacitance */
	rc = cypress_exec_cmd(CY_HST_CAT, cmd_buf, 2, return_buf, 1);
	if (rc < 0) {
		TP_LOG_ERR("Unable to execute Self Capacitance calibrate command.\n");
		goto out;
	}
	if (return_buf[0] != 0) {
		TP_LOG_ERR("Self Capacitance calibrate command unsuccessful\n");
		rc = -EINVAL;
		goto out;
	}

out:
	TP_LOG_INFO("Requesting mode change to Operational\n");
	rc2 = cypress_switch_to_operate();
	if (rc2 < 0)
		TP_LOG_ERR("Error on request set mode rc2 = %d\n", rc2);
	else
		TP_LOG_INFO("Mode changed to Operational: %d\n", rc2);
...
static bool cypress_need_fw_update(char *file_name)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Fail request firmware\n");
		return false;
	}

	if (core_data.force_fw_update ==  true) {
		TP_LOG_INFO("force to update fw\n");
		return true;
	}
	...
	if (fw_update == true) {
		TP_LOG_INFO("need to update fw\n");
	} else {
		TP_LOG_INFO("not need to update fw\n");
		if (fw_entry != NULL) {
			release_firmware(fw_entry);
			fw_entry = NULL;
		}
...
static int cypress_fw_update_boot(char *file_name)
{
	...
	if (rc < 0)
		TP_LOG_ERR("Firmware update failed with error rc = %d\n", rc);

	goto out;

firmware_release:
	TP_LOG_INFO("release_firmware\n");
	if (fw_entry != NULL) {
		release_firmware(fw_entry);
		fw_entry = NULL;
	}
...
static int cypress_regulator_get(void)
{
	...
	if (IS_ERR(core_data.vdd)) {
		TP_LOG_ERR("failed to get cypress vdd\n");
		return  -EINVAL;
	}

	core_data.vbus = regulator_get(NULL, TP_IO_VDD);
	if (IS_ERR(core_data.vbus)) {
		TP_LOG_ERR("failed to get cypress vbus\n");
...
static int cypress_gpio_request(void)
{
	...
	if (!gpio_is_valid(core_data.cypress_chip_data->reset_gpio)){
		TP_LOG_ERR("reset gpio:%d gpio not provided\n", core_data.cypress_chip_data->reset_gpio);
		rc = -EIO;
		goto out;
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("Fail to request gpio:%d. rc = %d\n", core_data.cypress_chip_data->reset_gpio,
			rc);
		goto out;
	}
...
static void cypress_regulator_set(void)
{
	...
	if(rc < 0){
		TP_LOG_ERR("failed to set cypress vbus\n");
	}
#endif
	return;
}
...
static void cypress_regulator_enable(void)
{
	...
	rc = regulator_enable(core_data.vbus);
	if (rc < 0) {
		TP_LOG_ERR("failed to enable cypress vbus, rc = %d\n", rc);
	}
	return;
}
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Failed to get regulator, rc = %d\n", rc);
		goto regulator_get_err;
	}

	rc = cypress_gpio_request();
	if(rc < 0){
		TP_LOG_ERR("Failed to request gpio, rc = %d\n", rc);
	...
	if (rc < 0) {
		TP_LOG_ERR("not find cypress device\n");
		goto err;
	} else {
		TP_LOG_INFO("find cypress device\n");
		goto out;
	}
...
static int cypress_init_chip(void)
{
	...
	rc = cypress_initialize_device();
	if (rc < 0) {
		TP_LOG_ERR("Failed to initialize device, rc = %d\n", rc);
		return rc;
	}
...
static int cypress_xy_worker(struct ts_fingers *info)
{
	...
	TP_LOG_DEBUG("hst_mode(xy_mode(%d)) = 0x%x, rep_len(xy_mode(%d)) = 0x%x"
	...
	TP_LOG_DEBUG("num_cur_tch = %d, sysinfo_reg_addr = 0x%x, tch_rec_size(each touch record size) = 0x%x\n", num_cur_tch,
				sysinfo_reg_addr, core_data.sysinfo.sysinfo_merged_data.tch_rec_size);

	if (rep_len == 0 && num_cur_tch > 0) {
		TP_LOG_ERR("report length error, rep_len=%d num_tch=%d\n", rep_len, num_cur_tch);
		return rc;
	}
	/* read touches */
	if (num_cur_tch > 0) {
		...
		if (rc < 0) {
			TP_LOG_ERR("Failed to read on touch regs, sysinfo_reg_addr = 0x%x, size = %d, rc = %d\n", rc,
	...
	if (IS_BAD_PKT(rep_stat)) {
		TP_LOG_INFO("Invalid buffer detected, report data invalid, rep_stat = 0x%x\n", rep_stat);
		return rc;
	}
	if (IS_LARGE_AREA(tt_stat)) {
		TP_LOG_INFO("Large area detected, set current num to 0, tt_stat = 0x%x\n", tt_stat);
		num_cur_tch = 0;
	}
	if (num_cur_tch > core_data.sysinfo.sysinfo_merged_data.max_tchs) {
		if (num_cur_tch > CY_MAX_TCH) {
			/* terminate all active tracks */
			TP_LOG_ERR("Num touch err detected (n=%d)\n", num_cur_tch);
			num_cur_tch = 0;
		} else {
			TP_LOG_ERR("(n=%d c=%d), too many tch, set num_cur_tch to max tch", num_cur_tch,
						core_data.sysinfo.sysinfo_merged_data.max_tchs);
	...
	TP_LOG_DEBUG("outdata: current finger number = %d\n", info->cur_finger_number);
	...
	for (n_finger = 0; n_finger < num_cur_tch; n_finger++) {
		for (abs = CY_TCH_X; abs < CY_TCH_NUM_ABS; abs++) {
			...
			TP_LOG_DEBUG("get %s = %d\n", cypress_tch_abs_string[abs], tch.abs[abs]);
		}
		TP_LOG_DEBUG("X = %d, Y = %d, P = %d, T = %d, E = %d\n",
		...
		TP_LOG_DEBUG("outdata: x = %d, y = %d, pressure = %d\n", info->fingers[touch_id].x,
...
static int cypress_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	rc = cypress_i2c_read(&reg_addr, sizeof(reg_addr), mode, sizeof(mode));
	if (rc) {
		TP_LOG_ERR("Fail read mode rc =%d\n", rc);
		return rc;
	}
	...
	if (unlikely(mode[0] == CY_CORE_BL_HOST_SYNC_BYTE)) {
		TP_LOG_DEBUG("False interrupt in bootloader mode, mode[0] = 0x%x\n", mode[0]);
		return NO_ERR;
	}

	if (IS_BOOTLOADER(mode[0])) {
		TP_LOG_DEBUG("bootloader mode is running\n");
	        if (core_data.process_state == CY_STATE_FW_UPDATE) {
			    out_cmd->command = TS_FW_UPDATE_BOOT;
	        } else {
			    TP_LOG_ERR("in bootloader and the state is %d\n", core_data.process_state);
	        }
		return NO_ERR;
	}
	...
	if (cur_mode == CY_HST_OPERATE) {
		...
		if (rc){
			TP_LOG_ERR("fail read mode regs rc =%d\n",rc);
			return rc;
		}
	}else
		TP_LOG_ERR("not in operation mode, current mode = 0x%x\n", cur_mode);
	/* attention IRQ */
	rc = cypress_xy_worker(info);
	if (rc){
		TP_LOG_ERR("xy_worker error rc = %d\n", rc);
		return rc;
	}
	/* handshake the event */
	rc = cypress_handshake(mode[0]);
	if (rc){
		TP_LOG_ERR("Fail handshake mode=0x%02X, rc = %d\n", mode[0], rc);
		return rc;
	}

	TP_LOG_DEBUG("cypress irq_bottom_half done\n");
	return NO_ERR;
}
...
static int cypress_get_data_and_check(enum check_data_type type, int offset)
{
	...
	if (rc < 0){
		TP_LOG_ERR("retrieve_scan error\n");
		return 0;
	}
	...
	if(rc < 0)	{
		TP_LOG_ERR("cypress_check_range failed\n");
		return -1;
	}
...
static int cypress_exec_scan_cmd(void)
{
	...
	if (rc < 0) {
		TP_LOG_ERR("Unable to send execute panel scan command.\n");
		return rc;
	}
	if (return_buf[0] != 0)
		return -EINVAL;
...
static int cypress_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	TP_LOG_INFO("cypress_get_rawdata\n");

	if (core_data.process_state != CY_STATE_NORMAL) {
		TP_LOG_ERR("can not get raw data,process state = %d\n", core_data.process_state);
	...
	if (!rawdatabuf) {
		TP_LOG_ERR("malloc rawdatabuf failed\n");
		rc = -ENOMEM;
		goto out;
	}
	...
	if (rc1 < 0) {
		TP_LOG_ERR("Sorry, failed to set mode to CAT rc = %d\n", rc1);
		goto cypress_get_rawdata;
	}
	rc2 = cypress_check_raw_data();
	if(rc2 <= 0)
		TP_LOG_ERR("cypress_check_raw_data return %d\n", rc2);
...
cypress_get_rawdata:
	...
	TP_LOG_INFO("info->used_size = %d\n",info->used_size);

	/*back to operational*/
	rc = cypress_switch_to_operate();
	if(rc < 0)
		TP_LOG_ERR("Sorry, Failed to set operational rc = %d\n", rc);
out:
	if (rawdatabuf) {
		kfree(rawdatabuf);
		rawdatabuf = NULL;
...
static int cypress_check_status(void)
{
	...
	if (core_data.process_state == CY_STATE_NORMAL) {
		...
		if ((rc < 0) || (rc1 < 0)) {
			TP_LOG_ERR("failed to access device in watchdog timer rc=%d, rc1=%d\n", rc, rc1);
			rc = (rc < 0) ? rc : rc1;
		} else {
			TP_LOG_DEBUG("mode=0x%02x, rep_stat=0x%02x, rep_ofs=0x%02x\n",
				mode, rep_stat, core_data.sysinfo.sysinfo_merged_data.rep_ofs);
			if (IS_BOOTLOADER(mode) && IS_BOOTLOADER(rep_stat)) {
				TP_LOG_ERR("device found in bootloader mode when operational mode, rep_stat=0x%02X\n",rep_stat);
				rc = -EINVAL;
			}
		}
	} else {
		TP_LOG_ERR("not in normal state, state = %d\n", core_data.process_state);
	}
