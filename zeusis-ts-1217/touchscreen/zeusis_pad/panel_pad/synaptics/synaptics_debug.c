...
extern char pad_raw_data_limit_flag;
...
DECLARE_COMPLETION(pad_f54_remove_complete);
...
static void set_report_size(void)
{
	...
	case F54_ADC_RANGE:
		if (f54->query.has_signal_clarity) {
			...
			if (rc < 0) {
				TP_LOG_INFO("Failed to read control reg_41\n");
				f54->report_size = 0;
				break;
			}
...
static int f54_rawimage_report (void)
{
	...
	if (Result == (mmi_buf_size/2)) {
		TP_LOG_DEBUG("rawdata is all right, Result = %d\n", Result);
		return 1;
	} else {
		TP_LOG_ERR("rawdata is out of range, Result = %d\n", Result);
	#if defined (CONFIG_ZEUSIS_DSM)
		if (!dsm_client_ocuppy(tp_dclient)) {
			TP_LOG_INFO("try to client record 20002 for rawdata \n");
...
static int write_to_f54_register(unsigned char report_type)
{
	...
	if (F54_RX_TO_RX1 != command)
	{
		...
		if (result < 0){
		       TP_LOG_ERR("Could not write report type to 0x%x\n", f54->data_base_addr);
			return result;
		}
	}
	mdelay(5);
...
static int f54_deltarawimage_report (void)
{
	...
	TP_LOG_INFO("f54_deltarawimage_report enter, delt_cap_uplimit = %d, delt_cap_lowlimit = %d\n", delt_cap_uplimit, delt_cap_lowlimit);
	...
	if (result == (mmi_buf_size/2)) {
		TP_LOG_DEBUG("deltadata is all right, Result = %d\n", result);
		return 1;
	} else {
		TP_LOG_ERR("deltadata is out of range, Result = %d\n", result);
	#if defined (CONFIG_ZEUSIS_DSM)
		if (!dsm_client_ocuppy(tp_dclient)) {
			TP_LOG_INFO("try to client record 20002 for noisedata \n");
			dsm_client_record(tp_dclient, "device status for 20002 is :%d\n", result);
			dsm_client_notify(tp_dclient, DSM_TP_RAWDATA_ERROR_NO);
		}
	#endif
...
static void mmi_deltacapacitance_test(void)
{
	...
	if(1 == result) {
		...
		TP_LOG_ERR("deltadata test is out of range, test result is 3F\n");
		strncat(buf_f54test_result, "-3F", MAX_STR_LEN);
	}
	return;
}
...
static int f54_delta_rx_report (void)
{
	...
	if (Result == (mmi_buf_size/2 - f54->rmi4_data->num_of_tx)) {
		TP_LOG_DEBUG("rawdata rx diff is all right, Result = %d\n", Result);
		return 1;
	}
	else {
		TP_LOG_ERR("rawdata rx diff is out of range, Result = %d\n", Result);
	#if defined (CONFIG_ZEUSIS_DSM)
		if (!dsm_client_ocuppy(tp_dclient)) {
			TP_LOG_INFO("try to client record 20002 for rx diff \n");
			dsm_client_record(tp_dclient, "device status for 20002 is :%d\n", Result);
			dsm_client_notify(tp_dclient, DSM_TP_RAWDATA_ERROR_NO);
		}
	#endif
...
static int f54_delta_tx_report (void)
{
	...
	if (Result == (mmi_buf_size/2 -f54->rmi4_data->num_of_rx)) {
		TP_LOG_DEBUG("rawdata tx diff is all right, Result = %d\n", Result);
		return 1;
	}
	else {
		TP_LOG_ERR("rawdata tx diff is out of range, Result = %d\n", Result);
	#if defined (CONFIG_ZEUSIS_DSM)
		if (!dsm_client_ocuppy(tp_dclient)) {
			TP_LOG_INFO("try to client record 20002 for tx diff \n");
			dsm_client_record(tp_dclient, "device status for 20002 is :%d\n", Result);
			dsm_client_notify(tp_dclient, DSM_TP_RAWDATA_ERROR_NO);
		}
	#endif
...
static void mmi_rawcapacitance_test(void)
{
	...
	TP_LOG_INFO("mmi_rawcapacitance_test called\n");
	...
	if (1 == result) {
		...
		TP_LOG_ERR("raw data is out of range, , test result is 1F\n");
		strncat(buf_f54test_result, "1F", MAX_STR_LEN);
	}
	if (1 == (f54_delta_rx_report() && f54_delta_tx_report())) {
		strncat(buf_f54test_result, "-2P", MAX_STR_LEN);
	} else {
		TP_LOG_ERR("raw data diff is out of range, test result is 2F\n");
...
static int synaptics_f54_malloc(void)
{
	...
	if (!f54) {
		TP_LOG_ERR("Failed to alloc mem for f54\n");
		return -ENOMEM;
	}
	...
	if (!f54->fn55) {
		TP_LOG_ERR("Failed to alloc mem for fn55\n");
		return -ENOMEM;
	}
...
static void rotate_rawdata_abcd2adcb(void)
{
	...
	if (!rawdatabuf_temp) {
		TP_LOG_ERR("Failed to alloc buffer for rawdatabuf_temp\n");
		return;
	}
...
static void synaptics_change_report_rate(void)
{
	...
	if (0 == f54->rmi4_data->synaptics_chip_data->report_rate_test) {
		...
		TP_LOG_INFO("s3207 change_report_rate default pass\n");
		return;
	}
	TP_LOG_INFO("change report rate 120 first then to 60, delay 50ms\n");
	command = (unsigned char)F54_120HI_RATE;
	rc = f54->fn_ptr->write(f54->rmi4_data, f54->control_base_addr + report_rate_offset, &command, 1);
	if(rc < 0){
		TP_LOG_ERR("set ic to 120HZ error because of i2c error");
	...
	if(rc < 0){
		TP_LOG_ERR("read 120HZ from ic error because of i2c error");
		strncat(buf_f54test_result, "-4F", MAX_STR_LEN);
		return;
	}
       TP_LOG_INFO("work report_rate 120 = %d\n", report_rate120);

	command = (unsigned char)F54_60LOW_RATE;
	rc = f54->fn_ptr->write(f54->rmi4_data, f54->control_base_addr + report_rate_offset, &command, 1);
	if(rc < 0){
		TP_LOG_ERR("write ic to 60HZ error because of i2c error");
		strncat(buf_f54test_result, "-4F", MAX_STR_LEN);
		return;
	}
	msleep(50);
	...
	if(rc < 0){
		TP_LOG_ERR("read 60HZ from ic error because of i2c error");
		strncat(buf_f54test_result, "-4F", MAX_STR_LEN);
		return;
	}
	TP_LOG_INFO("work report_rate 60 = %d\n", report_rate60);

	if((F54_DATA120_RATE == report_rate120) && (F54_DATA60_RATE == report_rate60)) {
		TP_LOG_DEBUG("change rate success\n");
		strncat(buf_f54test_result, "-4P", MAX_STR_LEN);
	} else {
		TP_LOG_ERR("change rate error");
		strncat(buf_f54test_result, "-4F", MAX_STR_LEN);
		#if defined (CONFIG_ZEUSIS_DSM)
			if (atomic_read(&g_tp_data.state) != TS_WORK_IN_SLEEP) {
				if (!dsm_client_ocuppy(tp_dclient)) {
					TP_LOG_INFO("try to client record 20006 for change report rate error \n");
...
int pad_synaptics_get_cap_data(struct ts_rawdata_info *info)
{
	int rc = NO_ERR;
	unsigned char command;

	TP_LOG_INFO("pad_synaptics_get_cap_data called\n");
	...
	if (rc < 0) {
		TP_LOG_ERR("failed to save glove/holster/palm or other status!\n");
	}
	...
	if (rc < 0) {
		TP_LOG_ERR("failed to write command to f01 reset!\n");
		goto exit;
	}
	rc = f54->rmi4_data->status_resume(f54->rmi4_data);
	if (rc < 0) {
		TP_LOG_ERR("failed to resume glove/holster/palm or other status!\n");
	}
...
static int synaptics_rmi4_f54_attention(void)
{
	...
	retval = f54->fn_ptr->write(f54->rmi4_data,
			...
			sizeof(report_index));
	if (retval < 0) {
		TP_LOG_ERR("Failed to write report data index\n");
		retval = -EINVAL;
		goto error_exit;
	}
	...
	TP_LOG_INFO("report_size = %d.\n",f54->report_size);
	TP_LOG_INFO("report_times_max = %d.\n",report_times_max);
	...
	for(i = 0;i < report_times_max;i ++){
		...
		TP_LOG_DEBUG("i = %d,report_size_temp = %d.\n",i,report_size_temp);
...
static int Synaptics_test_set_controls(void)
{
	...
	if ((f54->query.has_query13 == 1) && (f54->query_13.has_ctrl86 == 1)){
		...
		TP_LOG_INFO("no 2 offset = %d, report_rate_offset = %d\n", reg_addr, report_rate_offset);
...
exit_no_mem:
	TP_LOG_ERR("Failed to alloc mem for control registers\n");
	return -ENOMEM;
}
...
{
	TP_LOG_INFO("%s: module_name = %s,product_name=%s\n", __func__, module_name,g_tp_data.product_name);
	if(strcmp(g_tp_data.product_name,"chm") == 0) {
		if (strcmp(module_name, "oflim") == 0){
			...
			TP_LOG_ERR("%s: Failed to match module_name \n", __func__);
			return -EINVAL;
		}
	}
	return -EINVAL;
}
int pad_synaptics_rmi4_f54_init(struct synaptics_rmi4_data *rmi4_data,const char *module_name)
{
	int retval = -EINVAL;
	bool hasF54 = false;
	bool hasF55 = false;
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (ii = PDT_START; ii > PDT_END; ii -= PDT_ENTRY_SIZE) {
			...
			if (retval < 0) {
				TP_LOG_ERR("i2c read error, page = %d, ii = %d\n", page, ii);
				goto exit_free_mem;
			}

			if (!rmi_fd.fn_number) {
				TP_LOG_INFO("!rmi_fd.fn_number,page=%d,ii=%d\n",page,ii);
				retval = -EINVAL;
				break;
			}
			...
			if (rmi_fd.fn_number == SYNAPTICS_RMI4_F54) {
				...
				TP_LOG_DEBUG("f54->control_base_addr = 0x%x, f54->data_base_addr = 0x%x, f54->query_base_addr = 0x%x\n", f54->control_base_addr, f54->data_base_addr, f54->query_base_addr);
	...
	TP_LOG_INFO("f54->control_base_addr = 0x%x, f54->data_base_addr = 0x%x\n", f54->control_base_addr, f54->data_base_addr);

	if (!hasF54 || !hasF34) {
		TP_LOG_ERR("Funtion  is not available, hasF54=%d, hasF34 = %d\n", hasF54, hasF34);
		retval = -EINVAL;
		goto exit_free_mem;
	}
...
found:
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to set up f54 queries registers\n");
		goto exit_free_mem;
	}

	retval = Synaptics_test_set_controls();
	if (retval < 0) {
		TP_LOG_ERR("Failed to set up f54 control registers\n");
		goto exit_free_control;
	}

	retval = synaptics_read_f34();
	if (retval) {
		TP_LOG_ERR("Read F34 failed, retval = %d\n", retval);
		goto exit_free_mem;
	}

	f54->mmi_buf = (char *)kzalloc(mmi_buf_size, GFP_KERNEL);
	if (!f54->mmi_buf) {
		TP_LOG_ERR("Failed to alloc mmi_buf\n");
		retval = -ENOMEM;
		goto exit_free_mem;
	}
	...
	if (!f54->rawdatabuf) {
		TP_LOG_ERR(" Failed to alloc rawdatabuf\n");
		retval = -ENOMEM;
		goto exit_free_mem;
	}
