...
void pad_rmidev_sysfs_irq(void *data)
{
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length > (REG_ADDR_LIMIT - address)) {
		TP_LOG_ERR("Out of register map limit\n");
		return -EINVAL;
	}
	...
	if (length) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to read data\n");
			return retval;
		}
	} else {
		TP_LOG_ERR("write length is %d\n", length);
		return -EINVAL;
	}
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to write data\n");
			return retval;
		}
	} else {
		return -EINVAL;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TP_LOG_INFO("Attention interrupt disabled\n");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TP_LOG_INFO("Attention interrupt enabled\n");
...
static int rmidev_allocate_buffer(int count)
{
	if (count + 1 > rmidev->tmpbuf_size) {
		...
		if (!rmidev->tmpbuf) {
			TP_LOG_ERR("%s: Failed to alloc mem for buffer\n",__func__);
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (IS_ERR(dev_data)) {
		TP_LOG_ERR("Pointer of char device data is invalid");
		return -EBADF;
	}
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		TP_LOG_ERR("New position 0x%04x is invalid\n", (unsigned int)newpos);
		newpos = -EINVAL;
		goto clean_up;
	}
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		TP_LOG_INFO("rmidev device removed\n");
	}
...
static int rmidev_create_device_class(void)
{
	...
	if (IS_ERR(rmidev_device_class)) {
		TP_LOG_ERR("Failed to create /dev/%s\n", CHAR_DEVICE_NAME);
		return -ENODEV;
	}
...
void pad_synaptics_fw_debug_dev_init(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = NO_ERR;
	dev_t dev_no = 0;
	int attr_count;
	...
	TP_LOG_INFO("pad_synaptics_fw_debug_dev_init called\n");
	rmidev = kzalloc(sizeof(*rmidev), GFP_KERNEL);
	if (!rmidev) {
		TP_LOG_ERR("Failed to alloc mem for rmidev\n");
		goto err_rmidev;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to create device class\n");
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to register chrdev region\n");
			goto err_device_region;
		}
	} else {
		retval = alloc_chrdev_region(&dev_no, 0, 1, CHAR_DEVICE_NAME);
		if (retval < 0) {
			TP_LOG_ERR("Failed to allocate char device region\n");
			goto err_device_region;
		}
		rmidev_major_num = MAJOR(dev_no);
		TP_LOG_INFO("Major number of rmidev = %d\n", rmidev_major_num);
	}

	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		TP_LOG_ERR("Failed to alloc mem for dev_data\n");
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to add rmi char device\n");
	...
	if (IS_ERR(device_ptr)) {
		TP_LOG_ERR("Failed to create rmi char device\n");
		//retval = -ENODEV;
		goto err_char_device;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to export attention gpio\n");
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev),
				"attn", rmi4_data->synaptics_chip_data->irq_gpio);
		if (retval < 0) {
			TP_LOG_ERR("Failed to create gpio symlink\n");
		} else {
			TP_LOG_INFO("Exported attention gpio %d\n", rmi4_data->synaptics_chip_data->irq_gpio);
		}
	}
	...
	if (!rmidev->sysfs_dir) {
		TP_LOG_ERR("Failed to create sysfs directory\n");
		goto err_sysfs_dir;
	}
	...
	retval = sysfs_create_bin_file(rmidev->sysfs_dir,
			&attr_data);
	if (retval < 0) {
		TP_LOG_ERR("Failed to create sysfs bin file\n");
		goto err_sysfs_bin;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to create sysfs attributes\n");
			retval = -ENODEV;
