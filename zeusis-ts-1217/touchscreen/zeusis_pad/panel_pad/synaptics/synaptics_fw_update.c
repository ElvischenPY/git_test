...
static DECLARE_COMPLETION(zeusis_fwu_remove_complete);
...
static int synaptics_get_f34_addr(struct synaptics_rmi4_data *rmi4_data, unsigned char*f34_address)
{
	...
	for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE)
	{
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to read map register\n");
			return retval;
		}
	...
	TP_LOG_ERR("Failed to get f34 addr\n");
	return -EINVAL;
}

unsigned char pad_s3320_config_id[4];
/* Use to record configid */
int pad_synaptics_fw_configid(struct synaptics_rmi4_data *rmi4_data)
{
	int rc = 0;
	unsigned char f34_ctrl_base_addr = 0;

	TP_LOG_INFO("pad_synaptics_fw_configid called\n");

	rc = synaptics_get_f34_addr(rmi4_data, &f34_ctrl_base_addr);
	if (rc < 0){
		TP_LOG_ERR("failed to scan pdt\n");
	}

	rc = rmi4_data->i2c_read(rmi4_data, f34_ctrl_base_addr, pad_s3320_config_id, sizeof(pad_s3320_config_id));
	if (rc < 0) {
		TP_LOG_ERR("Could not read configid\n");
		return rc;
	}

	TP_LOG_INFO("config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X,addr = 0x%02x\n",
					pad_s3320_config_id[0], pad_s3320_config_id[1], pad_s3320_config_id[2], pad_s3320_config_id[3], f34_ctrl_base_addr);

	return 0;
}

/* Used to read configid */
int pad_synaptics_get_configid(u8 *buf, size_t buf_size)
{
	//int rc = 0;
	...
	snprintf(buf, buf_size, "%02x %02x %02x %02x\n",
				pad_s3320_config_id[0],
				pad_s3320_config_id[1],
				pad_s3320_config_id[2],
				pad_s3320_config_id[3]);
...
static int fwu_read_f34_queries(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read bootloader ID\n");
		return retval;
	}

	TP_LOG_INFO("bootloader_id[1] = %c", fwu->bootloader_id[1]);
	...
	if (fwu->bootloader_id[1] == '5') {
		...
		TP_LOG_ERR("Unrecognized bootloader version\n");
		return -EINVAL;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read flash properties\n");
		return retval;
	}
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			2);
	if (retval < 0) {
		TP_LOG_ERR("Failed to read block size info\n");
		return retval;
	}
...
static int fwu_read_f34_flash_status(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		TP_LOG_ERR("Failed to read flash command\n");
		return retval;
	}
...
static int fwu_reset_device(void)
{
	...
	TP_LOG_INFO("fwu_reset_device called\n");

	retval = fwu->rmi4_data->reset_device(fwu->rmi4_data);
	if (retval < 0) {
		TP_LOG_ERR("Failed to reset core driver after reflash\n");
		return retval;
	}
	return 0;
}
...
static int fwu_write_f34_command(unsigned char cmd)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		TP_LOG_ERR("Failed to write command 0x%02x\n", command);
		return retval;
	}
...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	do {
		...
		TP_LOG_DEBUG("flash command 0x%x, status 0x%x\n", fwu->command, fwu->flash_status);

		retval = fwu_read_f34_flash_status();
		if(retval < 0){
			TP_LOG_ERR("Failed to read f34 flash status: %d\n", retval);
		}
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	if (f01_device_status.flash_prog) {
		TP_LOG_INFO("In flash prog mode\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_flash_prog_fail;
		#endif
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				sizeof(config_id));
	if (retval < 0) {
		TP_LOG_ERR("Failed to read config ID (code %d).\n", retval);
		flash_area = NONE;
		goto exit;
	}
	deviceConfigID = extract_uint_be(config_id);

	TP_LOG_INFO("Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X,addr = 0x%02x\n",
			config_id[0], config_id[1], config_id[2], config_id[3],fwu->f34_fd.ctrl_base_addr);

	/* .img config id */
	TP_LOG_INFO("Img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
			fwu->config_data[3]);
	...
	if (imageConfigID != deviceConfigID) {
		...
		//if current update is come from boot, we need check version. only greater can update.
		if(g_pad_boot_fwv_greater_check) {
			if(imageConfigID < deviceConfigID) {
				TP_LOG_INFO("imageConfigID[%d] is little than deviceConfigID[%d], then no need update\n", imageConfigID, deviceConfigID);
				goto exit;
			}
		}
...
static int fwu_scan_pdt(void)
{
	...
	if (!f01found || !f34found) {
		TP_LOG_ERR("Failed to find both F01 and F34\n");
		return -EINVAL;
	}
		...
		unsigned char command)
{
	...
	TP_LOG_INFO("Start to update %s blocks\n",
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(block_offset));
	if (retval < 0) {
		TP_LOG_ERR("Failed to write to block number registers\n");
		return retval;
	}

	TP_LOG_INFO("write block number registers ok,block_cnt=%d\n", block_cnt);
	for (block_num = 0; block_num < block_cnt; block_num++) {
		retval = fwu->fn_ptr->write(fwu->rmi4_data,
				...
				fwu->block_size);
		if (retval < 0) {
			TP_LOG_ERR("Failed to write block data (block %d)\n", block_num);
			return retval;
		}

		retval = fwu_write_f34_command(command);
		if (retval < 0) {
			TP_LOG_ERR("Failed to write command for block %d\n", block_num);
			return retval;
		}

		retval = fwu_wait_for_idle(WRITE_WAIT_MS);
		if (retval < 0) {
			TP_LOG_ERR("Failed to wait for idle status (block %d)\n", block_num);
			return retval;
		}
...
static int fwu_write_bootloader_id(void)
{
	...
	TP_LOG_INFO("Write bootloader ID 0x%02X 0x%02X\n",
			fwu->bootloader_id[0],
			fwu->bootloader_id[1]);

	TP_LOG_INFO("bootloader id addr = 0x%x\n", fwu->f34_fd.data_base_addr + fwu->blk_data_off);
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			fwu->f34_fd.data_base_addr + fwu->blk_data_off,
			fwu->bootloader_id,
			sizeof(fwu->bootloader_id));
	if (retval < 0) {
		TP_LOG_ERR("Failed to write bootloader ID\n");
		return retval;
	}
...
static int fwu_enter_flash_prog(void)
{
	...
	TP_LOG_INFO("Enter bootloader mode\n");

	retval = fwu_write_bootloader_id();
	if (retval < 0)
		return retval;

	TP_LOG_INFO("fwu_write_f34_command\n");
	retval = fwu_write_f34_command(CMD_ENABLE_FLASH_PROG);
	if (retval < 0)
		return retval;

	msleep(500);
	TP_LOG_INFO("fwu_wait_for_idle 500\n");

	retval = fwu_wait_for_idle(ENABLE_WAIT_MS);
	if (retval < 0)
		return retval;

	TP_LOG_DEBUG("fwu->command = 0x%x, fwu->flash_status = 0x%x\n", fwu->command, fwu->flash_status);

	if (!fwu->program_enabled) {
		TP_LOG_ERR("Program enabled bit not set : %d\n", fwu->program_enabled);
		return -EINVAL;
	}
	...
	if (!f01_device_status.flash_prog) {
		TP_LOG_ERR("Not in flash prog mode : %d\n", f01_device_status.flash_prog);
		//return -EINVAL;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read F01 device control\n");
		return retval;
	}
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			sizeof(f01_device_control.data));
	if (retval < 0) {
		TP_LOG_ERR("Failed to write F01 device control\n");
		return retval;
	}
...
static int fwu_do_reflash(void)
{
	...
	if (retval < 0) {
		TP_LOG_INFO("fwu_enter_flash_prog fail\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_enter_flash_prog_fail;
		#endif
		return retval;
	}

	TP_LOG_INFO("Entered flash prog mode\n");

	retval = fwu_write_bootloader_id();
	if (retval < 0) {
		TP_LOG_INFO("fwu_write_bootloader_id fail\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_write_bootloader_id_fail;
		#endif
		return retval;
	}

	TP_LOG_INFO("Bootloader ID written done\n");

	retval = fwu_write_f34_command(CMD_ERASE_ALL);
	if (retval < 0) {
		TP_LOG_INFO("fwu_write_f34_command fail\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_write_f34_command_fail;
		#endif
		return retval;
	}

	TP_LOG_INFO("fwu_write_f34_command done\n");

	msleep(2000);

	retval = fwu_wait_for_idle(ERASE_WAIT_MS);
	if (retval < 0) {
		TP_LOG_INFO("fwu_wait_for_idle fail\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_wait_for_idle_fail;
		#endif
		return retval;
	}

	TP_LOG_INFO("Idle status detected\n");

	if (fwu->firmware_data) {
		retval = fwu_write_firmware();
		if (retval < 0) {
			TP_LOG_INFO("fwu_write_firmware fail\n");
			#if defined (CONFIG_ZEUSIS_DSM)
			g_tp_data.dsm_info.constraints_UPDATE_status = fwu_write_firmware_fail;
			#endif
			return retval;
		}
		TP_LOG_INFO("Firmware programmed\n");
	}

	if (fwu->config_data) {
		retval = fwu_write_configuration();
		if (retval < 0) {
			TP_LOG_INFO("fwu_write_configuration fail\n");
			#if defined (CONFIG_ZEUSIS_DSM)
			g_tp_data.dsm_info.constraints_UPDATE_status = fwu_write_configuration_fail;
			#endif
			return retval;
		}
		TP_LOG_INFO("Configuration programmed\n");
	}

	TP_LOG_INFO("fwu_do_reflash suc\n");

	return retval;
}

int pad_synaptics_fw_update(void)
{
	int retval = 0;
	struct f01_device_status f01_device_status;

	TP_LOG_INFO("pad_synaptics_fw_update called\n");
	retval = fwu_do_reflash();
	if (retval < 0) {
		TP_LOG_ERR("Failed to do reflash\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_do_reflash_fail;
		#endif
	}
	/* reset device */
	retval = fwu_reset_device();
	if (retval < 0) {
		TP_LOG_ERR("Failed to reset device\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_reset_device_fail;
		#endif
		goto exit;
	}
	...
	TP_LOG_INFO("Device is in %s mode\n", f01_device_status.flash_prog == 1 ? "bootloader" : "UI");

	if (f01_device_status.flash_prog) {
		TP_LOG_INFO("Device is in flash prog mode, Flash status 0x%02X\n", f01_device_status.status_code);
		retval = 0;
		goto exit;
	}

	TP_LOG_INFO("End of reflash process\n");
exit:
	return retval;
}
...
*/
int pad_synaptics_fw_data_init(struct synaptics_rmi4_data *rmi4_data)
{
	int retval = 0;
	struct pdt_properties pdt_props;
	TP_LOG_INFO("synaptics_rmi4_fwu_init called\n");

	if (NULL == fwu) {
		fwu = kzalloc(sizeof(*fwu), GFP_KERNEL);
		if (!fwu) {
			TP_LOG_ERR("Failed to alloc mem for fwu\n");
			retval = -ENOMEM;
			goto exit;
		}
	}
	...
	if (NULL == fwu->fn_ptr) {
		...
		if (!fwu->fn_ptr) {
			TP_LOG_ERR("Failed to alloc mem for fn_ptr\n");
			retval = -ENOMEM;
			goto exit;
		}
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read PDT properties, assuming 0x00\n");
	} else if (pdt_props.has_bsr) {
		TP_LOG_ERR("Reflash for LTS not currently supported\n");
		goto exit;
	}
	...
	if (retval < 0){
		TP_LOG_ERR("Failed to scan pdt\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_fw_scan_pdt_error;
		#endif
		goto exit;
	}
	...
	TP_LOG_INFO("F01 product info: 0x%04x 0x%04x, F01 product ID: %s\n", fwu->productinfo1, fwu->productinfo2, fwu->product_id);

	retval = fwu_read_f34_queries();
	if (retval < 0){
		TP_LOG_ERR("Failed to read f34 queries\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_read_f34_queries_error;
		#endif
		goto exit;
	}
	...
	TP_LOG_INFO("pad_synaptics_fw_data_init done\n");
	return 0;
 ...
 */
int pad_synaptics_get_fw_data_boot(char *file_name, struct touch_settings *synaptics_sett_param_regs)
{
	int retval;
	...
	TP_LOG_INFO("file_name size = %ld, Requesting firmware image %s\n",
	...
	if (retval != 0) {
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_tp_data.dsm_info.constraints_UPDATE_status = fwu_request_fw_fail;
		#endif
		return retval;
	}
	...
	TP_LOG_INFO("Firmware image size = %ld\n", fwu->fw_entry_boot->size);
	return NO_ERR;
}

/*    get the firmware data form file in sd card.
 */
int pad_synaptics_get_fw_data_sd(void)
{
	int retval;

	TP_LOG_INFO("pad_synaptics_get_fw_data_sd called\n");
	...
	TP_LOG_INFO("pad_synaptics_get_fw_data_sd done\n");
	return NO_ERR;
}

bool pad_synaptics_check_fw_version(void)
{
...
void pad_synaptics_fw_data_release(void)
{
