...
//#include <zeusis_platform/log/log_jank.h>
...
char pad_raw_data_limit_flag =false;
...
enum TP_state_machine  pad_touch_state = INIT_STATE;
...
int g_pad_boot_fwv_greater_check = 0;
...
#define SYNAPTICS_VCI_LDO_JDI_VALUE (3300000)
...
static int synaptics_regulator_get(void)
{
	if (1 == rmi4_data->synaptics_chip_data->vci_regulator_type) {
		...
		if (IS_ERR(rmi4_data->tp_vci)) {
			TP_LOG_ERR("regulator tp vci not used\n");
			return  -EINVAL;
		}
	}
	...
	if (1 == rmi4_data->synaptics_chip_data->vddio_regulator_type) {
		...
		if (IS_ERR(rmi4_data->tp_vddio)) {
			TP_LOG_ERR("regulator tp vddio not used\n");
...
/*
	...
	case TOSHIBA_MDY90_LCD:
		...
		TP_LOG_INFO("TOSHIBA_MDY90_LCD");
		break;
	case JDI_NT35695_LCD:
		vol_vlaue = SYNAPTICS_VCI_LDO_JDI_VALUE;
		TP_LOG_INFO("JDI_NT35695_LCD");
		break;
	case JDI_OTM1902B_LCD:
		vol_vlaue = SYNAPTICS_VCI_LDO_JDI_VALUE;
		TP_LOG_INFO("JDI_OTM1902B_LCD");
		break;
	default:
		TP_LOG_ERR("the board is no lcd\n");
		return -EINVAL;
	}
*/
	vol_vlaue = SYNAPTICS_VCI_LDO_JDI_VALUE;
       if (!IS_ERR(rmi4_data->tp_vci)) {
		TP_LOG_INFO("set voltage is called\n");
		retval = regulator_set_voltage(rmi4_data->tp_vci, vol_vlaue, vol_vlaue);
		if (retval < 0) {
			TP_LOG_ERR("failed to set voltage regulator tp_vci error: %d\n", retval);
			return -EINVAL;
		}

		retval = regulator_enable(rmi4_data->tp_vci);
		if (retval < 0) {
			TP_LOG_ERR("failed to enable regulator tp_vci\n");
			return -EINVAL;
		}
       }
	return 0;
...
static int synaptics_vci_disable(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("failed to disable regulator tp_vci\n");
		return -EINVAL;
	}
...
static int synaptics_vddio_enable(void)
{
	...
	if (IS_ERR(rmi4_data->tp_vddio)) {
		TP_LOG_ERR("tp_vddio is err\n");
		return -EINVAL;
	}
	retval = regulator_enable(rmi4_data->tp_vddio);
	if (retval < 0) {
		TP_LOG_ERR("failed to enable regulator tp_vddio\n");
		return -EINVAL;
	}
...
static int synaptics_vddio_disable(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("failed to disable regulator tp_vddio\n");
		return -EINVAL;
	}
...
static void synatpics_regulator_enable(void)
{
	TP_LOG_INFO("synatpics_regulator_enable is called\n");
	if (1 == rmi4_data->synaptics_chip_data->vci_regulator_type) {
		if (!IS_ERR(rmi4_data->tp_vci)) {
			TP_LOG_INFO("vci enable is called\n");
...
static int synaptics_pinctrl_select_normal(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("set iomux normal error, %d\n", retval);
	}
	return retval;
}
...
static int synaptics_pinctrl_select_lowpower(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("set iomux lowpower error, %d\n", retval);
	}
	return retval;
}
...
static int rmi_f11_read_finger_state(struct ts_fingers *info)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read f51_custom_CTRL03, code: %d.\n", retval);
		return retval;
	}
...
struct ts_device_ops pad_ts_synaptics_ops = {
...
static int synaptics_rmi4_dsm_debug(void)
{
	...
	TP_LOG_INFO("Reading tp work reg status is called by dsm_debug\n");
	...
	for (index = 0; ptr != NULL && index < ARRAY_SIZE(dsm_dump_register_map); ptr++, index++) {
		...
		if (retval < 0) {
			TP_LOG_ERR("read tp work status fail !\n");
		}
		len += data_length;
		reg_start = len - 1;
		TP_LOG_INFO("each length is len = %d , reg_start = %d, data_length = %d\n", len, reg_start, data_length);
	}

	TP_LOG_INFO("synaptics_interrupt_num is %d\n", synaptics_interrupt_num);
	for(i = 0; i < len; i++)
		TP_LOG_INFO("synaptics_reg_status[%d] read from tp is %d when error was happened\n",i, synaptics_reg_status[i]);
	return retval;
}
#endif
static int synaptics_get_rawdata(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if (1 == rmi4_data->synaptics_chip_data->unite_cap_test_interface) {
		TP_LOG_INFO("++++ get rawdata in\n");
		retval = pad_synaptics_rmi4_f54_init(rmi4_data,synaptics_sett_param_regs->module_name);
		if (retval < 0) {
			TP_LOG_ERR("Failed to init f54\n");
			return retval;
		}

		retval = pad_synaptics_get_cap_data(info);
		if (retval < 0) {
			TP_LOG_ERR("Failed to get rawdata\n");
			return retval;
		}
	}
	return NO_ERR;
...
static int synaptics_set_info_flag(struct ts_data *info)
{
	g_tp_data.get_info_flag = info->get_info_flag;
	return NO_ERR;
}
static int synaptics_chip_get_info(struct ts_chip_info_param *info)
{
	...
	TP_LOG_INFO("synaptics_chip_get_info called\n");
	TP_LOG_INFO("g_tp_data.get_info_flag=%d\n",g_tp_data.get_info_flag);
	if(!g_tp_data.get_info_flag)
	{
		if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
			retval = pad_synaptics_get_configid(buf, sizeof(buf));
			if (retval < 0)
				TP_LOG_ERR("failed to get configid\n");
		} else {
			retval = pad_synaptics_get_s3718_configid(buf, sizeof(buf));
			if (retval < 0)
				TP_LOG_ERR("failed to get configid\n");
		}
	}
...
static int synaptics_parse_dts(struct device_node *device,struct ts_device_data *chip_data)
{
	...
	if (retval) {
		TP_LOG_ERR("get rawdata_disp_format failed\n");
	...
	if (retval) {
		TP_LOG_ERR("get rawdata_arrange_swap failed\n");
	...
	if (1 == chip_data->vci_gpio_type) {
		...
		if (!gpio_is_valid(chip_data->vci_gpio_ctrl)) {
			TP_LOG_ERR("SFT: ok; ASIC: Real err----power gpio is not valid\n");
		}
	}
	...
	if (retval) {
		TP_LOG_ERR("get device SYNAPTICS_COVER_FORCE_GLOVE failed,use default!\n");
	...
	if (1 == chip_data->unite_cap_test_interface) {
		...
		if (array_len <= 0 || array_len > RAWDATA_NUM) {
			TP_LOG_ERR("raw_data_limit length invaild or dts number is larger than:%d\n", array_len);
			pad_raw_data_limit_flag =true;
			goto err;
		}
		...
		for(index = 0; index < array_len; index++){
			...
			if (retval) {
				TP_LOG_ERR("read index = %d,raw_data_limit = %s,retval = %d error,\n", index, raw_data_dts, retval);
				goto err;
			}

			chip_data->raw_limit_buf[index] = simple_strtol(raw_data_dts, NULL, 10);
			TP_LOG_INFO("rawdatabuf[%d] = %d\n", index, chip_data->raw_limit_buf[index]);
		}
	}

	retval = of_property_read_u32(device, SYNAPTICS_FWV_G_UPDATE_CHECK, &chip_data->fwv_g_update_check);
	if (retval) {
		TP_LOG_INFO("get device SYNAPTICS_FWV_G_UPDATE_CHECK null\n");
		chip_data->fwv_g_update_check = 0;
		retval = 0;
	}

	TP_LOG_INFO("reset_gpio = %d, irq_gpio = %d, irq_config = %d, algo_id = %d, ic_type = %d, x_max = %d, y_max = %d, x_mt = %d,y_mt = %d, rawdata_disp_format = %d, rawdata_arrange_swap = %d\n", \
...
void pad_ts_power_gpio_enable(void)
{
...
EXPORT_SYMBOL(pad_ts_power_gpio_enable);

void pad_ts_power_gpio_disable(void)
{
	mutex_lock(&ts_power_gpio_sem);
...
EXPORT_SYMBOL(pad_ts_power_gpio_disable);

static void synaptics_power_on(void)
{
	TP_LOG_INFO("synaptics_power_on called\n");
	/*0 is power supplied by gpio, 1 is power supplied by ldo*/
	...
	if ((1 == rmi4_data->synaptics_chip_data->vci_gpio_type) && (1 == rmi4_data->synaptics_chip_data->vddio_gpio_type)) {
		TP_LOG_INFO("Both  vci and vddio were controlled by gpio\n");
		if (rmi4_data->synaptics_chip_data->vci_gpio_ctrl == rmi4_data->synaptics_chip_data->vddio_gpio_ctrl) {
			gpio_direction_output(rmi4_data->synaptics_chip_data->vci_gpio_ctrl, 1);
			msleep(1);
		} else {
		...
		TP_LOG_INFO("Only vci was controlled by gpio\n");
		gpio_direction_output(rmi4_data->synaptics_chip_data->vci_gpio_ctrl, 1);
	} else if (1 == rmi4_data->synaptics_chip_data->vddio_gpio_type) {
		TP_LOG_INFO("Only vddio was controlled by gpio add delay 1ms\n");
		pad_ts_power_gpio_enable();
	}
...
static void synaptics_power_off(void)
{
	...
	/*0 is power supplied by gpio, 1 is power supplied by ldo*/
	if ((1 == rmi4_data->synaptics_chip_data->vci_gpio_type) && (1 == rmi4_data->synaptics_chip_data->vddio_gpio_type)) {
		TP_LOG_INFO("Both  vci and vddio were need to output 0\n");
		if (rmi4_data->synaptics_chip_data->vci_gpio_ctrl == rmi4_data->synaptics_chip_data->vddio_gpio_ctrl) {
		...
		TP_LOG_INFO("Only vci was need to output 0\n");
		gpio_direction_output(rmi4_data->synaptics_chip_data->vci_gpio_ctrl, 0);
	} else if (1 == rmi4_data->synaptics_chip_data->vddio_gpio_type) {
		TP_LOG_INFO("Only vddio was need to output 0\n");
		pad_ts_power_gpio_disable();
	}
...
static void synaptics_gpio_reset(void)
{
	TP_LOG_DEBUG("synaptics_gpio_reset\n");
	gpio_direction_output(rmi4_data->synaptics_chip_data->reset_gpio, 1);
	msleep(20);
	gpio_direction_output(rmi4_data->synaptics_chip_data->reset_gpio, 0);
	msleep(20);
...
static int synaptics_gpio_request(void)
{
	...
	if ((1 == rmi4_data->synaptics_chip_data->vci_gpio_type) && (1 == rmi4_data->synaptics_chip_data->vddio_gpio_type)) {
		if (rmi4_data->synaptics_chip_data->vci_gpio_ctrl == rmi4_data->synaptics_chip_data->vddio_gpio_ctrl) {
			...
			if (retval) {
				TP_LOG_ERR("SFT:Ok;  ASIC: Real ERR----unable to request vci_gpio_ctrl firset:%d\n", rmi4_data->synaptics_chip_data->vci_gpio_ctrl);
				goto ts_vci_out;
			}
		} else {
			retval = gpio_request(rmi4_data->synaptics_chip_data->vci_gpio_ctrl, "ts_vci_gpio");
			if (retval) {
				TP_LOG_ERR("SFT:Ok;  ASIC: Real ERR----unable to request vci_gpio_ctrl2:%d\n", rmi4_data->synaptics_chip_data->vci_gpio_ctrl);
				goto ts_vci_out;
			}
			retval = gpio_request(rmi4_data->synaptics_chip_data->vddio_gpio_ctrl, "ts_vddio_gpio");
			if (retval) {
				TP_LOG_ERR("SFT:Ok;  ASIC: Real ERR----unable to request vddio_gpio_ctrl:%d\n", rmi4_data->synaptics_chip_data->vddio_gpio_ctrl);
				goto ts_vddio_out;
			}
		}
	} else {
...
{
	TP_LOG_INFO("synaptics_gpio_free\n");
...
static int i2c_communicate_check(void)
{
	...
	for (i = 0; i < I2C_RW_TRIES; i++) {
		retval = synaptics_rmi4_i2c_read(rmi4_data, pdt_entry_addr,
						...
						sizeof(rmi_fd));
		if (retval < 0) {
			TP_LOG_ERR("Failed to read register map, i = %d, retval = %d\n", i, retval);
			msleep(50);
		} else {
			TP_LOG_INFO("i2c communicate check success\n");
			retval = NO_ERR;
			return retval;
		}
	}
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	TP_LOG_INFO("synaptics chip detect called\n");

	if (!device || !chip_data || !ts_dev) {
		TP_LOG_ERR("device, chip_data or ts_dev is NULL \n");
		return -ENOMEM;
	}

	rmi4_data = kzalloc(sizeof(*rmi4_data) * 2, GFP_KERNEL);
	if (!rmi4_data) {
		TP_LOG_ERR("Failed to alloc mem for struct rmi4_data\n");
		return -ENOMEM;
	}
...
*/
	...
	if (get_lcd_type() == JDI_1902B_LCD) {
		TP_LOG_INFO("JDI_1902B_LCD is called \n");
		rmi4_data->flip_x = true;
		rmi4_data->flip_y = true;
	} else {
		TP_LOG_INFO("lcd is not JDI_1902B_LCD \n");
		rmi4_data->flip_x = false;
...
*/
	...
	if (retval < 0) {
		TP_LOG_ERR("not find synaptics device\n");
		goto check_err;
	} else {
		TP_LOG_INFO("find synaptics device\n");
		goto out;
	}

out:
	TP_LOG_INFO("synaptics chip detect done\n");
	return NO_ERR;
...
static int synaptics_init_chip(void)
{
	...
	if (rc < 0 ) {
		TP_LOG_ERR("Failed to synaptics_rmi4_query_device\n");
		return rc;
	}
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (rc < 0) {
			TP_LOG_ERR("Failed to get FASTRATE info\n");
			return rc;
		}
	}
	for (count = 0; synaptics_sett_param_regs_map[count].module_name  != NULL; count++)
	{
		...
		case SYNAPTICS_S3207:
			...
			(synaptics_sett_param_regs_map[count].build_id[2] == rmi4_data->rmi4_mod_info.synaptics_build_id[2]))
			{
				...
				TP_LOG_INFO("SYNAPTICS_S3207 synaptics_sett_param_regs_map count is %d\n", count);
				goto out;
			}
		break;
		case SYNAPTICS_S3350:
			if (SYNAPTICS_S3350 != rmi4_data->synaptics_chip_data->ic_type)
				break;
			synaptics_sett_param_regs = &synaptics_sett_param_regs_map[count];
			TP_LOG_INFO("SYNAPTICS_S3350 synaptics_sett_param_regs_map count is %d\n", count);
			goto out;
		break;
		case SYNAPTICS_S3320:
			if (SYNAPTICS_S3320 != rmi4_data->synaptics_chip_data->ic_type)
				break;
			synaptics_sett_param_regs = &synaptics_sett_param_regs_map[count];
			TP_LOG_INFO(" SYNAPTICS_S3320 synaptics_sett_param_regs_map count is %d\n", count);
			goto out;
		break;
		case SYNAPTICS_S3718:
			...
			{
				synaptics_sett_param_regs = &synaptics_sett_param_regs_map[count];
				TP_LOG_INFO("synaptics_sett_param_regs_map count is %d\n", count);
				goto out;
			}*/
			synaptics_sett_param_regs = &synaptics_sett_param_regs_map[count];
			TP_LOG_INFO("SYNAPTICS_S3718 synaptics_sett_param_regs_map count is %d\n", count);
			goto out;
		break;
		}
       }
	...
	if (NULL == synaptics_sett_param_regs) {
		TP_LOG_ERR("no ic_type or module name is found\n");
		rc = -EINVAL;
	}
out:
	TP_LOG_INFO("module id is %s\n", synaptics_sett_param_regs->module_name);
	return rc;
}
...
static int synaptics_fw_update_boot(char *file_name)
{
	...
	pad_synaptics_fw_debug_dev_init(rmi4_data);
#endif
	WARN_ON(1);
	TP_LOG_INFO("synaptics_fw_update_boot called\n");

	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
		retval = pad_synaptics_fw_data_init(rmi4_data);
		if (retval) {
			TP_LOG_ERR("pad_synaptics_fw_data_init failed\n");
			#if defined (CONFIG_ZEUSIS_DSM)
			g_tp_data.dsm_info.constraints_UPDATE_status = fwu_fw_data_init_error;
			#endif
			goto data_release;
		}
		strncat(file_name, rmi4_data->rmi4_mod_info.product_id_string, PRODUCT_ID_FW_LEN);
		TP_LOG_INFO("file_name name is :%s\n", file_name);
		msleep(1000);
		retval = pad_synaptics_get_fw_data_boot(file_name, synaptics_sett_param_regs);
		if (retval) {
			TP_LOG_ERR("load fw data from bootimage error\n");
			#if defined (CONFIG_ZEUSIS_DSM)
			g_tp_data.dsm_info.constraints_UPDATE_status = fwu_get_fw_data_fail;
			#endif
			goto data_release;
		}

		if(rmi4_data->synaptics_chip_data->fwv_g_update_check) {
			g_pad_boot_fwv_greater_check = 1;
		}

		need_update = pad_synaptics_check_fw_version();

		if (rmi4_data->force_update || need_update) {
			retval = pad_synaptics_fw_update();
			if (retval) {
				TP_LOG_ERR("failed update fw\n");
				#if defined (CONFIG_ZEUSIS_DSM)
				g_tp_data.dsm_info.constraints_UPDATE_status = fwu_fw_update_fail;
				#endif
			} else {
				TP_LOG_INFO("successfully update fw\n");
			}
		}
		g_pad_boot_fwv_greater_check = 0;
	} else {
		retval = pad_synaptics_fw_data_s3718_init(rmi4_data);
		if (retval) {
			TP_LOG_ERR("pad_synaptics_fw_data_s3718_init failed\n");
			goto data_release;
		}
		strncat(file_name, rmi4_data->rmi4_mod_info.product_id_string, PRODUCT_ID_FW_LEN);
		TP_LOG_INFO("file_name name is :%s\n", file_name);
		msleep(1000);
		retval = pad_synaptics_get_fw_data_s3718_boot(file_name, synaptics_sett_param_regs);
		if (retval) {
			TP_LOG_ERR("load fw data from s3718 bootimage error\n");
			goto data_release;
		}

		need_update = pad_synaptics_check_fw_s3718_version();

		if (rmi4_data->force_update || need_update) {
			retval = pad_synaptics_fw_s3718_update();
			if (retval) {
				TP_LOG_ERR("failed update s3718 fw\n");
			} else {
				TP_LOG_INFO("successfully s3718 update fw\n");
			}
		}
	}
...
data_release:
	...
	{
		/* Refresh configid */
		retval = pad_synaptics_fw_configid(rmi4_data);
		if (retval) {
			TP_LOG_ERR("failed read config id !\n");
		} else {
			TP_LOG_INFO("successfully read config id !\n");
		}
		pad_synaptics_fw_data_release();
	}
	else
	{
		/* Refresh configid */
		retval = pad_synaptics_fw_s3718_configid(rmi4_data);
		if (retval) {
			TP_LOG_ERR("failed read config id !\n");
		} else {
			TP_LOG_INFO("successfully read config id !\n");
		}
		pad_synaptics_fw_data_s3718_release();
	}
	if(1 != rmi4_data->synaptics_chip_data->unite_cap_test_interface) {
		if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
			retval =pad_synaptics_rmi4_f54_s3207_init(rmi4_data,synaptics_sett_param_regs->module_name); // for test report function
			if(retval){
				TP_LOG_ERR("pad_synaptics_rmi4_f54_init failed\n");
			}
		}
	}
	return retval;
...
static int synaptics_fw_update_sd(void)
{
	...
	TP_LOG_INFO("synaptics_fw_update_sd called\n");
	...
	if (SYNAPTICS_S3718 != rmi4_data->synaptics_chip_data->ic_type) {
		...
		msleep(1000);
		retval = pad_synaptics_get_fw_data_sd();
		if (retval) {
			TP_LOG_ERR("load fw data from bootimage error\n");
			goto data_release;
		}
		...
		pad_synaptics_check_fw_version();

		retval = pad_synaptics_fw_update();
		if (retval < 0) {
			TP_LOG_ERR("failed update fw\n");
		} else {
			TP_LOG_INFO("success update fw\n");
		}
	} else {
		retval = pad_synaptics_fw_data_s3718_init(rmi4_data);
		if (retval) {
		...
		msleep(1000);
		retval = pad_synaptics_get_fw_data_s3718_sd();
		if (retval) {
			TP_LOG_ERR("pad_synaptics_get_fw_data_s3718_sd\n");
			goto data_release;
		}

		/*just check the fw version*/
		pad_synaptics_check_fw_s3718_version();

		retval = pad_synaptics_fw_s3718_update();
		if (retval < 0) {
			TP_LOG_ERR("failed update s3718 fw\n");
		} else {
			TP_LOG_INFO("success update s3718 fw\n");
		}
	}
...
static int synaptics_regs_operate(struct ts_regs_info *info)
{
	...
	if(regs_base_addr < 0) {
		TP_LOG_ERR("get regs_base_addr fail, regs_base_addr = %d\n", regs_base_addr);
	...
	case TS_ACTION_WRITE:
		...
		if (retval < 0) {
			TP_LOG_ERR("TS_ACTION_WRITE error, fhandler(%d) type: %d offset: %d\n", info->fhandler, info->type, info->offset);
			retval = -EINVAL;
			goto out;
		}
		break;
	case TS_ACTION_READ:
		...
		if (retval < 0) {
			TP_LOG_ERR("TS_ACTION_READ error, fhandler(%d) type: %d offset: %d\n", info->fhandler, info->type, info->offset);
			retval = -EINVAL;
			goto out;
		}
		for (i = 0; i < info->num; i++) {
			rmi4_data->synaptics_chip_data->reg_values[i] = value[i];
			TP_LOG_DEBUG("%s : %d  value[%d] : %d\n",__func__,__LINE__, i,  value[i]);
		}
		break;
	default:
		TP_LOG_ERR("reg operate default invalid action %d\n", info->op_action);
		retval = -EINVAL;
		break;
	}
out:
...
static int s3320_set_effective_window(struct synaptics_rmi4_data *rmi4_data)
{
	...
	int i = 0;
	int x0 = g_tp_data.feature_info.window_info.top_left_x0;
	int y0 = g_tp_data.feature_info.window_info.top_left_y0;
	int x1 = g_tp_data.feature_info.window_info.bottom_right_x1;
	int y1 = g_tp_data.feature_info.window_info.bottom_right_y1;
	int x_lcd = g_tp_data.chip_data->x_max_mt;
	int y_lcd_all = g_tp_data.chip_data->y_max_mt;
	int max_x = rmi4_data->sensor_max_x;
	int max_y = rmi4_data->sensor_max_y;

	TP_LOG_DEBUG("%s:in! x_lcd=%d, y_lcd_all=%d,max_x=%d, max_y=%d \n",__func__,
			g_tp_data.chip_data->x_max_mt,
			g_tp_data.chip_data->y_max_mt,
			rmi4_data->sensor_max_x,
			rmi4_data->sensor_max_y);

	TP_LOG_INFO("Holster window_info is (%d,%d)(%d,%d)\n",x0,y0,x1,y1);
	...
	for(i = 0;i < S3320_HOLSTER_WINDOW_REG_BYTES;i ++)
	{
		...
		TP_LOG_INFO("New effective_window[%d] = %d\n",i, effective_window[i]);
	}
	...
	if (ret < 0){
		...
		TP_LOG_INFO("%s: new effective_window is written success!\n", __func__);
	}
	...
	ret = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(f51_ctrl_15));
	if (ret < 0){
		TP_LOG_ERR("%s:erase effective_window cut error!ret = %d\n",__func__,ret);
	}
...
static int synaptics_set_wakeup_gesture_enable_switch(u8 enable)
{
	...
	case SYNAPTICS_S3718:
		...
		break;
	default:
		TP_LOG_ERR("rmi4_data->synaptics_chip_data->ic_type = %d do not support wakeup gesture enable switch\n", rmi4_data->synaptics_chip_data->ic_type);
		retval = -EINVAL;
		goto out;
	}

	TP_LOG_INFO("%s, write TP IC\n", __func__);
	...
	if (retval < 0) {
		TP_LOG_ERR("switch wakeup gesture enable mode failed: %d\n", retval);
	}
...
static int synaptics_set_holster_switch(u8 holster_switch)
{
	...
	TP_LOG_INFO("synaptics_set_holster_switch called\n");
	...
	case SYNAPTICS_S3718:
		...
		if (retval < 0) {
			TP_LOG_ERR("get current holster value Failed: %d\n", retval);
		}
		temp_value &= 0xFD;
		holster_switch |= temp_value;
		break;
	default:
		TP_LOG_ERR("rmi4_data->synaptics_chip_data->ic_type = %d\n", rmi4_data->synaptics_chip_data->ic_type);
		retval = -EINVAL;
		goto out;
	}

	TP_LOG_INFO("holster_enable_addr = 0x%4x, holster switch is %d \n", holster_enable_addr, holster_switch);

	retval = synaptics_rmi4_i2c_write(rmi4_data, holster_enable_addr, &holster_switch, 1);
	if (retval < 0) {
		TP_LOG_ERR("open holster function failed: %d\n", retval);
	}
	...
	if(rmi4_data->synaptics_chip_data->cover_force_glove)
	{
		...
		if (retval < 0) {
			TP_LOG_ERR("Read S3320_F54_CMD_BASE_ADDR Failed: %d\n", retval);
		}
		...
		if (retval < 0) {
			TP_LOG_ERR("Write S3320_F54_CMD_BASE_ADDR Failed: %d\n", retval);
		}
	}
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (retval < 0) {
		TP_LOG_ERR("Read F54_ANALOG_CMD0 Failed: %d\n", retval);
		}
		...
		if (1 != ((force_cal >> 1) & MASK_1BIT)) {
			...
			if (retval < 0) {
			TP_LOG_ERR("Write F54_ANALOG_CMD0 Failed: %d\n", retval);
			}

			TP_LOG_DEBUG("Synaptics S3207 Force Cal Done: %d\n", retval);
		}
	}
out:
	return retval;
...
static int synaptics_holster_switch(struct ts_holster_info *info)
{
	...
	if (!info) {
		TP_LOG_ERR("synaptics_holster_switch: info is Null\n");
		retval = -ENOMEM;
		return retval;
	}
	...
	switch (info->op_action) {
		case TS_ACTION_WRITE:
			...
			if (retval < 0) {
				TP_LOG_ERR("set holster switch(%d), failed: %d\n", info->holster_switch, retval);
			}
			break;
		default:
			TP_LOG_INFO("invalid holster switch(%d) action: %d\n", info->holster_switch, info->op_action);
			retval = -EINVAL;
			break;
	}
...
static int synaptics_set_roi_switch(u8 roi_switch)
{
	...
	roi_ctrl_addr = rmi4_data->rmi4_feature.f51_ctrl_base_addr + g_tp_data.feature_info.roi_info.roi_control_addr_offset;
	TP_LOG_INFO("roi_ctrl_write_addr=0x%4x, roi_switch=%d\n", roi_ctrl_addr, roi_switch);

	retval = synaptics_rmi4_i2c_write(rmi4_data, roi_ctrl_addr, &roi_switch, sizeof(roi_switch));
	if (retval < 0) {
		TP_LOG_ERR("set roi switch failed: %d\n", retval);
		return retval;
	}
...
static int synaptics_read_roi_switch(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("read roi switch failed: %d\n", retval);
		return retval;
	}
 	g_tp_data.feature_info.roi_info.roi_switch = roi_switch;
	f51_roi_switch = roi_switch;
	TP_LOG_INFO("roi_ctrl_read_addr=0x%4x, roi_switch=%d\n", roi_ctrl_addr, roi_switch);
#endif
	return retval;
}
...
static int synaptics_roi_switch(struct ts_roi_info *info)
{
...
#ifdef ROI
	if (!info) {
		TP_LOG_ERR("synaptics_roi_switch: info is Null\n");
		retval = -ENOMEM;
		return retval;
	}
	...
	switch (info->op_action) {
		case TS_ACTION_WRITE:
			...
			if (retval < 0) {
				TP_LOG_ERR("set roi switch(%d), failed: %d\n", info->roi_switch, retval);
			}
			break;
		case TS_ACTION_READ:
			...
			break;
		default:
			TP_LOG_INFO("invalid roi switch(%d) action: %d\n", info->roi_switch, info->op_action);
			retval = -EINVAL;
			break;
	}
#endif
...
static int synaptics_get_glove_switch(u8 *glove_switch)
{
	...
	u8 value = 0;
	if (!glove_switch) {
		TP_LOG_ERR("synaptics_get_glove_switch: glove_switch is Null\n");
		return -ENOMEM;
	}
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		retval = synaptics_rmi4_i2c_read(rmi4_data, cherry_glove_enable_addr, &value,
				sizeof(value));
		if (retval < 0) {
			TP_LOG_ERR("read glove switch(%d) err : %d\n", *glove_switch, retval);
			goto out;
		}
		if (0 == value) {
		...
		if ((value & GLOVE_REG_BIT) == 1) {			//glove open status
			...
			*glove_switch = GLOVE_SWITCH_OFF;
			...
			TP_LOG_ERR("wrong state: value = %d\n", value);
			retval = -EFAULT;
			goto out;
		}
	}
	...
	/*if (SYNAPTICS_S3320 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		TP_LOG_INFO("get pen addr : 0x%2x\n",pen_enable_addr);

		retval = synaptics_rmi4_i2c_read(rmi4_data, pen_enable_addr, glove_switch,
				sizeof(*glove_switch));
		if (retval < 0) {
			TP_LOG_ERR("read pen switch(%d) err : %d\n", *glove_switch, retval);
		}
	}*/
	TP_LOG_DEBUG("synaptics_get_glove_switch done : %d\n", *glove_switch);
out:
	return retval;
}
...
static int synaptics_set_glove_switch(u8 glove_switch)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		TP_LOG_INFO("set glove switch : %d\n",glove_switch);
		...
		if (retval < 0) {
			TP_LOG_ERR("open glove function failed: %d\n", retval);
			goto out;
		}
	} else {
		glove_enable_addr = rmi4_data->rmi4_feature.f12_ctrl_base_addr + rmi4_data->rmi4_feature.glove_feature.offset;
		TP_LOG_INFO("set glove addr : 0x%2x, glove_switch= %d 1:on 0:off\n",glove_enable_addr, glove_switch);
		if(GLOVE_SWITCH_ON == glove_switch)
			value = 1;
		else if (GLOVE_SWITCH_OFF == glove_switch)
			value = 0;
		...
		if (retval < 0) {
			TP_LOG_ERR("get glove value failed: %d\n", retval);
			goto out;
		}
		temp_value &= 0xFE;
		value |= temp_value;
...
static int synaptics_glove_switch(struct ts_glove_info *info)
{
	...
	if (!info) {
		TP_LOG_ERR("synaptics_glove_switch: info is Null\n");
		retval = -ENOMEM;
		return retval;
	}
	switch (info->op_action) {
		case TS_ACTION_READ:
			...
			if (retval < 0) {
				TP_LOG_ERR("get glove switch(%d), failed : %d", info->glove_switch, retval);
				break;
			}
			info->glove_switch = buf;
			TP_LOG_INFO("read_glove_switch=%d, 1:on 0:off\n",info->glove_switch);
			break;
		case TS_ACTION_WRITE:
			buf = info->glove_switch;
			TP_LOG_INFO("write_glove_switch=%d\n",info->glove_switch);
			if((GLOVE_SWITCH_ON != info->glove_switch) && (GLOVE_SWITCH_OFF != info->glove_switch))
			{
				TP_LOG_ERR("write wrong state: buf = %d\n", buf);
				retval = -EFAULT;
				break;
			}
			retval = synaptics_set_glove_switch(buf);
			if (retval < 0) {
				TP_LOG_ERR("set glove switch(%d), failed : %d", buf, retval);
			}
			break;
		default:
			TP_LOG_ERR("invalid switch status: %d", info->glove_switch);
			retval = -EINVAL;
			break;
	}
...
static int synaptics_get_palm_switch(u8 *palm_switch)
{
	...
	if (!palm_switch) {
		TP_LOG_ERR("synaptics_get_palm_switch: palm_switch is Null\n");
		return -ENOMEM;
	}
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		*palm_switch = rmi4_data->synaptics_chip_data->easy_wakeup_info.palm_cover_flag;
		...
		TP_LOG_INFO("get palm addr : 0x%2x\n",palm_enable_addr);

		retval = synaptics_rmi4_i2c_read(rmi4_data, palm_enable_addr, palm_switch,
				sizeof(*palm_switch));
		if (retval < 0) {
			TP_LOG_ERR("read palm switch(%d) err : %d\n", *palm_switch, retval);
			goto out;
		}
	}

	TP_LOG_DEBUG("synaptics_get_palm_switch done : %d\n", *palm_switch);
out:
	return retval;
}
...
static int synaptics_set_palm_switch(u8 palm_switch)
{
	...
	unsigned char palm_enable_offset = 0;
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		TP_LOG_INFO("just set palm_contrlo_flag = control for S3207, palm_switch = %d\n", palm_switch);
...
static int synaptics_palm_switch(struct ts_palm_info *info)
{
	...
	if (!info) {
		TP_LOG_ERR("synaptics_palm_set_switch: info is Null\n");
		retval = -ENOMEM;
		return retval;
	}
	switch (info->op_action) {
	case TS_ACTION_READ:
		if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
			...
			if (retval < 0) {
				TP_LOG_ERR("get glove switch(%d), failed : %d\n", info->palm_switch, retval);
			}
	...
	case TS_ACTION_WRITE:
			if (info->palm_switch == 1) {
				...
				TP_LOG_ERR("wrong input : %d\n", info->palm_switch);
				buf = 1;
			}
			retval = synaptics_set_palm_switch(buf);
			if (retval < 0) {
				TP_LOG_ERR("set palm switch(%d), failed : %d\n", buf, retval);
			}
		break;
	default:
		TP_LOG_ERR("invalid switch status: %d\n", info->palm_switch);
		retval = -EINVAL;
		break;
	}
...
static void synaptics_shutdown(void)
{
	TP_LOG_INFO("synaptics_shutdown\n");
...
static void synaptics_put_device_into_easy_wakeup(void)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to enter sleep mode2\n");
			rmi4_data->sensor_sleep = false;
			return;
		} else {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to set fastrate\n");
			rmi4_data->sensor_sleep = false;
			return;
		} else {
			rmi4_data->sensor_sleep = true;
		}
		TP_LOG_DEBUG(" read from  f11_ctrl_base_addr92  rmi4_data->f11_ctrl_base_addr  = 0x%0x rmi4_data->f11_ctrl_base_addr+92 =0x%0x\n",rmi4_data->rmi4_feature.f11_ctrl_base_addr,
		...
		if (retval < 0) {
			TP_LOG_ERR("%s: Failed to set wakeup mode3\n",__func__);
			rmi4_data->sensor_sleep = false;
			return;
		}else {
			rmi4_data->sensor_sleep = true;
		}
		TP_LOG_DEBUG(" read from  f11_ctrl_base_addr92  ctrl1 = %0x retval =%0x\n",device_ctrl,retval);

		retval = synaptics_rmi4_i2c_read(rmi4_data, F54_ANALOG_CMD0,	&device_ctrl, sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("Failed to set wakeup mode7\n");
			rmi4_data->sensor_sleep = false;
			return;
		} else {
			rmi4_data->sensor_sleep = true;
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to set wakeup mode8\n");
			rmi4_data->sensor_sleep = false;
			return;
		} else {
			rmi4_data->sensor_sleep = true;
		...
		retval = synaptics_rmi4_i2c_read(rmi4_data,
				...
				sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("%s: Failed to read doze mode\n", __func__);
		}
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data,
				...
				sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("%s: Failed to set into mode\n", __func__);
		}
	}else {
		...
		if (retval < 0) {
			TP_LOG_ERR("read Wakeup Gesture Only reg F12_2D_CTRL20 fail !\n");
		} else {
			TP_LOG_DEBUG("read Wakeup Gesture Only reg F12_2D_CTRL20(00)/00 Motion Suppression : CTRL20(00)/00 : 0x%x  CTRL20(00)/01 : 0x%x CTRL20(01)/00 : 0x%x CTRL20(04)/00 : 0x%x\n", device_ctrl_data[0], device_ctrl_data[1], device_ctrl_data[2], device_ctrl_data[3]);
		}
		...
		if (retval < 0) {
			TP_LOG_ERR("easy wake up suspend write Wakeup Gesture Only reg fail !\n");
		} else {
			TP_LOG_INFO("easy wake up suspend write Wakeup Gesture Only reg OK address(0x%2x) valve(0x%2x)\n", f12_ctrl_base+gusture_ctrl_offset, device_ctrl_data[2]);
		}
	}
...
static void synaptics_put_device_outof_easy_wakeup(struct synaptics_rmi4_data *rmi4_data)
{
	...
	TP_LOG_DEBUG("synaptics_put_device_outof_easy_wakeup  info->easy_wakeup_flag =%d\n",info->easy_wakeup_flag);
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data, EASY_WAKEUP_FASTRATE, &device_ctrl, sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("Failed to out of  sleep mode1\n");
			rmi4_data->sensor_sleep = true;
			return;
		}
		retval = synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->rmi4_feature.f11_ctrl_base_addr, &device_ctrl, sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("Failed to out of  sleep mode4\n");
			rmi4_data->sensor_sleep = true;
			return;
		}
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data, rmi4_data->rmi4_feature.f11_ctrl_base_addr, &device_ctrl, sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("Failed to out of  sleep mode5\n");
			rmi4_data->sensor_sleep = true;
			return;
		} else {
			rmi4_data->sensor_sleep = false;
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data,
				...
				sizeof(device_ctrl));
		if (retval < 0) {
			TP_LOG_ERR("Failed to set out of doze mode\n");
		}
	}else {
		gusture_ctrl_offset = rmi4_data->rmi4_feature.geswakeup_feature.f12_2d_ctrl20_lpm;
		f12_ctrl_base = rmi4_data->rmi4_feature.f12_ctrl_base_addr;
		...
		retval = synaptics_rmi4_i2c_write(rmi4_data, f12_ctrl_base+gusture_ctrl_offset, &device_ctrl_data[0], sizeof(device_ctrl_data));
		if (retval < 0) {
			TP_LOG_ERR("easy wake up resume write Wakeup Gesture Only reg fail\n");
		} else {
			TP_LOG_INFO("easy wake up suspend write Wakeup Gesture Only reg OK address(0x%2x) valve(0x%2x)\n", f12_ctrl_base+gusture_ctrl_offset, device_ctrl_data[2]);
		}
	}
...
static int synaptics_suspend(void)
{
	...
	TP_LOG_INFO("in last time wake mode synaptics_interrupt_num = %d interrupts\n", synaptics_interrupt_num);
...
{
	int retval = NO_ERR;
	TP_LOG_INFO("between suspend and resumed there is synaptics_interrupt_num = %d interrupts\n", synaptics_interrupt_num);
	synaptics_interrupt_num = 0;
	TP_LOG_INFO("resume +\n");
	switch (rmi4_data->synaptics_chip_data->easy_wakeup_info.sleep_mode) {
	case TS_POWER_OFF_MODE:
...
static int synaptics_after_resume(void *feature_info)
{
	...
	TP_LOG_INFO("after_resume +\n");

	msleep(150);
	TP_LOG_INFO("synaptics_after_resume increase delay 50ms\n");
	/*empty list and query device again*/
	synaptics_rmi4_empty_fn_list(rmi4_data);
	retval = synaptics_rmi4_query_device(rmi4_data);
	if (retval < 0) {
		TP_LOG_ERR("Failed to query device\n");
		return retval;
	}
	retval = synaptics_holster_switch(&info->holster_info);
	if (retval < 0) {
		TP_LOG_ERR("Failed to set holster switch(%d), err: %d\n",
	...
	if (SYNAPTICS_S3207 != rmi4_data->synaptics_chip_data->ic_type){
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to set glove switch(%d), err: %d\n",
				rmi4_data->rmi4_feature.glove_feature.glove_switch, retval);
		}
	}
#endif
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to set plam switch(%d), err: %d\n",
...
static int synaptics_wakeup_gesture_enable_switch(struct ts_wakeup_gesture_enable_info *info)
{
	...
	if (info->op_action == TS_ACTION_WRITE){
		...
		TP_LOG_DEBUG("write deep_sleep switch: %d\n", info->switch_value);
		if (retval < 0) {
			TP_LOG_ERR("set deep_sleep switch(%d), failed: %d\n", info->switch_value, retval);
		}
	}else {
		TP_LOG_INFO("invalid deep_sleep switch(%d) action: %d\n", info->switch_value, info->op_action);
		retval = -EINVAL;
	}
...
rescan:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		...
		if (done) {
			TP_LOG_DEBUG("Find F01 in page description table 0x%x\n", rmi4_data->rmi4_feature.f01_cmd_base_addr);
			break;
		}
	}

	if (!done) {
		TP_LOG_ERR("Cannot find F01 in page description table\n");
		return -EINVAL;;
	}
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		TP_LOG_ERR("Failed to issue reset command, error = %d\n", retval);
		return retval;
	}
...
static int synaptics_rmi4_reset_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!rmi4_data) {
		TP_LOG_ERR("rmi4_data is NULL \n");
		retval = -ENOMEM;
		return retval;
	}

	TP_LOG_DEBUG("synaptics_rmi4_reset_device called\n");

	retval = synaptics_rmi4_reset_command(rmi4_data);
	if (retval < 0) {
		TP_LOG_ERR("Failed to send command reset\n");
		return retval;
	}
	...
	TP_LOG_DEBUG("synaptics_rmi4_reset_device end\n");
	return 0;
}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retval != PAGE_SELECT_LEN) {
		TP_LOG_ERR("error, retval != PAGE_SELECT_LEN\n");
		goto exit;
	}

	if (!rmi4_data->synaptics_chip_data->bops->bus_read) {
		TP_LOG_ERR("error, invalid bus_read\n");
		retval = -EIO;
		goto exit;
	}
...
rescan:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (retval < 0){
					TP_LOG_ERR("Failed to read device_info \n");
					return retval;
				}
				if (status.flash_prog == 1) {
					TP_LOG_INFO("In flash prog mode, status = 0x%02x\n", status.status_code);
					rmi4_data->force_update = true;	/*crc error, force update fw to workaround*/
				#if defined (CONFIG_ZEUSIS_DSM)
					if (!dsm_client_ocuppy(tp_dclient)) {
						TP_LOG_INFO("try to client record 20003 \n");
				...
				if ((status.status_code != 0) && (status.status_code != 1)) {
					TP_LOG_ERR("device  status is error ,status value = 0x%02x\n", status.status_code);
				#if defined (CONFIG_ZEUSIS_DSM)
					if (!dsm_client_ocuppy(tp_dclient)) {
						TP_LOG_INFO("try to client record 20004 \n");
			...
			case SYNAPTICS_RMI4_F11:
				...
				if (retval < 0) {
					TP_LOG_ERR("Failed to init f11 handler , retval = %d\n", retval);
					return retval;
				}
				break;
			...
			case SYNAPTICS_RMI4_F1A:
				...
				if (retval < 0){
					TP_LOG_ERR("Failed to init f1a handler , retval = %d\n", retval);
					return retval;
				}
				break;
			case SYNAPTICS_RMI4_F51:
				...
				TP_LOG_INFO("SYNAPTICS_RMI4_F51\n");
				TP_LOG_INFO("rmi_fd.intr_src_count =0x%x\n",rmi_fd.intr_src_count);
				fhandler = synaptics_rmi4_alloc_fh(&rmi_fd, page_number);
				if (NULL == fhandler) {
					TP_LOG_ERR("Failed to alloc for F%d\n", rmi_fd.fn_number);
					retval = -ENOMEM;
	...
	TP_LOG_DEBUG("Number of interrupt registers = %d\n", rmi4_data->num_of_intr_regs);
	...
	TP_LOG_ERR("the num of data sources init fail from IC == %d\n", data_sources);
	}

	/* Enable the interrupt sources */
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			TP_LOG_DEBUG("Interrupt enable mask %d = 0x%02x\n", ii, rmi4_data->intr_mask[ii]);
			...
			if (retval < 0) {
				TP_LOG_ERR("Failed to enable interrupt sources, error:%d", retval);
				return retval;
			}
		}
	}
			...
			struct synaptics_rmi4_f01_device_status *status)
{
	...
	while (1) {
		retval = synaptics_rmi4_i2c_read(rmi4_data,
				...
				sizeof(status->data));
		if (retval < 0) {
			TP_LOG_ERR("read rmi4_data->f01_data_base_addr = %d status register failed\n", rmi4_data->rmi4_feature.f01_data_base_addr);
			return false;
		}
		if (status->status_code == STATUS_CRC_IN_PROGRESS) {
			TP_LOG_ERR("CRC is in progress..., the statu_scode is %d \n", status->status_code);
			rescan = true;
			msleep(20);
		} else {
			TP_LOG_DEBUG("CRC is success, CRC test times = %d\n", times);
			break;
		}
		if (times++ > 500)
			return false;
		...
		struct synaptics_rmi4_fn_desc *rmi_fd, int page_number)
{
	...
	if (!fhandler) {
		TP_LOG_ERR("Failed to alloc memory for fhandler\n");
		return NULL;
	}
	...
	TP_LOG_DEBUG("handler number is %d, it's data_base_addr = %d, ctrl_base_addr = %d, cmd_base_addr = %d,query_base_addr = %d, page_number = %d\n",
					...
					struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(f01_query));
	if (retval < 0){
		TP_LOG_ERR("Failed to read f01_query_base_addr %d\n", rmi4_data->rmi4_feature.f01_query_base_addr);
		goto out;
	}
	...
	TP_LOG_DEBUG("count is %d \n", count);
	TP_LOG_DEBUG("synaptics_build_id is (%c  %c  %c)\n", f01_query[count-3],f01_query[count-2],f01_query[count-1]);
	TP_LOG_DEBUG("product ic string is %s\n", rmi4_data->rmi4_mod_info.product_id_string);
	TP_LOG_DEBUG("synaptics_ic_name 0 is %d,  synaptics_ic_name 1 is %d\n", rmi->synaptics_ic_name[0],rmi->synaptics_ic_name[1]);
	if (rmi->manufacturer_id != 1) {
		TP_LOG_ERR("Non-Synaptics device found, manufacturer ID = %d\n",
			rmi->manufacturer_id);
	}
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rmi->build_id));
	if (retval < 0) {
		TP_LOG_ERR("Failed to read firmware build id (code %d)\n", retval);
	}
out:
	return retval;
}
		...
		unsigned int intr_count)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(query));
	if (retval < 0){
		TP_LOG_ERR("Failed to read f11 query base reg\n");
		return retval;
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(control));
	if (retval < 0){
		TP_LOG_ERR("Failed to read f11 ctrl base reg\n");
		return retval;
	}
	/* Maximum x and y */
	...
	TP_LOG_INFO("Function %02x max x = %d max y = %d\n", fhandler->fn_number,
			rmi4_data->sensor_max_x,
			rmi4_data->sensor_max_y);
		...
		unsigned short ctrl28)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to set enable f12, error:%d\n", retval);
		return retval;
	}
		...
		unsigned int intr_count)
{
	...
	TP_LOG_DEBUG("fhandler->num_of_data_sources = %d, fhandler->fn_number = %d\n", fhandler->num_of_data_sources, fhandler->fn_number);
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(query_5.data));
	if (retval < 0) {
		TP_LOG_ERR("Failed to read f12->full_addr.query_base\n");
		return retval;
	}
	...
	TP_LOG_DEBUG("ctrl_8_offset is %d\n", ctrl_8_offset);
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read f12_ctrl_base\n");
		return retval;
	}
	TP_LOG_INFO("rx = %d, tx = %d, f12init\n", f12_2d_data[F12_RX_NUM_OFFSET], f12_2d_data[F12_TX_NUM_OFFSET]);
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(ctrl_23.data));
	if (retval < 0) {
		TP_LOG_ERR("Failed to read f12 ->full_addr.ctrl_base = %d + ctrl_23_offset = %d\n", fhandler->full_addr.ctrl_base, ctrl_23_offset);
		return retval;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to set enable f12,fhandler->full_addr.ctrl_base =%d, ctrl_28_offset = %d\n", fhandler->full_addr.ctrl_base, ctrl_28_offset);
		return retval;
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(ctrl_8.data));
	if (retval < 0) {
		TP_LOG_ERR("Failed to set enable f12,fhandler->full_addr.ctrl_base =%d, ctrl_8_offset = %d\n", fhandler->full_addr.ctrl_base, ctrl_8_offset);
		return retval;
	}
	...
	if (fhandler->data == NULL) {
		TP_LOG_ERR("Failed to alloc memory for fhandler->data\n");
		retval = -ENOMEM;
		return retval;
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!f1a) {
		TP_LOG_ERR("Failed to alloc mem for function handle\n");
		return -ENOMEM;
	}
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read query registers\n");
		return retval;
	}
	...
	if (!f1a->button_data_buffer) {
		TP_LOG_ERR("Failed to alloc mem for data buffer\n");
		return -ENOMEM;
	}
	...
	if (!f1a->button_map) {
		TP_LOG_ERR("Failed to alloc mem for button map\n");
		return -ENOMEM;
	}
				...
				struct synaptics_rmi4_fn *fhandler)
{
	...
	if (!pdata->cap_button_map) {
		TP_LOG_ERR("%s: cap_button_map is" \
				"NULL in board file\n",
				__func__);
		return -ENODEV;
		...
		unsigned int intr_count)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to alloc memory for f1a, retval = %d\n", retval);
		goto error_exit;
	}
		...
		unsigned char page)
{
	...
	TP_LOG_DEBUG("f51 init , data_base_addr = 0x%4x\n"
		...
		unsigned int reprot_gesture_point_num,struct ts_fingers *info)
{
	...
	if (reprot_gesture_point_num != 0) {
		...
		if (retval < 0) {
			TP_LOG_ERR("%s:f51_data_base read error!retval = %d",__func__,retval);
			return retval;
		}
	...
	for (i = 0; i < reprot_gesture_point_num; i++) {
		...
		if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
			x = x * g_tp_data.chip_data->x_max_mt/ rmi4_data->sensor_max_x;
			if(g_tp_data.chip_data->has_virtualkey){
				y = y * g_tp_data.chip_data->lcd_full / rmi4_data->sensor_max_y;
			}else{
				y = y * g_tp_data.chip_data->y_max_mt/ rmi4_data->sensor_max_y;
			}
			if ( (x > g_tp_data.chip_data->x_max_mt) ||( x < 0)) {
				TP_LOG_ERR("x position over the size ,x = %d ",x );
			}
			if ( (y > g_tp_data.chip_data->lcd_full) ||( y <0) ) {
				TP_LOG_ERR("x position over the size ,y = %d ",y );
			}
		}

		TP_LOG_INFO("%s: Gesture Repot Point %d:\n"
						"x = %d\n"
						"y = %d\n",
						__func__,i,x, y);
		rmi4_data->synaptics_chip_data->easy_wakeup_info.easywake_position[i] = x << 16 | y;
		TP_LOG_INFO("easywake_position[%d] = 0x%4x\n", i, rmi4_data->synaptics_chip_data->easy_wakeup_info.easywake_position[i]);
		}
	}
		...
		struct ts_fingers *info, struct ts_easy_wakeup_info *gesture_report_info, unsigned char *get_gesture_wakeup_data)
{
	...
	TP_LOG_INFO("get_gesture_wakeup_data is %d, 1 click 64 letter,\n", get_gesture_wakeup_data[0]);
	...
	case DOUBLE_CLICK_WAKEUP:
		if (IS_APP_ENABLE_GESTURE(GESTURE_DOUBLE_CLICK) & gesture_report_info->easy_wakeup_gesture) {
			TP_LOG_INFO("@@@DOUBLE_CLICK_WAKEUP detected!@@@\n");
	...
	case LINEAR_SLIDE_DETECTED:
		TP_LOG_INFO("@@@LINEAR_SLIDE_DETECTED detected!@@@ \n");
		...
		case LINEAR_SLIDE_LEFT_TO_RIGHT:
			if (IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_L2R) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@LINEAR_SLIDE_LEFT_TO_RIGHT detected!@@@\n");
		...
		case LINEAR_SLIDE_RIGHT_TO_LEFT:
			if (IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_R2L) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@LINEAR_SLIDE_RIGHT_TO_LEFT detected!@@@\n");
		...
		case LINEAR_SLIDE_TOP_TO_BOTTOM:
			if (IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_T2B) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@LINEAR_SLIDE_TOP_TO_BOTTOM detected!@@@\n");
		...
		case LINEAR_SLIDE_BOTTOM_TO_TOP:
			if (IS_APP_ENABLE_GESTURE(GESTURE_SLIDE_B2T) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@LINEAR_SLIDE_BOTTOM_TO_TOP detected!@@@\n");
	...
	case SPECIFIC_LETTER_DETECTED:
		...
		case SPECIFIC_LETTER_c:
			if (IS_APP_ENABLE_GESTURE(GESTURE_LETTER_c) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@SPECIFIC_LETTER_c detected!@@@\n");
		...
		case SPECIFIC_LETTER_e:
			if (IS_APP_ENABLE_GESTURE(GESTURE_LETTER_e) & gesture_report_info->easy_wakeup_gesture)	{
				TP_LOG_INFO("@@@SPECIFIC_LETTER_e detected!@@@\n");
		...
		case SPECIFIC_LETTER_m:
			if (IS_APP_ENABLE_GESTURE(GESTURE_LETTER_m) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@SPECIFIC_LETTER_m detected!@@@\n");
		...
		case SPECIFIC_LETTER_w:
			if (IS_APP_ENABLE_GESTURE(GESTURE_LETTER_w) & gesture_report_info->easy_wakeup_gesture) {
				TP_LOG_INFO("@@@SPECIFIC_LETTER_w detected!@@@\n");
				reprot_gesture_key_value = TS_LETTER_w;
				reprot_gesture_point_num = LETTER_LOCUS_NUM;
			}
			break;
		default:
			TP_LOG_INFO("@@@unknow letter!f11_2d_data39[6] = %d@@@\n",get_gesture_wakeup_data[2]);
			return 0;
		}
		break;
	default:
		TP_LOG_INFO("@@@unknow gesture detected!\n");
		return 0;
	}

	if(0 != reprot_gesture_key_value) {
		wake_lock_timeout(&g_tp_data.ts_wake_lock, 5*HZ);
		mutex_lock(&wrong_touch_lock);
		if (true == rmi4_data->synaptics_chip_data->easy_wakeup_info.off_motion_on) {
			...
			if (retval < 0) {
				mutex_unlock(&wrong_touch_lock);
				TP_LOG_INFO("%s: report line_coordinate error!retval = %d\n", __func__, retval);
				return retval;
			}
		...
		struct synaptics_rmi4_fn *fhandler, struct ts_fingers *info)
{
	...
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		...
		if (retval < 0) {
			TP_LOG_ERR("F11_2D_DATA38_OFFSET read error, retval = %d\n", retval);
			return 0;
		}
		TP_LOG_DEBUG("f11_data_base_addr = %d,device_data = %d", rmi4_data->rmi4_feature.f11_data_base_addr,device_data);
		...
		if (retval < 0) {
			TP_LOG_ERR("LINEAR_SLIDE_DETECTED error!retval = %d\n",retval);
			return 0;
		}
		TP_LOG_DEBUG("cherry_guesture is linear, f51_custom_data0 = %d", f51_custom_data0);
		...
		if (retval < 0) {
			TP_LOG_ERR("F11_2D_DATA39_OFFSET error!retval = %d\n",retval);
			return 0;
		}
		TP_LOG_DEBUG("%s:###read### f11_2d_data39[7] = 0x%x, f11_2d_data39[6] = 0x%x\n",__func__,f11_2d_data39[7], f11_2d_data39[6]);
		...
		if (retval < 0) {
			TP_LOG_ERR("%s read wake up value failed!\n", __func__);
			return retval;
		}
	...
	if (get_gesture_wakeup_data[0] != 0) {
		...
		TP_LOG_DEBUG("%s:get_gesture_wakeup_data[0] value is %d!\n", __func__, get_gesture_wakeup_data[0]);
		if (retval < 0) {
			TP_LOG_ERR("Failed to report gesture event!, retval = %d\n", retval);
			return 1;
		}
	}
	return 0;
		...
		struct ts_fingers *info)
{
	...
	if (!rmi4_data->synaptics_chip_data->easy_wakeup_info.palm_cover_flag) {
		TP_LOG_DEBUG(" If in easy_wake_up mode,no need to ack palm report, rs_flag = %d\n", rmi4_data->synaptics_chip_data->easy_wakeup_info.palm_cover_flag);
		return NO_ERR;
	}
	if (SYNAPTICS_S3207 == rmi4_data->synaptics_chip_data->ic_type) {
		retval = synaptics_rmi4_i2c_read(rmi4_data, cherry_palm_status_addr, &palm_status, sizeof(palm_status));
		if (retval < 0) {
			TP_LOG_ERR("read plam object_type_and_status fail !\n");
		} else {
			TP_LOG_DEBUG("read plam object_type_and_status OK object_type_and_status = 0x%x  \n",palm_status);
		}

		if (PALM_COVER_SLEEP == (palm_status & 0x02)) {
			TP_LOG_INFO("%s:palm gesture detected!\n",__func__);
			info->gesture_wakeup_value =  TS_PALM_COVERED;
			return PALM_COVER_SLEEP;
		} else {
			TP_LOG_DEBUG("%s:no palm_data!, palm_data read from ic is\n",__func__);
		}
		...
		if (object_type_and_status[0] == PALM_COVER_SLEEP_S3320) {
			TP_LOG_DEBUG("%s:palm gesture detected!\n",__func__);
			info->gesture_wakeup_value =  TS_PALM_COVERED;
			return 1;
		} else {
			TP_LOG_DEBUG("%s:no palm_data!, palm_data read from ic is %d\n",__func__, object_type_and_status[0]);
			return NO_ERR;
		}
	}
	return NO_ERR;
		...
		struct synaptics_rmi4_fn *fhandler, struct ts_fingers *info)
{
	...
	if (retval < 0){
		TP_LOG_ERR("Failed to read: %d\n", retval);
		return;
	}
#ifdef GLOVE_SIGNAL
	retval = rmi_f11_read_finger_state(info);
	if (retval < 0) {
	    TP_LOG_ERR("error: finger_state = %d\n", retval);
	    return;
	}
#endif
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			retval = synaptics_rmi4_i2c_read(rmi4_data,
					...
					data_reg_blk_size);
			if (retval < 0) {
				TP_LOG_ERR("Failed to read data report: %d\n", retval);
				return;
			}
	...
	TP_LOG_DEBUG("f11_abs_report, touch_count = %d\n", touch_count);
	return;
}
		...
		struct synaptics_rmi4_fn *fhandler, struct ts_fingers *info)
{
	...
	if (extra_data->data15_size) {
		...
		if (retval < 0) {
			TP_LOG_ERR("Failed to read data status: %d\n", retval);
			return;
		}
		...
		TP_LOG_INFO("Number of fingers to process = %d\n", fingers_to_process);
	}

	if (!fingers_to_process) {
		TP_LOG_ERR("fingers to process is 0\n");
		return;
	}
#endif
	...
	if (retval < 0) {
		TP_LOG_ERR("Failed to read: %d, read data offset is %d \n", retval, extra_data->data1_offset);
		return;
	}

	data = (struct synaptics_rmi4_f12_finger_data *)fhandler->data;

	TP_LOG_DEBUG("Number of fingers = %d\n", fingers_to_process);
	...
	if (ENABLE_ROI == f51_roi_switch) {
		if ((temp_finger_status != pre_finger_status && ((temp_finger_status&pre_finger_status)!=temp_finger_status))) {
			roi_data_addr = rmi4_data->rmi4_feature.f51_data_base_addr + g_tp_data.feature_info.roi_info.roi_data_addr_offset;
			TP_LOG_DEBUG("roi_data_addr=0x%4x\n", roi_data_addr);
		...
		struct synaptics_rmi4_fn *fhandler, struct ts_fingers *info)
{
	...
	case 4:
		...
		break;
	default:
		TP_LOG_ERR("the get_button_number is invald : %d\n", button_number);
		return;
	}
...
{
	int retval = 0;
	TP_LOG_DEBUG("Function %02x reporting\n", fhandler->fn_number);
	retval = synaptics_rmi4_gesture_report(rmi4_data, fhandler, info);
	if(true == retval){
		TP_LOG_DEBUG("synaptics_rmi4_gesture_report is called and report gesture\n");
		return;
	}
	retval = synaptics_rmi4_palm_sleep_report(rmi4_data,fhandler,info);
	if(true == retval){
		TP_LOG_DEBUG("synaptics_rmi4_palm_sleep_report is called and report gesture\n");
		return;
	}
	TP_LOG_ERR("--cbw-- fn_number=%d \n",fhandler->fn_number);
...
static int synaptics_irq_top_half(struct ts_cmd_node *cmd)
{
	...
	TP_LOG_DEBUG("synaptics irq top half called\n");
	return NO_ERR;
}
...
static int synaptics_irq_bottom_half(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if (rmi4_data->fw_debug == true) {
		...
		pad_rmidev_sysfs_irq(rmi4_data);
		return NO_ERR;
	}
#endif
