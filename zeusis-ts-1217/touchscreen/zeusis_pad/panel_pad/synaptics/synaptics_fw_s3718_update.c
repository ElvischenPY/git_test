...
unsigned char pad_s3718_config_id[4];

DECLARE_COMPLETION(pad_zuesis_fwu_remove_complete);
...
int pad_synaptics_fw_s3718_configid(struct synaptics_rmi4_data *rmi4_data)
{
	int rc = 0;
	...
	rc = rmi4_data->i2c_read(rmi4_data, f34_ctrl_base_addr, pad_s3718_config_id, sizeof(pad_s3718_config_id));
	if (rc < 0) {
	...
	TP_LOG_INFO("config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X,addr = 0x%02x\n",
					pad_s3718_config_id[0], pad_s3718_config_id[1], pad_s3718_config_id[2], pad_s3718_config_id[3], f34_ctrl_base_addr);

	return 0;
}

/* Used to read configid */
int pad_synaptics_get_s3718_configid(u8 *buf, size_t buf_size)
{
	TP_LOG_INFO("%s called\n",__func__);

	snprintf(buf, buf_size, "%02x %02x %02x %02x\n",
				pad_s3718_config_id[0],
				pad_s3718_config_id[1],
				pad_s3718_config_id[2],
				pad_s3718_config_id[3]);
...
static void fwu_parse_image_header_05_06(void)
{
	...
	if (fwu->img.contains_disp_config) {
		...
		if (retval < 0) {
			TP_LOG_ERR("%s: Failed to copy custom product ID string\n",__func__);
		}
	...
	retval = secure_memcpy(fwu->img.product_id,
			...
			PRODUCT_ID_SIZE);
	if (retval < 0) {
		TP_LOG_ERR("%s: Failed to copy product ID string\n",__func__);
	}
...
//	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
	...
	case IMAGE_HEADER_VERSION_06:
		...
		break;
	default:
		TP_LOG_ERR("%s: Unsupported image file format (0x%02x)\n",__func__, header->major_header_version);
		return -EINVAL;
	}

	if (fwu->bl_version == BL_V7) {
		if (!fwu->img.contains_flash_config) {
			TP_LOG_ERR("%s: No flash config found in firmware image\n",	__func__);
			return -EINVAL;
		}
...
static int fwu_get_image_firmware_id(unsigned int *fw_id)
{
	...
	if (fwu->img.contains_firmware_id) {
		*fw_id = fwu->img.firmware_id;
		...
		if (!strptr) {
			TP_LOG_ERR("%s: No valid PR number (PRxxxxxxx) ""found in image file name (%s)\n", __func__, fwu->image_name);
			return -EINVAL;
		}
		...
		if (!firmware_id) {
			TP_LOG_ERR("%s: Failed to alloc mem for firmware_id\n", __func__);
			return -ENOMEM;
		}
		...
		kfree(firmware_id);
		if (retval) {
			TP_LOG_ERR("%s: Failed to obtain image firmware ID\n", __func__);
			return -EINVAL;
		}
	}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (retval < 0) {
		TP_LOG_ERR("%s: Failed to read device config ID, ctrl_base_addr: 0x%02x\n",__func__, fwu->f34_fd.ctrl_base_addr);
		flash_area = NONE;
		goto exit;
	}
	device_config_id = be_to_uint(config_id);
	TP_LOG_INFO("Device config ID = 0x%02x 0x%02x 0x%02x 0x%02x, ctrl_base_addr: 0x%02x\n",
			config_id[0],
			config_id[1],
			config_id[2],
			config_id[3],
...
static int fwu_do_read_config(void)
{
	...
	case DP_CONFIG_AREA:
		if (!fwu->flash_properties.has_disp_config) {
			TP_LOG_ERR("%s: Display configuration not supported\n", __func__);
			return -EINVAL;
		}
	...
	case PM_CONFIG_AREA:
		if (!fwu->flash_properties.has_pm_config) {
			TP_LOG_ERR("%s: Permanent configuration not supported\n", __func__);
			return -EINVAL;
		}
	...
	case BL_CONFIG_AREA:
		if (!fwu->flash_properties.has_bl_config) {
			TP_LOG_ERR("%s: Bootloader configuration not supported\n", __func__);
			return -EINVAL;
		}
		block_count = fwu->blkcount.bl_config;
		break;
	default:
		TP_LOG_ERR("%s: Invalid config area\n", __func__);
		return -EINVAL;
	}

	if (block_count == 0) {
		TP_LOG_ERR("%s: Invalid block count\n", __func__);
		return -EINVAL;
	}
...
static int fwu_start_write_guest_code(void)
{
	...
	if (!fwu->has_guest_code) {
		TP_LOG_ERR("%s: Guest code not supported\n", __func__);
		return -EINVAL;
	}

	if (!fwu->img.contains_guest_code) {
		TP_LOG_ERR("%s: No guest code in firmware image\n", __func__);
		return -EINVAL;
	}

	if (rmi4_data->sensor_sleep) {
		TP_LOG_ERR("%s: Sensor sleeping\n", __func__);
		return -ENODEV;
	}
...
static int fwu_start_write_config(void)
{
	...
	case UI_CONFIG_AREA:
		...
		if (device_fw_id != image_fw_id) {
			TP_LOG_ERR("%s: Device and image firmware IDs don't match\n", __func__);
			return -EINVAL;
		}
		retval = fwu_check_ui_configuration_size();
		if (retval < 0)
	...
	case DP_CONFIG_AREA:
		...
		if (!fwu->img.contains_disp_config) {
			TP_LOG_ERR("%s: No display configuration in firmware image\n",	__func__);
			return -EINVAL;
		}
		retval = fwu_check_dp_configuration_size();
		if (retval < 0)
		...
		break;
	default:
		TP_LOG_ERR("%s: Configuration not supported\n", __func__);
		return -EINVAL;
	}
	...
	retval = fwu_erase_configuration();
	if (retval < 0) {
		TP_LOG_ERR("%s: Failed to erase config\n", __func__);
		goto exit;
	}
...
static int fwu_recovery_erase_all(void)
{
	...
	retval = fwu->fn_ptr->write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		TP_LOG_INFO("fwu_recovery_erase_all\n");
		#if defined (CONFIG_ZEUSIS_DSM)
...
int pad_synaptics_fw_s3718_update(void)
{
	int retval;
...
EXPORT_SYMBOL(pad_synaptics_fw_update);
...
#endif
int pad_synaptics_fw_data_s3718_init(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	...
	if (fwu) {
		TP_LOG_DEBUG("%s: Handle already exists\n",__func__);
		return 0;
	}
...
static bool is_new_carrera_module(char *file_name)
{
	...
	if (!begin_with(file_name, carrrera)){
		TP_LOG_ERR("%s: file_name: %s does not contin %s\n",__func__, file_name, carrrera);
		goto exit;
	}
	...
	TP_LOG_INFO("%s: config_id: 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",__func__, pad_s3718_config_id[0], pad_s3718_config_id[1], pad_s3718_config_id[2], pad_s3718_config_id[3]);
	if (pad_s3718_config_id[0] != 0 ||pad_s3718_config_id[1] !=0){
		return true;
	}
...
void pad_synaptics_format_fw_file_name(char* firmware_name, int firmware_name_len, char *file_name,const char *module_name)
{
 ...
 */
int pad_synaptics_get_fw_data_s3718_boot(char *file_name, struct touch_settings *synaptics_sett_param_regs)
{
	int retval;
	...
	pad_synaptics_format_fw_file_name(firmware_name, sizeof(firmware_name), file_name, synaptics_sett_param_regs->module_name);
 ...
 */
int pad_synaptics_get_fw_data_s3718_sd(void)
{
	int retval;
...
bool pad_synaptics_check_fw_s3718_version(void)
{
	enum flash_area flash_area;
	int retval = 0;

	TP_LOG_INFO("pad_synaptics_check_fw_s3718_version in\n");
...
void pad_synaptics_fw_data_s3718_release(void)
{
