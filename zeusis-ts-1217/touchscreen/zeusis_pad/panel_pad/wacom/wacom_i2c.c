...
static int wacom_chip_get_info(struct ts_chip_info_param *info)
{
	...
	TP_LOG_INFO("wacom_chip_get_info called\n");
...
{
	int retval = NO_ERR;
	TP_LOG_INFO("wacom_gpio_request\n");
...
{
	TP_LOG_INFO("wacom_regulator_enable is called\n");
	if (1 == wac_data->wacom_chip_data->vci_regulator_type) {
		if (!IS_ERR(wac_data->tp_vci)) {
...
static void wacom_power_on(void)
{
	TP_LOG_INFO("wacom_power_on called\n");
	/*1 is power supplied by gpio, 0 is power supplied by ldo*/
	...
	if ((1 == wac_data->wacom_chip_data->vci_gpio_type) && (1 == wac_data->wacom_chip_data->vddio_gpio_type)) {
		...
		TP_LOG_INFO("Only vddio was controlled by gpio\n");
		...
		struct ts_device_data *chip_data, struct platform_device *ts_dev)
{
	...
	msleep(100);
	TP_LOG_INFO("wacom chip detect called\n");
	...
	if (!wac_data) {
		TP_LOG_ERR("Failed to alloc mem for struct wac_data\n");
		return -ENOMEM;
	}
	...
	wac_data->client = g_tp_data.client;
	...
	if (!(wac_data->features)) {
		TP_LOG_ERR("%s:features malloc fail!\n",__func__);
		retval = -ENOMEM;
	...
	retval= wacom_query_device(g_tp_data.client, wac_data->features);
	if (retval < 0) {
		TP_LOG_ERR( "%s:wacom_query_device failed.\n", __func__);
		goto err_i2c_check;
	}else {
		TP_LOG_INFO("find wacom device\n");
		goto out;
	}

out:
	TP_LOG_INFO("wacom chip detect done\n");
	return NO_ERR;
...
static int wacom_parse_dts(struct device_node * device, struct ts_device_data * chip_data)
{
	...
	if (retval) {
		TP_LOG_ERR("get device WACOM_COVER_FORCE_GLOVE failed,use default!\n");
		chip_data->cover_force_glove = 0;//if not define in dtsi,set 0 to disable it
		retval = 0;
	}

	TP_LOG_INFO("reset_gpio = %d, irq_gpio = %d, irq_config = %d, algo_id = %d, x_max = %d, y_max = %d, x_mt = %d,y_mt = %d\n", \
...
{
	int ret=0;
	ret = device_create_file(&g_tp_data.ts_dev->dev, &dev_attr_pen_battery_capacity);
	if (ret) {
		TP_LOG_ERR("%s: Error, could not create attr_pen_battery_capacity\n", __func__);
	}
}
...
static int wacom_init_chip(void)
{
	...
	TP_LOG_INFO("init chip only read battery cap in sysfs\n");
	return retval;
}

static int wacom_input_config(struct input_dev *input)
{
	TP_LOG_INFO("wacom_input_config called\n");
...
static int wacom_irq_top_half(struct ts_cmd_node *cmd)
{
	...
	TP_LOG_DEBUG("wacom irq top half called\n");
	return NO_ERR;
}
...
//	struct ts_fingers *info = &out_cmd->cmd_param.pub_params.algo_param.info;
	...
	if (retval < 0) {
		TP_LOG_ERR("wacom report data error: %d\n", retval);
		return retval;
	}
...
static int wacom_gpio_reset(void)
{
	...
	if (!buf) {
	        TP_LOG_INFO("No memory resource is left: (%d)\n", ret);
		return -ENOMEM;
	}

	ret = i2c_master_send(wac_data->client, cmd, CMD_SIZE);
	if (ret < 0 || ret != CMD_SIZE) {
		TP_LOG_INFO("%s: Sending reset command failed \n", __func__);
		goto err;
	}
	...
	if (ret < 0 || ret != size) {
		TP_LOG_INFO("%s: Receving data failed \n", __func__);
		goto err;
	}
