			 ...
			 HID_DESC hid_desc)
{
	...
	if (!report_desc) {
		TP_LOG_ERR( "No memory left for this device \n");
		return -ENOMEM;
	}
	...
	if (ret < 0) {
		TP_LOG_ERR( "%s obtaining report descriptor failed \n", __func__);
		goto err;
	}
			   ...
			   u16 cmdreg, u16 datareg)
{
	...
	if (!sFeature) {
		TP_LOG_ERR( "%s cannot preserve memory \n", __func__);
		goto out;
	}
	...
	if (ret != total) {
		TP_LOG_ERR( "Sending Set_Feature failed sent bytes: %d \n", ret);
		goto err;
	}
			   ...
			   u16 cmdreg, u16 datareg)
{
	...
	if (ret != GFEATURE_SIZE) {
		TP_LOG_ERR( "%s Sending Get_Feature failed; sent bytes: %d \n", __func__, ret);
		goto err;
	}

	ret = i2c_master_recv(client, recv, (buf_size));
	if (ret != buf_size) {
		TP_LOG_ERR( "%s Receiving data failed; recieved bytes: %d \n", __func__, ret);
		goto err;
	}
...
int wacom_query_device(struct i2c_client *client, struct wacom_features *features)
{
	...
	if (ret < 0) {
		TP_LOG_ERR( "%s obtaining query failed: %d \n", __func__, ret);
		goto err;
	}
	if (ret != ARRAY_SIZE(msgs)) {
		TP_LOG_ERR( "%s input/output error occured; \n returned: %dbyte(s) \n", __func__, ret);
		ret = -EIO;
		goto err;
	}
	...
	TP_LOG_INFO( "Retrieving report descriptor \n");
	ret = retrieve_report_desc(client, features, hid_descriptor);
	if (ret < 0)
		goto err;

	TP_LOG_INFO( "addr: %x, x_max:%d, y_max:%d\n", client->addr,
	       features->x_max, features->y_max);
	TP_LOG_INFO( "addr: %x, x_touch:%d, y_touch:%d\n", client->addr,
	       features->x_touch, features->y_touch);
	TP_LOG_INFO( "pressure_max:%d, fw_version:%x \n",
