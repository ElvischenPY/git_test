...
//#include <zeusis_platform/log/hw_log.h>
...
static struct platform_device zeusis_tp_color = {
	.name = "zeusis_tp_color",
	.id = -1,
};
...
static int __init tp_color_info_init(void)
{
	...
	ret = platform_device_register(&zeusis_tp_color);
	if (ret) {
		hwlog_err("%s: platform_device_register failed, ret:%d.\n",
				__func__, ret);
		goto REGISTER_ERR;
	}
	if (device_create_file(&zeusis_tp_color.dev, &tp_color_file)) {
...
SYSFS_CREATE_FILE_ERR:
	platform_device_unregister(&zeusis_tp_color);
REGISTER_ERR:
	return ret;
...
MODULE_AUTHOR("zeusis driver group of k3");
MODULE_LICENSE("GPL");
