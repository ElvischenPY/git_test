...
unsigned char s3320_config_id[4];
...
int synaptics_get_configid(u8 *buf, size_t buf_size)
{
	//int rc = 0;
...
static enum flash_area fwu_go_nogo(struct image_header_data *header)
{
	...
	if (f01_device_status.flash_prog) {
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_flash_prog_fail;
		#endif
...
int synaptics_fw_update(void)
{
	...
	if (retval < 0) {
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_do_reflash_fail;
		#endif
	}
	/* reset device */
	...
	if (retval < 0) {
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_reset_device_fail;
		#endif
		goto exit;
	}
...
int synaptics_fw_data_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0){
		TS_LOG_ERR("Failed to scan pdt\n");
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_fw_scan_pdt_error;
		#endif
		goto exit;
	}
	...
	if (retval < 0){
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_read_f34_queries_error;
		#endif
		goto exit;
	}
...
int synaptics_get_fw_data_boot(char *file_name, struct touch_settings *synaptics_sett_param_regs)
{
	...
	if (retval != 0) {
		...
		#if defined (CONFIG_ZEUSIS_DSM)
		g_ts_data.dsm_info.constraints_UPDATE_status = fwu_request_fw_fail;
		#endif
		return retval;
	}
