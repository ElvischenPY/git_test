...
static int fwu_do_lockdown(void)
{
	...
	if (fwu->flash_properties.unlocked == 0) {
		TS_LOG_INFO("%s: Device already locked down\n", __func__);
		return 0;
	}
...
void synaptics_format_fw_file_name(char* firmware_name, int firmware_name_len, char *file_name,const char *module_name)
{
