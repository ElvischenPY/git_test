...
#define UPDATE_FILE_PATH_1          "/data/goodix_fw.img"
#define UPDATE_FILE_PATH_2          "/sdcard/goodix_fw.img"

#define CONFIG_FILE_PATH_1          "/data/goodix_config.cfg"
#define CONFIG_FILE_PATH_2          "/sdcard/goodix_config.cfg"
...
// debug-
//static struct fw_update_info update_info = {
...
{
	u8 buf[100];
	int ret = goodix_i2c_write(addr, buffer, length);
	if (ret) {
		return ret;
	}
	ret = goodix_i2c_read(addr, buf, length);
	if (ret) {
		return ret;
	}
...
{
	int reset_gpio = goodix_ts->dev_data->reset_gpio;
...
*/
int gt1x_auto_update_proc(int type)
{
	...
	if (update_info.update_type == UPDATE_TYPE_FILE) {
		...
		if (ret & (FOUND_CFG_PATH_1 | FOUND_CFG_PATH_2)) {
			...
			if (gt1x_parse_config(filename, config) > 0) {
				ret = goodix_i2c_write(GTP_REG_CONFIG_DATA, config, GTP_CONFIG_ORG_LENGTH);
				if (ret < 0) {
...
static int gt1x_search_update_files(void)
{
	int retry = 3;	//wait 10s(max) if fs is not ready
...
gt1x_update_exit:
	...
	// debug+
	goodix_ts->fw_update_ok = true;
	// debug-
	GTP_INFO("Update firmware successfully!");
exit:
	update_info.status = UPDATE_STATUS_IDLE;
	return ret;
}
...
int gt1x_update_prepare(char *filename)
{
	...
	if (filename == NULL) {
		...
		update_info.fw = NULL;
		ret = request_firmware(&update_info.fw, goodix_ts->firmware_name,
			&goodix_ts->pdev->dev);
		if (ret < 0) {
		...
		GTP_INFO("Firmware size: %zd", update_info.fw->size);
		...
		if (IS_ERR_OR_NULL(update_info.fw_file)) {
			GTP_ERROR("Open update file(%s) error:%zd!", update_info.fw_name,
							PTR_ERR(update_info.fw_file));
...
gt1x_update_pre_fail1:
	if (update_info.update_type == UPDATE_TYPE_HEADER) {
...
int gt1x_update_judge(void)
{
	...
	do {
		...
		ret = goodix_i2c_read_dbl_check(GTP_REG_FW_CHK_SUBSYS, &reg_val[1], 1);
		if (ret < 0) {
			goto _reset;
		} else if (ret > 0) {
			continue;
...
_reset:
	...
	ret = goodix_read_version(&ver_info);
	if (ret < 0) {
	...
	if (((fw_ver_info.patch_id & 0xFFFF) <= (ver_info.patch_id & 0xFFFF)) &&
							(update_info.update_type != UPDATE_TYPE_FILE)) {
		GTP_INFO("The version of the fw(%#x) is not high than the IC's(%#x)!",
				fw_ver_info.patch_id & 0xFFFF,ver_info.patch_id & 0xFFFF);
		return ERROR_CHECK;
	}
	GTP_INFO("FW upgrade from IC(%#x) to fw(%#x)",
				ver_info.patch_id & 0xFFFF,fw_ver_info.patch_id & 0xFFFF);
	return 0;
}
...
int gt1x_hold_ss51_dsp(void)
{
	...
	do {
		goodix_chip_reset();
		ret =  goodix_i2c_read(0x4220, buffer, 1);
	} while (retry-- && ret < 0);
	...
	// soft reset
	buffer[0] = 0x01;
	ret = goodix_i2c_write(_bWO_MISCTL__CPU_SWRST_PULSE, buffer, 1);
	if (ret) {
...
int gt1x_run_ss51_isp(u8 * ss51_isp, int length)
{
	...
	// load ss51 isp
	ret = goodix_i2c_write(0xC000, ss51_isp, length);
	if (ret) {
	...
	msleep(100);
	// check run state
	ret = goodix_i2c_read(0x8006, buffer, 2);
	if (ret) {
...
int gt1x_recall_check(u8 * chk_src, u16 start_addr, u16 chk_length)
{
	...
	while (chk_length > 0) {
		...
		ret = goodix_i2c_read(start_addr + compared_length, rd_buf, len);
		if (ret) {
...
int gt1x_burn_subsystem(struct fw_subsystem_info *subsystem)
{
	...
	while (length > 0 && retry > 0) {
		...
		ret = goodix_i2c_write(0x8100 + 4, fw, block_len);
		if (ret) {
		...
		while (wait_time-- > 0) {
			...
			ret = goodix_i2c_read(0x8022, buffer, 1);
			if (ret < 0) {
				continue;
			}
			msleep(5);
			ret = goodix_i2c_read(0x8022, &confirm, 1);
			if (ret < 0) {
				continue;
			}
			if (buffer[0] != confirm) {
...
int gt1x_read_flash(u32 addr, int length)
{
	...
	buffer[0] = 0xAA;
	buffer[1] = 0xAA;
	ret |= goodix_i2c_write(0x8020, buffer, 2);
	if (ret) {
	...
	while (wait_time > 0) {
		wait_time--;
		msleep(5);
		ret = goodix_i2c_read_dbl_check(0x8022, buffer, 1);
		if (ret) {
			continue;
		}
		if (buffer[0] == 0xBB) {
...
void gt1x_leave_update_mode(void)
{
	...
	if (update_info.status != UPDATE_STATUS_ABORT) {
		if (goodix_ts) {
			goodix_ts->ops.chip_reset();
			goodix_ts->noise_env = false;
			goodix_ts->ops.feature_resume(goodix_ts);
		}
	}
	update_info.status = UPDATE_STATUS_IDLE;
...
void dump_to_file(u16 addr, int length, char *filepath)
{
	...
	while (length > 0) {
		...
		memset(buf, 0x33, len);
		if (goodix_i2c_read(addr + read_length, buf, len)) {
			memset(buf, 0x33, len);
		}
...
int gt1x_load_patch(u8 * patch, u32 patch_size, int offset, int bank_size)
{
	...
	while (loaded_length != patch_size) {
		if (loaded_length == 0 || (loaded_length + offset) % bank_size == 0) {
			...
			ret = goodix_i2c_write(_bRW_MISCTL__SRAM_BANK, &bank, 1);
			if (ret) {
		...
		ret = goodix_i2c_write(address, &patch[loaded_length], len);
		if (ret) {
...
int gt1x_startup_patch(void)
{
	...
	buffer[0] = 0x00;
	buffer[1] = 0x00;
	ret |= goodix_i2c_write(_bRW_MISCTL__PATCH_AREA_EN_, buffer, 2);

	memset(buffer, 0x55, 8);
	ret |= goodix_i2c_write(GTP_REG_FLASH_PASSBY, buffer, 8);
	ret |= goodix_i2c_write(GTP_REG_VERSION, buffer, 5);

	buffer[0] = 0xAA;
	ret |= goodix_i2c_write(GTP_REG_CMD, buffer, 1);
	ret |= goodix_i2c_write(GTP_REG_ESD_CHECK, buffer, 1);

	buffer[0] = 0x00;
	ret |= goodix_i2c_write(_rRW_MISCTL__SWRST_B0_, buffer, 1);
