...
#endif
#include "zeusis_touchscreen_chips.h"
...
static ssize_t ts_loglevel_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(value == 1){
		g_ts_log_cfg = value;
		TS_LOG_ERR("--cbw-- g_ts_log_cfg");
	}else{
		noise_test();
		TS_LOG_ERR("--cbw-- noise_test");
	}
	error = count;
...
static int rawdata_proc_show(struct seq_file *m, void *v)
{
    ...
    error = put_one_cmd(cmd, LONG_SYNC_TIMEOUT);  //rawdate test timeout
	if (error) {
		TS_LOG_ERR("put cmd error :%d\n", error);
		error = -EBUSY;
		goto out;
...
static inline void ts_report_input(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	struct input_dev *slide_dev = g_ts_data.slide_dev;
	...
	if(g_ts_data.feature_info.slide_info.slide_supported && finger->slide_flag){
		for(id = 0; id < TS_MAX_SLIDE_FINGER; id++) {
			...
			if(finger->slide_fingers[id].status == TS_FINGER_PRESS) {
				TS_LOG_DEBUG("down: id is %d, finger->slide_fingers[id].pressure = %d, finger->slide_fingers[id].x = %d, finger->slide_fingers[id].y = %d\n",
	...
	if(finger->finger_flag || !g_ts_data.feature_info.slide_info.slide_supported){
		for (id = 0; id < TS_MAX_FINGER; id++) {
			if (finger->fingers[id].status == 0) {
				TS_LOG_DEBUG("never touch before: id is %d\n", id);
				continue;
...
static int ts_parse_roi_supported(void)
{
	...
	if (g_ts_data.node) {
		error = of_property_read_u8(g_ts_data.node, "roi_supported", &g_ts_data.feature_info.roi_info.roi_supported);
		if (error) {
		...
		error = of_property_read_u16(g_ts_data.node, "roi_control_addr_offset", &g_ts_data.feature_info.roi_info.roi_control_addr_offset);
		if (error) {
		...
		error = of_property_read_u16(g_ts_data.node, "roi_data_addr_offset", &g_ts_data.feature_info.roi_info.roi_data_addr_offset);
		if (error) {
...
static int ts_parse_slide_supported(void)
{
	int error = NO_ERR;
	u32 ts_slide_supported = 0;
	u32 chip_slide_supported = 0;

	if (g_ts_data.node) {//touchscreen slide support
		error = of_property_read_u32(g_ts_data.node,"ts_slide_supported",&ts_slide_supported);
		if (error) {
			TS_LOG_ERR("%s:get ts_slide_supported failed\n",__func__);
			g_ts_data.feature_info.slide_info.slide_supported = 0;
			return -EINVAL;
		}
	}

	if(g_ts_data.dev_node){//chip slide support
		error = of_property_read_u32(g_ts_data.dev_node,"chip_slide_supported",&chip_slide_supported);
		if (error) {
			TS_LOG_ERR("%s:get chip_slide_supported failed\n",__func__);
			g_ts_data.feature_info.slide_info.slide_supported = 0;
			return -EINVAL;
		}
	}
	g_ts_data.feature_info.slide_info.slide_supported = ts_slide_supported && chip_slide_supported;

	TS_LOG_INFO("ts_slide_supported=%d  chip_slide_supported=%d slide_supported=%d\n",
		ts_slide_supported,chip_slide_supported,g_ts_data.feature_info.slide_info.slide_supported);
...
static struct kobj_attribute virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.zeusis,touchscreen",
		.mode = S_IRUGO,
	},
...
static int ts_init(void)
{
	...
	g_ts_data.ts_dev = platform_device_alloc("zeusis_touch", -1);
	if (!g_ts_data.ts_dev) {
	...
	error = ts_parse_slide_supported();
	if (error) {
		TS_LOG_ERR("ts parse slide supported config failed : %d\n", error);
	}
	...
	if(g_ts_data.feature_info.slide_info.slide_supported){
		slide_dev = input_allocate_device();
		if (!slide_dev) {
			TS_LOG_ERR("failed to allocate memory for input slide dev\n");
			error = -ENOMEM;
			goto err_del_platform_dev;
		}
		...
		slide_dev->dev.parent = &g_ts_data.ts_dev->dev;
		slide_dev->open = ts_input_open;
		slide_dev->close = ts_input_close;
		g_ts_data.slide_dev = slide_dev;

		if (g_ts_data.chip_data->ops->chip_slide_config) //config input for diff chip
			error = g_ts_data.chip_data->ops->chip_slide_config(g_ts_data.slide_dev);
		if (error)
			goto err_free_dev;

		input_set_drvdata(slide_dev, &g_ts_data);

		error = input_register_device(slide_dev);
		if (error) {
			TS_LOG_ERR("input slide dev register failed : %d\n", error);
			goto err_free_dev;
		}
	}
