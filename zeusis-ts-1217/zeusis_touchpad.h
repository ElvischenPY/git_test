/*
 * Zeusis Touchpanel driver
 *
 * Copyright (C) 2013 Zeusis Device Co.Ltd
 * License terms: GNU General Public License (GPL) version 2
 *
 */
#ifndef __ZEUSIS_TOUCHSCREEN_H_
#define __ZEUSIS_TOUCHSCREEN_H_
...
#include <zeusis_platform/log/zs_log.h>
...
#endif
#define TP_DEV_NAME "zeusis,touchpad"
#define TS_SLIDE_DEV_NAME "touch_frame_input"
...
#define TP_CMD_QUEUE_SIZE  20
#define TP_MAX_FINGER 10
#define TP_MAX_SLIDE_FINGER 10
#define TP_ERR_NEST_LEVEL  5
#define TP_RAWDATA_BUFF_MAX 1600
#define TP_RAWDATA_RESULT_MAX	50
#define TP_FB_LOOP_COUNTS 100
#define TP_FB_WAIT_TIME 5

#define I2C_WAIT_TIME 25 //25ms wait period
#define I2C_RW_TRIES 3 //retry 3 times
#define I2C_DEFAULT_ADDR 0x71
...
#define CHIP_FULL_NAME "touchpad/support_chip_name_"
#define CHIP_SLAVE_ADDR_NAME "/slave_address"
#define INT_CONVERT_OFFSET	49
#endif
extern u8 g_tp_log_cfg;

#define VIRTUAL_KEY_ELEMENT_SIZE	3
#define MAX_PRBUF_SIZE	PIPE_BUF
#define HWLOG_TAG	PAD
HWLOG_REGIST();
#define TP_LOG_INFO(x...)		_hwlog_info(HWLOG_TAG, ##x)
#define TP_LOG_ERR(x...)		_hwlog_err(HWLOG_TAG, ##x)
#define TP_LOG_DEBUG(x...)	\
do { \
	if (g_tp_log_cfg)	\
		_hwlog_info(HWLOG_TAG, ##x);	\
}while(0)
#define cbw_dbg(fmt,arg...)           printk("--cbw-- [%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
//#define cbw_dbg(fmt,arg...)   //        printk("--cbw-- [%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
/* external varible*/
...
struct ts_fingers{
	struct ts_finger fingers[TP_MAX_FINGER];
	struct ts_finger slide_fingers[TP_MAX_SLIDE_FINGER];
	int cur_finger_number;
	int slide_cur_finger_number;
	bool finger_flag;
	bool slide_flag;
...
struct ts_rawdata_info{
	...
	ktime_t time_stamp;
	int buff[TP_RAWDATA_BUFF_MAX];
	char result[TP_RAWDATA_RESULT_MAX];
};
...
struct ts_chip_info_param{
	...
	u8 mod_vendor[CHIP_INFO_LENGTH*4];
...
struct ts_feature_info {
	...
	struct ts_slide_info slide_info;
};
...
struct ts_cmd_queue{
	...
	struct ts_cmd_node ring_buff[TP_CMD_QUEUE_SIZE];
};
...
struct ts_device_ops{
	...
	int (*chip_slide_config)(struct input_dev *slide_dev);
	...
	int (*chip_pocket_switch)(struct ts_pocket_info *info);
